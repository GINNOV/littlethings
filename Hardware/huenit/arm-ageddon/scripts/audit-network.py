...
from evidence_common import EvidenceError, JsonValue, exclusive_json, read_json, require_mapping, sha256_file, sha256_tree
...
def main() -> int:
    ...
    try:
        ...
        executable = app / "Contents/MacOS/Armageddon" if app.is_dir() else app
        ...
        strings = subprocess.run(["strings", str(executable)], check=False, capture_output=True, text=True)
        ...
        app_hash = sha256_tree(app) if app.is_dir() else sha256_file(app)
        value: JsonValue = {"schemaVersion": 1, "kind": "network-audit", "app": str(app), "executable": str(executable), "appSHA256": app_hash, "allowlistSHA256": sha256_file(args.allowlist.resolve(strict=True)), "matches": matches, "outcome": "PASS"}
