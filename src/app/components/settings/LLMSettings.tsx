...
export function LLMSettings() {
  const { form, setForm, updateField, updateNumberField, updateBooleanField, defaultPrompt, isDirty } = useSettingsContext();
  const {
    llmTest,
    testingLlm,
    clearingLogs,
  ...
  return (
    <SettingsSection
      ...
      description="Configure model access, prompts, and processing-log controls."
    >
      <div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-6">
        <div className="space-y-2 md:col-span-3">
          <label className="text-sm font-semibold">
            LLM base URL * <HelpTooltip text="The API endpoint for your model server (e.g. LM Studio, vLLM, or OpenAI)." />
          </label>
          <input
            type="text"
        ...
        <div className="md:col-span-3">
          <SecretField
            label={<>LLM API key <HelpTooltip text="Authentication key for your API provider. Use 'lm-studio' for local instances." /></>}
        ...
        <div className="space-y-2 md:col-span-2">
          <label className="text-sm font-semibold">
            LLM model * <HelpTooltip text="The exact model ID loaded in LM Studio or your API provider." />
          </label>
          <div className="relative" ref={historyRef}>
            <div className="flex items-center gap-2">
              ...
              >
                <svg viewBox="0 0 20 20" fill="currentColor" className={`h-5 w-5 ${fetchingModels ? "animate-spin" : ""}`}>
        ...
        <div className="space-y-2 md:col-span-2">
          <label className="text-sm font-semibold">
            LLM concurrency <HelpTooltip text="Number of parallel enrichment requests. Local models perform best with 1." />
          </label>
          <input
            type="number"
            min={1}
        ...
        <div className="space-y-2 md:col-span-2">
          <label className="text-sm font-semibold">
            Context window (Input) <HelpTooltip text="Limits the amount of input text sent to the model to prevent 'Context size exceeded' errors. Use tokens (e.g. 128000)." />
          </label>
          <input
            type="number"
            min={1000}
        ...
        <div className="space-y-2 md:col-span-2">
          <label className="text-sm font-semibold">
            Limit response length <HelpTooltip text="Maximum tokens for the generated response. Set to 0 for no limit (model default)." />
          </label>
          <input
            type="number"
            min={0}
            ...
            placeholder="2000"
            className={`w-full rounded-md border border-black/10 bg-white px-4 py-3 text-sm ${form.llmResponseLimit === 0 ? "text-emerald-700 font-bold" : ""}`}
          />
          {form.llmResponseLimit === 0 && <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-tight">No output limit active</p>}
        </div>
        <div className="space-y-2 md:col-span-2">
          <label className="text-sm font-semibold">
        ...
        <div className="space-y-2 md:col-span-6">
          <label className="text-sm font-semibold">
            LLM system prompt <HelpTooltip text="High-level instructions sent to the model to define its persona and output rules." />
          </label>
          <textarea
            value={form.llmSystemPrompt || ""}
            onChange={updateField("llmSystemPrompt")}
            placeholder="System instructions for the model..."
            className="min-h-[80px] w-full rounded-md border border-black/10 bg-white px-4 py-3 text-sm font-mono"
          />
        </div>
        <div className="space-y-2 md:col-span-6">
          <label className="text-sm font-semibold">
            LLM prompt (advanced) <HelpTooltip text="The detailed template used to guide the enrichment process. Use placeholders carefully." />
          </label>
          <textarea
            value={form.llmPrompt || ""}
            onChange={updateField("llmPrompt")}
            placeholder={defaultPrompt}
            className="min-h-[240px] w-full rounded-md border border-black/10 bg-white px-4 py-3 text-sm font-mono"
          />
          <p className="text-xs text-slate-500">
            Customize how your bookmarks are summarized. Click &quot;Reset prompt to default&quot; above to restore the Master Researcher template.
          </p>
        </div>
        <label className="flex items-start gap-3 rounded-md border border-black/10 bg-white px-4 py-3 text-sm md:col-span-3">
          ...
          <span>
            <span className="flex items-center gap-1.5 font-semibold">
              Enable LLM thinking <HelpTooltip text="When off, xBook prepends /no_think to the system prompt for models that support it. When on, that instruction is removed." />
            </span>
        ...
        <label className="flex items-start gap-3 rounded-md border border-black/10 bg-white px-4 py-3 text-sm md:col-span-3">
          ...
          <span>
            <span className="flex items-center gap-1.5 font-semibold">
              Store LLM technical logs <HelpTooltip text="Retains full prompt and response payloads for debugging in the Processing page." />
            </span>
            <span className="block text-xs text-slate-500">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        {isDirty && (
          <span className="flex items-center gap-1.5 rounded-full bg-amber-50 px-3 py-1 text-[10px] font-bold text-amber-700 border border-amber-100 uppercase tracking-tight">
