*****************************************************************
*								*
* DOS-2-DOS routines for direct access to MS-DOS drive or file.	*
*								*
*****************************************************************
...
OpenFlop:
	...
	BSR	OpenDev
	ERROR	9$
	BSR	InhibitTrkDisk		;turn off trackdisk for this flop
	JSR	DISK_INIT
9$:	RTS
...
* Disables TrackDisk driver for selected floppy by zapping drive select bits,
* causing TrackDisk to be unable to access the drive.  This appears to be a
* benign way to keep TrackDisk out of our hair, but means that we are highly
* dependent on the structure of the TrackDisk unit table.  The unit table
* has changed from time to time, causing compatibility problems...sigh.

InhibitTrkDisk:
	PUSH	D2/A2
;	CLR.B	WB2Flg
	MOVE.L	DevIOB,A2		;floppy IOB
	MOVE.L	io_Unit(A2),A2		;get trackdisk unit table ptr
	MOVE.L	$A(A2),A0		;point to driver code
	MOVE.W	#$4AFC,D0		;driver magic code
	MOVEQ	#100,D1			;limit the search
1$:	CMP.W	-(A0),D0		;search for driver romtag structure
	BEQ.S	2$
	DBF	D1,1$
	BRA	8$			;no match...
2$:	MOVE.L 	$12(A0),A0		;point to ID string
	LEA	TMP2.,A1
	BSR	Copy			;copy trackdisk id to TMP2.
	Z_STG.	TMP2.,TMP.
	SCAN.	TMP.,TMP2.		;get "trackdisk"
	SCAN.	TMP.,TMP2.		;get version
	LEA	TMP2.+1,A0
	MOVE.B	(A0)+,D0
	SWAB	D0
	MOVE.B	(A0),D0			;get second byte of version
	CMP.W	#'34',D0		;version 34 or less?
	BLE.S	3$			;yes...use old offset
	LEA	$59(A2),A0
	LEA	$36(A2),A1
	BRA.S	4$
3$:	LEA	$41(A2),A0
	LEA	$4C(A2),A1
4$:	MOVE.L	A0,SelectPtr
	MOVE.L	A1,TrackPtr

	MOVEQ	#25,D2
6$:	CALLSYS	Forbid,SysBase		;no task swapping while we check
	MOVE.L	SelectPtr,A0
	MOVE.B	(A0),D0			;GET MOTOR FLAG AND SELECT BITS
	BTST	#7,D0			;MOTOR OFF?
...
5$:	MOVE.B	D0,SelectBits		;SAVE SELECT CODE
	MOVEQ	#-1,D0
	MOVE.L	SelectPtr,A0
	MOVE.B	D0,(A0)
	MOVE.L	TrackPtr,A0
...
EnableTrkDisk:
	...
	BEQ.S	9$
	MOVE.L	D0,A0
	MOVE.B	SelectBits,(A0) 	;RESTORE DRIVE SELECT BITS
9$:	RTS
...
OpenErr.	DC.B	19,'Device open error: '
...
TokenCount	DS.B	1
CommentFlag	DS.B	1
SelectBits	DS.B	1	;saved TrackDisk select bits
RetryCount	DS.B	1
DeviceName	DS.B	40
TMP2.		DS.B	40
	END
