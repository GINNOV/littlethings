...
#ifdef PARANOID
					default:
						debug_printf("unknown case %d in main_menu_driver()\n",
							sel->function_case);
						break;
#endif PARANOID
					}
...
#ifdef DEBUG
debug_printf("in_selection(%lx %lx %lx) vis->(%d %d)\n",m, sel,vis);
#endif DEBUG
x = vis->curx - m->xoff;
y = vis->cury - m->yoff;
