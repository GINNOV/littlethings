...
switch( type )
...
#ifdef DEBUG
	debug_printf("splitting SAMPLE_OP\n");
#endif
...
#ifdef DEBUG
	debug_printf("first sample %d, last_sample %d\n", first_sample, last_sample);
#endif
	...
	*scratch++ = i = samples;
	...
	while (--i >= 0)
		{
#ifdef DEBUG
		debug_printf("(%d ", *op - firstx);
#endif
		*(scratch++) = *(op++) - firstx;
#ifdef DEBUG
		debug_printf("%d ", *op - firsty);
#endif
		*(scratch++) = *(op++) - firsty;
#ifdef DEBUG
		debug_printf("%d)\t", *op - firstz);
#endif
		*(scratch++) = *(op++) - firstz;
		}
#ifdef DEBUG
