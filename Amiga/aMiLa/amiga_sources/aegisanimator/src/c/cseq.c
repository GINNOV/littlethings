...
#ifdef DEBUG
debug_printf("c_points(%ld %lx %d %d %d)\n", fp, point, point_count, num_count,
	in_line);
#endif DEBUG
...
fp = fopen(name, "w");
if (fp == NULL)
	status_printf("can't open %s",name);
else
	{
	status_printf("saving %s",name);
	c_script(fp, ss, name, stripit);
	fclose(fp);
	}
#endif NOSAVE
