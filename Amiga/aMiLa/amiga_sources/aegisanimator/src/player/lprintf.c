...
lfprintf(file, frmt, args)
FILE *file;
char		*frmt;
char		*args;
{
ildoprnt(fsink, file, frmt, &args);
}


static char *lvsprintf(frmt, args)
char		*frmt;
char		*args;
{
...
char *s;
s = buf;
ildoprnt(ssink, &s, frmt, args);
*s = '\0';
return buf;
}


char *lsprintf(frmt, args)
char		*frmt;
char		*args;
{
return lvsprintf(frmt, &args);
}


status_printf(frmt, args)
char		*frmt;
char		*args;
{
bottom_line(lvsprintf(frmt, &args));
}
...
char		*ap;
{
	...
	if(!format)
		{
		debug_printf("NULL format string in _ildoprnt\n");
		return;
		}
	while (c = *format++)
		if ( c != '%')
