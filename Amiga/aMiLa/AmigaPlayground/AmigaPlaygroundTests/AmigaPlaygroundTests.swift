...
class AmigaPlaygroundTests: XCTestCase {
    ...
    func testEmulatorGetAvailableRoms() {
        ...
        if FileManager.default.fileExists(atPath: service.romsDirectory) {
            for rom in roms {
                let lower = rom.lowercased()
                XCTAssertTrue(lower.hasSuffix(".rom") || lower.hasSuffix(".zip"), "ROM filename \(rom) does not have expected extension")
                XCTAssertFalse(lower.hasPrefix("."), "ROM filename \(rom) should not be a hidden file")
            }
        } else {
