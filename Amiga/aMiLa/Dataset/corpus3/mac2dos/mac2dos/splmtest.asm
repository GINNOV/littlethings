...
STEP:
	MOVE.B	#NONE,CIAB		;check direction
	MOVE.W	(A2),D0
	AND.W	#$20,D0
	CMP.B	DIR,D0			;same as we want?
	BEQ.S	3$			;yes...no need to change
	BSR.S	SetDirection		;else set direction
3$:	MOVE.B	#CA0!CA1,CIAB		;now issue step command
	MOVE.B	#CA1,CIAB
...
1$:	MOVE.W	(A2),D0
	AND.W	#$20,D0
	BEQ.S	1$			;wait for step to complete
2$:	RTS
