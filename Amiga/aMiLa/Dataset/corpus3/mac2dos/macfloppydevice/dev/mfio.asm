...
PulseWidth	EQU	716	;x 1.397 usec/clock=1ms pulse width
...
2$:	MOVE.L	#50000,D0		;wait just a bit
...
MeasureSpeed:
	...
	BSR	SpeedControlOn
1$:	BSR	WaitaBit
	BSR	MeasureRPM		;measure 1 rev
	SUB.W	#196,D0			;FM...should be 200
...
* Measures number of TimerB ticks in one revolution of disk.
...
;	Disable
3$:	MOVE.B	CIAA,D0
	AND.W	#$20,D0		;get initial drive pulse state
	MOVE.B	D0,D3		;save initial state
1$:	MOVE.B	CIAA,D0
	AND.W	#$20,D0
	CMP.B	D0,D3
	BEQ.S	1$		;loop till initial state changes
	MOVE.L	COUNT,D2
	MOVEQ	#119,D1		;1 rev = 60 pulses = 120 transitions
2$:	MOVE.B	CIAA,D0		;wait for state change
	AND.W	#$20,D0
	CMP.B	D0,D3
	...
	MOVE.L	COUNT,D0
	SUB.L	D2,D0
	BMI	3$		;if number is negative...do calc again
;	Enable
