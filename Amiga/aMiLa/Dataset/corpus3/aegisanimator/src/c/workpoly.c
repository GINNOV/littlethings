...
#ifdef DEBUG
debug_printf("find_closest(%lx->(%lx %lx) %lx %lx %lx)\n",
...
#ifdef PARANOID
		default:
			debug_printf("unknown raster type %d in in_poly\n",poly->type);
			break;
#endif PARANOID
		}
	if (ghost_mode)
...
#ifdef DEBUG
debug_printf("which_whic(%lx->(%d %d) %lx )\n",vis,x,y,point_ix);
#endif DEBUG
