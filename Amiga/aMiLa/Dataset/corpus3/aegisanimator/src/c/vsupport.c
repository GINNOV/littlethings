...
#ifdef DEBUG
debug_printf("initialize_function(%lx %lx %lx %lx)\n",m, sel, vis, function);
#endif DEBUG
...
#ifdef DEBUG
debug_printf("which_segment(%lx %lx %lx %lx %lx)\n", m, sel, vis, addr, poly_list);
#endif DEBUG
...
#ifdef DEBUG
debug_printf("one_address(%lx %lx %lx %lx %d %lx %d %d\n", m, sel, vis, seq,
...
#ifdef DEBUG
debug_printf("get_address_list(%lx %lx %lx %lx %d %lx %d)\n", m, sel, vis, seq,
...
#ifdef DEBUG
debug_printf("undraw_a_list(%lx %d %lx)\n",address_list, count, poly_list);
#endif DEBUG
...
#ifdef DEBUG
debug_printf("rrub_a_list(%lx %d %lx)\n",address_list, count, poly_list);
#endif DEBUG
...
#ifdef DEBUG
debug_printf("last_coors( %lx %d (%d %d) (%d %d) )\n", op_list, op_count,
...
while (--op_count >= 0)
	...
	switch (op_type)
		...
		default:
#ifdef DEBUG
			debug_printf("unknown case in last_coors\n");
#endif
			break;
		}
	}
