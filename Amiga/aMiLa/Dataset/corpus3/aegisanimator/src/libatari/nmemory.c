...
if (mb = free_list)
	...
	if (mb->size == nbytes)
		{
#ifdef DEBUG1
		debug_printf("alloc first exact %lx\n", mb);
#endif DEBUG1
		free_list = mb->next;
		mem_free -= nbytes;
		return((int *)mb);
...
if ( nb < mb)
	...
	if ( pt+amount == (char *)mb)	/*coalesce into first block*/
		...
		nb->size += mb->size;
#ifdef DEBUG1
		debug_printf("coalescing into first chunk\n");
#endif DEBUG1
		}		
#ifdef DEBUG1
	else
		debug_printf("new first chunk\n");	
#endif DEBUG1
	return;
	}
for (;;)
	...
	if (pt-lb->size == (char *)lb)	/*coalesce into previous block*/
		...
		if (pt + amount == (char *)mb)
			...
			lb->next = mb->next;
#ifdef DEBUG1
			debug_printf("coalescing both sides\n");
#endif DEBUG1
			}
#ifdef DEBUG1
		else
			debug_printf("coalescing into previous block\n");
#endif DEBUG1
		return;
		}
	if (pt+amount == (char *)mb)	/*coalesce into next block*/
		...
		lb->next = nb;
#ifdef DEBUG1
		debug_printf("coalescing into next block\n");
#endif DEBUG1
		return;
		}
	if (nb < mb)
		{
#ifdef DEBUG1
		debug_printf("adding block in middle\n");
#endif DEBUG1
		nb->next = mb;
		lb->next = nb;
		nb->size = amount;
...
if (pt-lb->size == (char *)lb)	/*a rare case ... */
	{
#ifdef DEBUG1
	debug_printf("coalescing into end of last block\n");
#endif DEBUG1
	lb->size += amount;
	return;
	}
#ifdef DEBUG1
debug_printf("adding last block\n");
#endif DEBUG1
lb->next = nb;
nb->next = NULL;
nb->size = amount;
