...
extern struct poly *Xclip();

#define first_param 32
#define poly first_param
#define X first_param+4
...
xin_out:
	move.w	llpoint_y(a1),d0
	sub.w	llpoint_y(a0),d0
	bvs	Xclip_return0	;on overflow HUGE polygon, abort
	move.w	llpoint_x(a0),d1
	sub.w	d3,d1
	bvs	Xclip_return0	;on overflow HUGE polygon, abort
	muls	d1,d0
	move.w	llpoint_x(a0),d1
	sub.w	llpoint_x(a1),d1
	bvs	Xclip_return0	;on overflow HUGE polygon, abort
	divs	d1,d0
	add.w	llpoint_y(a0),d0
	move.w	d0,llpoint_y(a6)
	move.w	d3,llpoint_x(a6)
...
xout_in:
	move.w	llpoint_y(a0),d0
	sub.w	llpoint_y(a1),d0
	bvs	Xclip_return0	;on overflow HUGE polygon, abort
	move.w	llpoint_x(a1),d1
	sub.w	d3,d1
	bvs	Xclip_return0	;on overflow HUGE polygon, abort
	muls	d1,d0
	move.w	llpoint_x(a1),d1
	sub.w	llpoint_x(a0),d1
	bvs	Xclip_return0	;on overflow HUGE polygon, abort
	divs	d1,d0
	add.w	llpoint_y(a1),d0
	move.w	d0,llpoint_y(a6)
	move.w	d3,llpoint_x(a6)
