...
#ifdef DEBUG
debug_printf("add_raster_to_script(%lx %lx %s)\n", raster, script, name);
#endif DEBUG
...
#ifdef DEBUG
debug_printf("read_define(%lx %lx)\n", f, script);
#endif DEBUG
...
if (t->token != TYPE_TOKEN)
	{
#ifdef DEBUG
	debug_printf("define without a type line %d\n", t->lines);
#endif
	pushback_token(t);
	return;
	}
switch ((int) t->value)
	...
	case BITPLANE_RASTER:
		...
		if (t->token == UNDEF_TOKEN)
			...
			if ( !suffix_in( t->name, dot_msk) )
				status_printf("file not a %s file line %d", 
					dot_msk, t->lines);
			else
			{
	...
	default:
#ifdef PARANOID
		debug_printf("unknown type %d in read_define line %d\n", 
			t->value, t->lines);
#endif PARANOID
		break;
	}
...
switch (type)
	...
	case INSERT_RASTER:
		...
		if ( t->token != NUMBER_TOKEN)
			{
#ifdef DEBUG
			debug_printf("insert raster where? line %d\n", t->lines);
#endif
			pushback_token(t);
			}
		else
		...
		if (t->token == RASTER_TOKEN)
			...
			{
#ifdef DEBUG
			debug_printf("INSERT_RASTER - not a raster line %d\n",t->lines);
#endif
			t->fatal++;
			pushback_token(t);
			free_tube(tube);
	...
	case INSERT_STENCIL:
		...
		if ( t->token != NUMBER_TOKEN)
			{
#ifdef DEBUG
			debug_printf("insert stencil where? line %d\n", t->lines);
#endif
			pushback_token(t);
			}
		else
		...
		if (t->token == RASTER_TOKEN)
			...
			{
#ifdef DEBUG
			debug_printf("INSERT_STENCIL - not a raster line %d\n",t->lines);
#endif
			t->fatal++;
			pushback_token(t);
			free_tube(tube);
	...
	case LOAD_BACKGROUND:
		...
		if (t->token != UNDEF_TOKEN)
			{
#ifdef DEBUG
			debug_printf("LOAD_BACKGROUND - no file name\n");
#endif
			return(NULL);
			}
	...
	case INSERT_STRIP:
		...
		if ( t->token != NUMBER_TOKEN)
			{
#ifdef DEBUG
			debug_printf("insert strip where? line %d\n", t->lines);
#endif
			pushback_token(t);
			}
		else
		...
		if (t->token == RASTER_TOKEN)
			...
			{
#ifdef DEBUG
			debug_printf("INSERT_STRIP - %s not declared line %d\n",
			t->name, t->lines);
#endif
			pushback_token(t);
			t->fatal += 1;
...
for (;;)
	...
	{
	debug_printf("no tweens in %s", title);
	lsleep(1);
	fclose(f);
	free_s_sequence(script);
	free_usr_symbols(&token);
...
for(;;)
	...
	if (token.token == EOF_TOKEN)
		{
		status_printf("no *act in file %s\n",title);
		fclose(f);
		free_usr_symbols(&token);
		return(NULL);
		}
	if (token.token == BIN_TOKEN)
		{
		status_printf("%s is not an ascii file\n",title);
		fclose(f);
		free_usr_symbols(&token);
		return(NULL);
		}
