...
#ifdef DEBUG
debug_printf("remove_frome_tube_list(%lx %d %lx)\n",list, *count, element);
#endif DEBUG
...
#ifdef DEBUG
debug_printf("add_to_tube_list(%lx %d %lx)\n",list, *count, new_tube);
#endif DEBUG
new_list = (WORD **)alloc( (*count+1) * sizeof(WORD *) );
...
#ifdef DEBUG
debug_printf("free_tube_list(%lx %d)\n",list, count);
#endif DEBUG
...
#ifdef DEBUG
debug_printf("tube_compare(%lx %lx)\n", s1, s2);
#endif DEBUG
