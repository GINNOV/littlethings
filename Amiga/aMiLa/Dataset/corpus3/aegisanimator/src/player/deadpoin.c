...
#ifdef DEBUG
debug_printf("find_dead_segment(%lx %d %lx %d %lx %lx)\n",tween, poly_ix, poly,
...
free_tube(dead_list);
#ifdef DEBUG
debug_printf("next = %d this %d last = %d pt_count = %d\n",
...
#ifdef DEBUG
debug_printf("build_dead_list(%lx %d)\n", tween, poly_ix);
#endif DEBUG
...
#ifdef DEBUG
debug_printf("doto_dead_too(seq%lx %lx %lx %d)\n", seq, poly_list, list, *count);
#endif DEBUG
