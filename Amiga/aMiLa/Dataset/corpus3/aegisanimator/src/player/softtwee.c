...
{
#ifdef DEBUG
debug_printf("startup_sequence(%lx)\n");
#endif DEBUG
...
#ifdef DEBUG
debug_printf("loop_around_script( %lx )\n",ss);
#endif DEBUG
sc = ss->child_scripts;
i = ss->child_count;
while (--i >= 0)
...
#ifdef DEBUG
debug_printf("adjust_pos_in_tween( %lx )\n",ss, begin);
#endif DEBUG
...
#ifdef DEBUG
debug_printf("advance_timer(%lx %ld %d)\n",ss,toff, stop_when);
lsleep(1);
#endif DEBUG
...
#ifdef DEBUG
debug_printf("remove_dead(%lx %lx)\n", poly, dead_list);
#endif DEBUG
...
#ifdef DEBUG
debug_printf("act_on_poly_list(%lx %d %lx %d %lx)\n",
...
while (--i >= 0)
	...
	default:
#ifdef DEBUG
		debug_printf("unknown case in act_on_poly_list\n");
#endif
		break;
	}
	}
...
#ifdef DEBUG
debug_printf("end_frame(%lx %lx %lx)\n", poly_list, tween, ss);
lsleep(1);
#endif DEBUG
...
#ifdef DEBUG
debug_printf("tween_poly_list(%lx)\n", ss);
lsleep(1);
#endif DEBUG
