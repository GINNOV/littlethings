...
#ifdef DEBUG
debug_printf("find_poly_center(%lx)\n",poly);
#endif DEBUG
x = y = z = 0;
pts = poly->pt_list;
i = poly->pt_count;
...
{
#ifdef DEBUG
debug_printf("scale_point_list(%lx %d %d %d %d)\n",
...
#ifdef PARANOID
		default:
			debug_printf("unknown raster type %d in draw_pl_with\n",
			poly->type);
			break;
#endif PARANOID
		}
...
#ifdef PARANOID
		default:
			debug_printf("unknown poly type %d in draw_pl_with\n",poly->type);
			break;
#endif PARANOID
		}
	}
...
#ifdef PARANOID
		default:
			debug_printf("unknown raster type %d in find_bounding_box\n",
				poly->type);
			break;
#endif PARANOID
		}
