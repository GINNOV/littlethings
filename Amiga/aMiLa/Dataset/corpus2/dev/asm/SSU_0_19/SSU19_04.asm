...
DEEP	= 16	; hány mintát jelenítsen meg egyszerre
RENDER_W	= 320
RENDER_H	= 100
;********************
;*  Init line draw  *
...
gp_jumi:	BSR.W	STARTUP
	...
	move.l	#MEMF_CHIP,d1
	jsr	ALLOCMEM(a6)
	lea	gp_p_shader_bp0_7(pc),a0	; A filet ahova betölthetem pointer
	move.l	d0,(a0)			; 0 draw shape
	beq.w	.ESC
	
	; level_01.txt
	lea	gp_s_level_01(pc),a0	; a file neve
	move.l	#8*4*1000+2*1000,d0
	move.l	gp_p_level_ascii(pc),a1 ; a regi buff cime
	bsr.w	GP_LOAD
	lea	gp_p_level_ascii(pc),a0		; uj buffer d0-ban
	move.l	d0,(a0)
	tst.l	d0
	beq	.ESC
	...
	move.l	#3*40*256+4000,d0
	move.l	gp_p_fulke(pc),a1	; a regi buff cime
	bsr.w	GP_LOAD
	...
	; gp_s_shader. iff
	lea	gp_s_shader(pc),a0	; a file neve
	move.l	#8*80*320+4000,d0
	move.l	gp_p_shader(pc),a1	; a regi buff cime
	bsr.w	GP_LOAD
	lea.l	gp_p_shader(pc),a0	; uj buffer d0-ban
	move.l	d0,(a0)
	tst.l	d0
	beq	.ESC
	
	move.l	gp_p_shader(pc),a0
	bsr	GP_SHAD_CPY
	
	move.w	gp_debug(PC),d0
	bne	.no_pic_cpy
...
; -------- MOVE_XY ----------------------------
	lea.l	gp_dir(pc),a4
	move.l	20(a4), d4	; TARGET_XY -> tx:ty
	move.l	12(a4), d5	; POS_XY -> px:py
	
	sub.w	d5,d4	; dy = ty - py 
	asl.w	#1,d4	; dy*2
	asr.w	#3,d4	; dy/8
	add.w	d4,d5	; ty = (ty - py)*2/8
	swap	d4	; tx:ty -> ty:tx
	swap	d5	; px:py -> py:px 
	sub.w	d5,d4	; dx = tx - px
	asl.w	#1,d4	; dx*2
	asr.w	#3,d4	; dx/8
	add.w	d4,d5	; tx = (tx - px)*2/8
	swap	d5	; ty:tx -> tx:ty
	
	move.l	d5,12(a4)	; tx:ty -> POS_XY
	move.l	d5,a5	; tx:ty -> a5
	
; -------- MOVE_Z ----------------------------	
	move.l	#DEEP,d7	; mélység ( DEEP = 16)
.vx_loop	
	lea.l	gp_dir(pc),a4
	move.l	16(a4), d1	; POS_Z	
	
	divu.w	#DETAIL,d1	; d1 0-DETAIL : POS_Z/DETAIL   ( DETAIL = 16 )
	move.l	d1,-8(a4)	; d1 -> LOW_Z
...
; select PATERN & SHAPE -----------------------------------------------
	...
	move.l	a0,a1
	lsl.w	#1,d2
	adda.w	0(a3,d2.w),a0	; a0 SHAPE_START
	add.w	#2,d2
	adda.w	0(a3,d2.w),a1	; a1 SHAPE_END
; LUT ----------------------------------------------------------
...
; CUT -----------------------------------------------------------
	...
	move.l 	gp_p_shape_cut(pc), a4
	move.l	#(RENDER_H-16)*$10000+(RENDER_W-1),d6
	bsr	GP_CUT_X
		
	move.l	a2,a0
	move.l	a3,a1
	move.l	a3,a2
	move.l	#(RENDER_H-16)*$10000+(RENDER_W-1),d6	; csak a példa kedvéért( szerintem nem kell GP_CUT_X -nem bántja )
	swap	d6
	bsr	GP_CUT_Y
...
; class GP_COLED
;-------------+-------------+------------+------------+
; cél: d6 x:y | SRC: a0->a1 | vec d5 x:y | pos d6 x:y |
;-------------+-------------+------------+------------+
gp_coled_alt:	dc.w	0,0	; 0,2
gp_coled_a:	dc.w	0,0	; 4 - min_alt	: 6 - temp_alt
gp_coled_d3d2:	dc.w	0,0	; 8 - dot(pos,norm)	: 10 - dot(norm,norm)

gp_coled_pos:	dc.w	0,0	; 12 14
gp_va:	dc.w	0,0	; 16 18  
gp_da:	dc.l	0	; 20
gp_db:	dc.l	0	; 24
gp_alt:	dc.l	0	; 28
gp_base:	dc.l	0	; 32
...
GP_COLED:
	move.l	a0,a3
	lea.l	gp_coled_alt(pc),a4
	
	move.l	d6, GP_POS(a4)
	move.w	#$7fff,GP_DCP(a4)
	move.w	#$0,GP_PLUS(a4)
	move.w	#$8000,GP_DCM(a4)
	move.w	#$0,GP_MINUS(a4)
...
.return	
	...
	move.w	GP_DCP(a4),d1
	ext.l	d1
	muls.w	GP_PLUS(a4),d1
	; MINUS
	move.w	GP_DCM(a4),d2
	ext.l	d2
	muls.w	GP_MINUS(a4),d2
	
	clr.l	d0	; c = 0;
	cmp.l	#0,d1	; if( GP_PLUS*GP_DCP > 0 )
	ble	.mm	;{
	move.w	GP_DCP(a4),d0	;	c = GP_DCP;
	add.w	#1,d0
	cmp.l	#0,d2	;	if( GP_MINUS*GP_DCM > 0 )
	beq	.calc
	cmp.l	d1,d2	;	if( GP_PLUS*GP_DCP > GP_MINUS*GP_DCM )
	bge	.calc
	move.w	GP_DCM(a4),d0	;		c = GP_DCM;	
	sub.w	#1,d0
	bra	.calc	;}
.mm	
	cmp.l	#0,d2	;else if( GP_MINUS*GP_DCM > 0 )
	ble	.calc
	move.w	GP_DCM(a4),d0
	sub.w	#1,d0
.calc
...
.next	
	cmp.l	a3,a1
	beq	.return	; elértünk végig
		
	move.l	(a3)+,d1
	cmp.l	d0,d1
	beq	.next	; egyenlõre keresni kell egy tavolabbit
...
.no_end
	...
	; d3 = right*vl/rl azaz vl hoszú right
	;- Y ---
	move.w	d2,d4
	ext.l	d4
	muls.w	GP_VL+2(a4),d4
	divs.w	GP_RL+2(a4),d4
	move.w	d4,d3
	swap	d2
	swap	d3
	;- X ---
	...
	swap	d3
	add.l	d4,GP_PARA(a4)
	beq	.skip	; a line párhuzamos
	
	move.l	GP_PARA(a4),d4
	bpl	.pp
	neg.l	d4
.pp	move.l	GP_VV(a4),d3
	divs.w	d4,d3
	cmp.w	#100,d3	; if( abs(para) <= 0 )
	blt	.no_para	; a line nem párhuzamos, de kicsi a szög ;következõ vonal
	
	cmp.w	#0,GP_PLUS(a4)
	beq	.no_dcp
	cmp.l	GP_DCPXY(a4),d0
	bne	.no_dcp
	jsr	gp_bp	
	move.w	#0,GP_PLUS(a4)
	bra	.skip
.no_dcp:	cmp.w	#0,GP_MINUS(a4)
	beq	.no_dcm
	jsr	gp_bp
	cmp.l	GP_DCMXY(a4),d0
	bne	.no_dcm
	move.w	#0,GP_MINUS(a4)
.no_dcm:
...
.no_para:	
	...
	swap	d3	
	swap	d6
	move.l	d3,GP_VA(a4)	;READY: gp_va
...
.no_problem:	swap	d2
	...
	;	gp_alt = vec*gp_da/gp_db,
	move.w	d5,d4
	ext.l	d4
	muls.w	GP_DA+2(a4),d4
	divs.w	GP_DB+2(a4),d4
	move.w	d4,d3
	swap	d3
	swap	d5
	move.w	d5,d4
	...
	swap	d3
	swap	d5
	move.l	d3,GP_ALT(a4)	;READY: GP_ALT
	
	;	gp_base = gp_alt-gp_va,
	sub.w	GP_VA+2(a4),d3
	swap	d3
	sub.w	GP_VA(a4),d3
	swap	d3
	move.l	d3,GP_BASE(a4)	;READY: GP_BASE
	...
	swap	d1
	add.l	d4,d3
	; ha negatív a metszéspont valahol a vonal mögött van
	blt	.skip	; minus nem érdekes, kérem a következõ vonalat
	move.l	d3,GP_BL(a4)	;READY: GP_BL
	...
	add.l	d4,d3
	cmp.l	GP_BL(a4),d3
	; if ll < bl akkor a metszés a vonalon túl van ez sem érdekes
	blt	.skip	; kisebb nem érdekes, kérem a következõ vonalat
	
	move.l	d3,GP_LL(a4)	;READY: GP_LL
	
	;	gp_dc = gp_alt*vec
	move.w	d5,d4
	ext.l	d4
	muls.w	GP_ALT+2(a4),d4
	swap	d5
	move.w	d5,d3
	swap	d5
	muls.w	GP_ALT(a4),d3
	add.l	d3,d4
	beq	.stop	; rajta van egy vonalon az még jó
	;divs.w	GP_VV+2(a4),d4
	;-------------------------------------
	move.w	d4,GP_DC(a4)	; READY: GP_DC	
	;---------------------------------------------------------------------------------
	
	tst	d4	; if( c > 0 )
	bmi	.minus	; {
	; pozitív
	cmp.w	GP_DCP(a4),d4	;	if( cp > c )
	bgt	.skip	;	{
	move.w	d4,GP_DCP(a4)	;		cp = c;
	cmp.l	#0,GP_DB(a4)	;		if( gp_db < 0 )
	bgt	.plus_on
	move.w	#0,GP_PLUS(a4)	;			GP_PLUS = 0
	bra	.skip
.plus_on
	;beq	.skip	;		else if( gp_db > 0 )
	move.w	#1,GP_PLUS(a4)	;			GP_PLUS = +1
	move.l	d0,GP_DCPXY(a4)	;	}
	add.w	d1,GP_DCPXY+2(a4)
	swap	d1
	add.w	d1,GP_DCPXY(a4)
	swap	d1
	bra	.skip	; }
.minus	
	;beq	.skip	; else if( c < 0 )
			; {
	cmp.w	GP_DCM(a4),d4	;	if( cm < c )
	blt	.skip	;	{
	move.w	d4,GP_DCM(a4)	;		cm = c;
	cmp.l	#0,GP_DB(a4)	;		if( gp_db > 0 )
	bmi	.minus_on	
	move.w	#0,GP_MINUS(a4)	;			GP_MINUS = 0
	bra	.skip
.minus_on	
	;beq	.skip	;		else if( gp_db < 0 )
	move.w	#-1,GP_MINUS(a4)	;			GP_MINUS = -1
	move.l	d0,GP_DCMXY(a4)	;	}
	add.w	d1,GP_DCMXY+2(a4)
	swap	d1
	add.w	d1,GP_DCMXY(a4)
	swap	d1
			; }
.skip	
	move.l	-4(a3),d0
...
.view_loop:
	...
	;------------------------------------
	; + a végén kiszámítok egy x:y = 0:0 
	;-------------------------------------
	move.l	#-64,d0	; d0 px:py
	swap	d0
	sub.w	d6,d0	; d0 px:py-cy
	swap	d6
	swap	d0
...
;-------------------------------------------------------------------
; class GP_MAX_D
; y2-y1 vagy x2-x1 kozul a nagyobb az eredmény
;----------------+---------+---------+
; DEST: d3 -:max | d2 x1y1 | d3 x2y2 |
;----------------+---------+---------+
GP_MAX_D:
	move.l	d3,d4	; d4 x2:y2
	sub.w	d2,d3	; d3 x2:y2-y1
	bpl	.plus_y	; pozitív
	neg.w	d3
.plus_y:
	swap	d2	; d2 y1:x1
...
;-------------------------------------------------------------------
; class GP_QD
; p2*p2+p1*p1 a távolság négyzete
;----------------+---------+---------+
; DEST: d3 -:QD | d2 x1y1 | d3 x2y2 |
;----------------+---------+---------+
GP_QD:
	move.w	d3,d4
	sub.w	d2,d4
...
.next	
	...
	move.l	(a0)+,d2	; d2 px:py ; ez a fontos
	move.l	d1,d3	; d1 prev
	bsr	GP_MAX_D
	cmp.w	d6,d3
	blt	.next	; d3 > LOD akkor jó
...
.x_lh_good	
	...
	cmp.w	d6,d0
	bge	.x_cut_skip	;  xl > RENDER_W-1 ? x_cut_skip : continue
	cmp.w	#0,d1
	ble	.x_cut_skip	; xh < 0 ? x_cut_skip : continue
	
	cmp.w	#1,d0	; xl > 1 ? bent van akkor nem metszi a bal oldal
	bge	.xl_plus
...
.xl_plus
	cmp.w	d6,d1	;  d1 < RENDER_W-1 ? jump : cut
	blt	.xh_cut_done
...
gp_zero_xy:	dc.w	0,0	;-12 - zero x:y
...
gp_vyvy	dc.l	0	; 28
gp_coled_xy12n	dc.l	0,0,0	; 32 - 1; 36 - 2; 40 - n_colled
...
GP_COLOR_IP_SET:
	...
	LEA.L	COLORS1(PC),A2		; Pointer to the color list
	LEA.L	COLORS2(PC),A3		; Pointer to the color list
	MOVE.w	#0,D0
.loop:	
	move.w	0(a2,d0.w),d1
	move.w	0(a3,d0.w),d2
...
.no_minus	move.w	d1,d2
	...
	lsl.w	#1,d1
	lsl.w	#1,d2
	move.w	0(a2,d1.w),d3		; d3 cos
	swap	d3			; d3 cos:sin
	move.w	0(a2,d2.w), d3		; d3 sin
...
GP_SQRT_D1:	exg	d1,d0
	bsr	GP_SQRT_D0
	exg	d1,d0
	rts
	
GP_SQRT_D2:	exg	d2,d0
	bsr	GP_SQRT_D0
	exg	d2,d0
	rts
	
GP_SQRT_D3:	exg	d3,d0
	bsr	GP_SQRT_D0
	exg	d3,d0
	rts

GP_SQRT_D4:	exg	d4,d0
	bsr	GP_SQRT_D0
	exg	d4,d0
	rts
	
GP_SQRT_D5:	exg	d5,d0
	bsr	GP_SQRT_D0
	exg	d5,d0
	rts
	
GP_SQRT_D6:	exg	d6,d0
	bsr	GP_SQRT_D0
	exg	d6,d0
	rts

GP_SQRT_D7:	exg	d7,d0
	bsr	GP_SQRT_D0
	exg	d7,d0
	rts
...
gp_s_fulke:		dc.b	"ssu:fulke_raw.iff",0
		even
gp_p_fulke:		dc.l	0

gp_s_shader:	dc.b	"ssu:shader640x320_raw.iff",0
		even
gp_p_shader:	dc.l	0
;-------------------------------------------------------
; class GP_COMP
...
;public
	; IN
	gp_p_file_name:	dc.l	0	; 0 -
	gp_file_size:	dc.l	0	; 4 -
	; IN/OUT	
	gp_pp_file_buf:	dc.l	0	; 8 -	
;privat
...
GP_SHAD_CPY:
	move.l 	gp_p_shader_bp0_7(pc),a1
	add.l	#$37e,a0
	move.l	a0,a2
	add.l	#80*8*320,a2
...
.alloc	
	...
	add.l	#1,d4		; d4 += 1
	ext.l	d4		
	mulu	d1,d4		; n_new_size: d4 *= n_seg
...
;-----------------------------------------------------------
; class GP_BLIT_CPY_LUT		gp_p_work_bp0 -> gp_p_work_bp1-X
; normal d0 x:y	d1 Z(0-7):LUT	
GP_BLIT_CPY_LUT:
	movem.l	D0-D7/A0-A6,-(A7)
...
; - gp_p_work_bp0 -> gp_p_work_bp1 -----------------------------------------------------------
	move.l	gp_p_work_bp0(PC),a3
	move.l	gp_p_shader_bp0_7(pc),a4
...
.good_x1:
	...
	swap	d7
	move.w	#$0f0a, d7	; LUT == 0 -> D = !A * C : 0A
	btst	#0, d1
	beq	.wait_cpy1
	move.w	#$0fca, d7	; LUT == 1 -> D = AB + !AC : CA
...
.wait_cpy1:	btst 	#$e,$2(A6)
	...
	move.l	#-1,$44(A6)		;BLTAFWM : BLTALWM
	move.l	#0*$10000+(8*80)-40,$60(A6)	;BLTCMOD : BLTBMOD
	move.l	#0,$64(A6)		;BLTAMOD : BLTDMOD
	
	move.l	a3,$50(A6)		;BLTAPTH : BLTAPTL
	move.l	a4,$4c(A6)		;BLTBPTH : BLTBPTL
...
COLORS:
	; elõtér - azaz a fülke
	DC.W	$0000,$0123,$0323,$0230
	DC.W	$0534,$0744,$0150,$0985
	; hatter
	DC.W	$0fff,$0533,$0643,$0535
	DC.W	$0666,$0953,$0363,$0335 

COLORS1:
	; elõtér - azaz a fülke
	DC.W	$0000,$0000,$0010,$0230
	DC.W	$0121,$0232,$0121,$0985
	; hatter
	DC.W	$0fff,$0533,$0643,$0535
	DC.W	$0666,$0953,$0363,$0335
