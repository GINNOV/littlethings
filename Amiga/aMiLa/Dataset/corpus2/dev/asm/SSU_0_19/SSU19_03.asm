...
; class GP_COLED
;---------------+-------------+--------------+
; cél: d6 cx:cy | SRC: a0->a1 | cam d6 cx:cy |
;---------------+-------------+--------------+
...
.a3a1_good	
	move.l	(a3)+,d0
	lea	gp_coled_mxy(pc),a4	
.next	
	cmp.l	a3,a1
	beq	.esc	; elértünk végig
	
	move.l	(a3)+,d1
	cmp.l	d0,d1
	beq	.next	; elértünk végig
	...
	move.l	d1,d2
	sub.w	d0,d2
	neg.w	d2	; left( d1-d0 ) !! csak az egyik
	swap	d0
	swap	d2
	sub.w	d0,d2
	; neg.w	d2	; right( d1-d0 ) !! csak az egyik
	swap	d0
	...
	move.w	d2,d4
	muls.w	d3,d4
	add.w	d4,d5	;d5  - : dot(d3,d2)	
	bpl	.skip	; elvileg ha ez pozitív akor nem ütközött
		
	swap	d2
	swap	d3
	swap	d5	;d5  dot(d3,d2) : -	
	
	;--------------------------------------------
	; dot(d2,d2) -azaz- dot( norm, norm )
	;--------------------------------------------
	move.w	d2,d4
	muls.w	d4,d4
	move.w	d4,d5
	...
	move.w	d2,d4
	muls.w	d4,d4
	add.w	d4,d5	; d5  dot(d3,d2) : dot(d2,d2)
	swap	d2
	
	;--------------------------------------------
	;alt = norm*dot( pos, norm )/dot( norm, norm )
	;--------------------------------------------
	;--------- alt.y -------------------	
	swap	d5	; d5  dot(d2,d2) : dot(d3,d2)
	move.w	d2,d4	; d2.y -> d4 
	muls.w	d5,d4	; d4.y * dot(d3,d2)
	
	swap	d5	; d5  dot(d3,d2) : dot(d2,d2)
	
	divs.w	d5,d4	; d4.y * dot(d3,d2) / dot(d2,d2)
	move.w	d4,2(a4)
	...
	move.l	d2,d4
	swap	d4	; d2.x -> d4
	muls.w	d5,d4	; d4.x * dot(d3,d2)
	
	swap	d5	; d5  dot(d3,d2) : dot(d2,d2)
	
	divs.w	d5,d4	; d4.x * dot(d3,d2) / dot(d2,d2)
	move.w	d4,0(a4)
	
	move.l	0(a4), d4	; alt.xy d4.x:y	
