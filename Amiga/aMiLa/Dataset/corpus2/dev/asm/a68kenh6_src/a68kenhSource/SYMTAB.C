...
/* Gets the next statement and extracts its component parts.
	...
	ThirdOp - third operand for CAS, CAS2, PACK, UNPK
	...
	ThirdLoc- displacement to start of third operand
								...
								*/
{
	...
	Line[0] = Label[0] = OpCode[0] = SrcOp[0] = DestOp[0] = ThirdOp[0] = '\0';
	LabLoc = OpLoc = SrcLoc = DestLoc = ThirdLoc = 0;
	Src.Mode = Dest.Mode = Third.Mode = NULL;
	ErrLim = AddrAdv = InstSize = nO = nOx = nS = nD = nX = 0;
...
void GetMacLine (dummy) int dummy;
...
/* Break up Line into its component parts. */
{
	...
	*x++ = '\0';			/* Chop off size extension. */
    ...
    if (!isspace (*s) && (*s != '\0') && (*s != ';')) {
	ThirdLoc = s - Line;
	s = GetField (s, ThirdOp);	/* Op3 (extra) */
    }
}
