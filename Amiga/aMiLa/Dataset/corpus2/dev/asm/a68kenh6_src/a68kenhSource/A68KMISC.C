...
/* Moves long words or portions thereof. */
{
	...
	AppendSdata (ObjOpX, nOx);	/* Opcode extension */
...
void PutRel (addr, hunk, size) long addr, hunk; int size;
...
/* General write routine */
{
	...
	if (f == &Srec)
#ifdef MSDOS
	    j = _write (f->fd, f->Buf, i);
#else
	    j = write (f->fd, f->Buf, i);
#endif
	else j = write (f->fd, f->Buf, i);	/* Write the buffer. */
	if (j != i) {
...
void DisplayLine (dummy) int dummy;
