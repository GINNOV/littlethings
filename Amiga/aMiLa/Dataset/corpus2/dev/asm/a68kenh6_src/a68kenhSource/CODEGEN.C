...
/*==================== End unrecognized opcode =================*/
	...
	if (Oprnds3 IN AdrModeC) {
	    Third.Loc = ThirdLoc;
	    GetOperand (ThirdOp, &Third, 0);
	}
    }
...
/* ============ Begin optimizations ================= */
    ...
    else if ((Dest.Mode == ARDir) && (Src.Mode <= Imm)
...
/*		if (Src.Mode != Null)
	...
	if ((AdrModeA != 0) || (AdrModeB != 0)) {
		...
		if (Oprnds3 IN AdrModeC)
		    j = ThirdLoc + strlen (ThirdOp);
		  else j = DestLoc + strlen (DestOp);
		if ((Line[j] != '\0')
		&& (Line[j] != ';')
/* kludge */	&& (Line[j] != '{')
		&& (!isspace (Line[j]))) {
...
/* Check for special cases first */
	...
	if ((DivMul IN AdrModeC) && (Size == Long)) {
	    if (Mul IN AdrModeC)
		Op = 0x4C00;
	    else if (DivL IN AdrModeC)
		     ;
		 else Op = 0x4C40;
	    ExtL = Dest.Rn << 12;
	    if (Dest.Mode == DPair) {
...
/* Now do generalized address modes */
    ...
    if (OpCAS IN AdrModeC) {
	if (Pass2) {
	    if (Src.Mode != DReg)
		Error (SrcLoc, ModeErr);
	    if (Dest.Mode != DReg)
		Error (DestLoc, ModeErr);
	    if (Size == Byte) Op |= 0x0200;
	    if (Size == Word) Op |= 0x0400;
	    if (Size == Long) Op |= 0x0600;
	    EffAdr (&Third, mea | aea);
	    ExtL = Src.Rn + (Dest.Rn << 6);
	}
	nO += 2;
	InstSize += (nS = GetInstModeSize (Third.Mode)) + 2;
	ObjSrc = Third.Value;
    }

    if (OpCAS2 IN AdrModeC) {
	if (Pass2) {
	    if (Src.Mode != DPair)
		Error (SrcLoc, ModeErr);
	    if (Dest.Mode != DPair)
		Error (DestLoc, ModeErr);
	    if (Third.Mode != RPairI)
		Error (ThirdLoc, ModeErr);
	    if (Size == Byte) Error (OpLoc + 5, SizeErr);
	    if (Size == Word) Op |= 0x0400;
	    if (Size == Long) Op |= 0x0600;
	    ExtL = Src.Rn2 + (Dest.Rn2 << 6) + (Third.Rn << 12);
	    ObjOpX = Src.Rn + (Dest.Rn << 6) + (Third.Rn2 << 12);
	}
	nO = 2;
	nOx = 2;
	InstSize += 4;
    ...
    if (DgtCvt IN AdrModeC) {
	if (Pass2) {
	    if ((Src.Mode != DReg) && (Src.Mode != ARPre))
		Error (SrcLoc, ModeErr);
	    if (Src.Mode != Dest.Mode)
		Error (DestLoc, NGCombo);
	    if (Third.Mode != Imm)
		Error (ThirdLoc, ModeErr);
	    if (Third.Value > 65536)
        	Error (ThirdLoc, SizeErr);
	    Op |= Src.Rn + (Dest.Rn << 9) + (((Src.Mode == ARPre) & 1) << 3);
	    ExtL = Third.Value;
	}
	nO += 2;
	InstSize += 2;
	...
	if (OpM68S IN AdrModeA) {	/* EXT or EXTB (Sign extension) */
	    if (Src.Mode == DReg)
		Op |= Src.Rn;
	    else
		Error (SrcLoc, ModeErr);
	...
	if (EA05b IN AdrModeB)	/* Eff. Addr. - all except ARDir */
	    if (Dest.Mode == DReg) {
		EffAdr (&Src, dea);
		if (!(DivMul IN AdrModeC)) {
		    if (Size == Byte) Error (OpLoc + 4, SizeErr);
		    if (Size == Word) Op |= 0x0180;
		    if (Size == Long) Op |= 0x0100;
                }
		if ((DivMul IN AdrModeC) && (Size == Long))
		    ;
		else Op |= (Dest.Rn << 9);
	    } else if (Dest.Mode == DPair) {
		       EffAdr (&Src, dea);
		   }
	    else Error (DestLoc, ModeErr);

	if (EA05c IN AdrModeB)		/* BTST */
	    if (Src.Mode == DReg)
		EffAdr (&Dest, dea);	/* All but ARDir */
	    else
		EffAdr (&Dest, (1<<(Imm-1))+(1<<(ARDir-1)));	/* All but ARDir/Imm */
	...
	if (Pass2) {
	    /* temporary */
	    if (destD IN AdrModeC)
		ExtL = Src.bfCode + (Dest.Rn << 12);
	    else if (srcD IN AdrModeC)
		     ExtL = Dest.bfCode + (Src.Rn << 12);
	    else ExtL = Src.bfCode;
	}
	nO += 2;	/* Extension is part of OpCode. */
	InstSize += 2;
    }
	...
	if (Pass2) {
	    i = TRUE;
	    if (!((Src.Mode >= USP) && (Src.Mode <= VBR))
	       && !((Src.Mode == DReg) || (Src.Mode == ARDir))) {
		Error (SrcLoc, ModeErr);
		i = FALSE;
	    }
	    if (!((Dest.Mode >= USP) && (Dest.Mode <= VBR))
	       && !((Dest.Mode == DReg) || (Dest.Mode == ARDir))) {
		Error (DestLoc, ModeErr);
		i = FALSE;
	    }
	    if ((Dest.Mode >= USP)
	...
	if (EA611 IN AdrModeB) {	/* Eff. Addr. in 6-11 (MOVE) */
		...
		Op = 0x44C0;
		EffAdr (&Src, dea);
	    } else if (Src.Mode	== CCR) {	/* MOVE from CCR */
		Op = 0x42C0;
		EffAdr (&Dest, dea | aea);
		...
		if (Src.Mode > Imm)
		    Error (SrcLoc, ModeErr);
		if (((1<<(Dest.Mode-1)) IN (dea|aea)) || (Dest.Mode > Imm))
		    Error (DestLoc, ModeErr);
		else if (Dest.Mode < AbsW)	/* Register direct or indirect */
		    Op |= ((Dest.Mode - 1) << 6) | (Dest.Rn << 9);
		else		/* Absolute, PC relative, or immediate */
		    Op |= 0x01C0 | ((Dest.Mode - AbsW) << 9);
    ...
    if (((AdrModeA != 0) || (AdrModeB != 0)) && (!(Oprnds3 IN AdrModeC))) {
