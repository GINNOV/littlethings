/*
 *	This program takes two files of alphabetically sorted labels,
 *	and merges them to the standard output.
 *	The lines should be of the form <LABEL> <SPACE(S)> <VALUE>.
...
int scanline(file, line, name, value)	/* Name first */
FILE *file;
char *line;
char **name;
long *value;
...
int newscanline(file, line, name, value)	/* Value first */
FILE *file;
char *line;
char **name;
long *value;
...
char *argv[];
{
	...
	while (finished == 0) {
		...
		if (comp < 0) {	/* First name is lower */
			writeline(name1, value1);
			if (scanline(file1, line1, &name1, &value1))
				finished = 1;
		} else if (comp > 0) {	/* Second name is lower */
			writeline(name2, value2);
			if (scanline(file2, line2, &name2, &value2))
				finished = 2;
		} else {	/* Names are the same */
			if (value1 != value2) {	/* Strange... */
				fprintf(stderr, "*** CONFLICT %s = %8lx %8lx\n",
					name1, value1, value2);
				putchar(' ');	/* Make it more visible */
				writeline(name1, value1);
			}
			writeline(name2, value2);
			if (scanline(file1, line1, &name1, &value1))
