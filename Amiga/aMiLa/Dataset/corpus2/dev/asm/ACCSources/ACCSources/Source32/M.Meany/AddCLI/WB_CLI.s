...
*****	Title		Test WB startup
*****	Function	Attemping to link a CLI to a WorkBench launched
*****			utility to allow Execute() from dos.library to be
*****			used.
*****	Size		
*****	Author		Mark Meany
*****	Date Started	Oct 93
...
; If program is launched from WorkBench, a Console is opened and it's handle
;copied into the programs internal STD_OUT variable. Next the pointer to the
;Message port of this console is written into this programs Process structure
;in the pr_ConsoleTask field. All dos output is now directed into the console
;we opened. Not sure how ligitimate this is, but it works under WB3!
...
Init		moveq.l		#0,d0
		...
		beq.s		.IsDOS			yes, quit now!

	; Find our process structure and add StdIn, StdOut
...
.IsDOS		CALLDOS		Output			determine CLI handle
		move.l		d0,STD_OUT(a5)		save it for later
		beq.s		.err			quit if no handle

.IsWB		move.l		_args(a5),a0		get addr of CLI args
...
.WBStdIn	dc.b		'CON:0/200/640/50/StdIn',0
		even
...
Msg		dc.b		'In 5 seconds the Execute() function will be called:',$0a,0
		even
