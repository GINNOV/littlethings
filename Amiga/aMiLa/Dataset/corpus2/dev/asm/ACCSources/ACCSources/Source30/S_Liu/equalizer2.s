	...
	******* EQUALIZER 2 - BY KHUL IN FEB 1993 ******
	*NOTE: This is based on every new note means max (colour)
* This replay source HAS been changed but the changes can be made to nearly
* 99% of all other replay sources!!
...
equalizer:
	lea	mt_equ1,a0
	lea	gfx_equ+6,a1	point for first equalizer copper gap
	moveq	#3,d7		no. of voices

equ_lp:	tst.b	(a0)		Is flag set?
	beq	equ_no
	move.w	#$000f,(a1)	Set blue colour
	sf.b	(a0)		Clear flag
	bra.b	equ_end		and loop to next voice
	
equ_no:	tst.w	(a1)		If colour is 0 then dont subtract
	beq	equ_end
	sub.w	#1,(a1)

equ_end:add.w	#16,a1		update copper address
