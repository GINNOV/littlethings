...
;*
;* Filename         : main.s
;* Created on       : 23-Apr-92
;* Created by       : M.Meany
;* Current revision : V0.005
;*
;*
;* Purpose: Tutorial Displayer.
;*          Load & Display tutorials and examples at the same time.
;*                                                    M.Meany (23-Apr-92)
;*          
;*
;* V0.005 : Added: Saves & Restores position in a file.
;*                                                    M.Meany (02-Aug-93)
;*          
;* V0.004 : Added: A68K can be called from Mouse Menu.
;*                                                    M.Meany (20-Aug-92)
;*          
;* V0.003 : Added: CLI arguments passed to examples when run.
;*                 Choice of example to load. Double-Mouse menu.
;*                                                    M.Meany (02-May-92)
;*          
;* V0.002 : Added Status Text, Run Example, Search Next/Previous.
;*          LMB control for scrolling and switching sections.
;*                                                    M.Meany (25-Apr-92)
;*          
;* V0.001 : Load, Display & Scroll Examples.
;*          Scroll Tutorial
;*                                                    M.Meany (24-Apr-92)
;*          
...
WriteReq	rs.b		IOSTD_SIZE	IO struct for console
DStream		rs.l		5		DataStream
IEStructure	rs.b		ie_SIZEOF	input event structure
KeyBuffer	rs.b		80		for RawKeyConvert
Param		rs.b		22		CLI params for example

OldPos		rs.l		1		last line position
OldEg		rs.l		1		last Example Number
OldEgPos	rs.l		1		last Example Position
...
Openwin		lea		MyWindow,a0		a0->window args
		...
		move.l		win.ptr(a4),a0		a0->window
...
DoLoadMenu	
		lea		StrGadg2,a1		a1->gadget
		lea		KeyBuffer(a4),a0	a0->buffer
		move.l		a0,gg_SIZEOF(a1)	attach to gadget
		move.l		#0,(a0)			default empty
...
DoRunMenu	
		lea		StrGadg1,a1		a1->gadget
		lea		Param(a4),a0		a0->buffer
...
LoadPrev	lea		LoadPrevWindow,a0
		bsr		DoMenuWindow
		rts
...
SaveCurr	lea		SaveCurrentWindow,a0
		bsr		DoMenuWindow
		rts
...
; Following the IntuiText Structure will be a pointer to a string gadget or
;NULL. If a string gadget, activate it

		move.l		d6,a0			a0->NewWindow
		move.l		nw_SIZE+4(a0),d0	d0->Gadget
		beq.s		.Wait			skip if not one
		move.l		d0,a0			a0->Gadget
		move.l		men.ptr(a4),a1		a1->Window
...
.error1		move.l		win.ptr(a4),a0		Window
		move.l		#GADGETUP!CLOSEWINDOW!RAWKEY!MOUSEBUTTONS!INTUITICKS,d0 IDCMP
		CALLINT		ModifyIDCMP		do it!
...
;--------------
;--------------	Load an example specified by the user
;--------------

DoSpecify	lea		StrGadg2,a0		int gadget
		move.l		gg_SpecialInfo(a0),a0	a0->StringInfo
		move.l		si_LongInt(a0),d0	d0=integer value
		beq		DoLoadNext		exit if NULL
		bmi		.error			or negative

		addq.l		#1,d0			correct!

; See if a valid example number

		lea		control(a4),a3		file info
		cmp.l		file.lnum(a3),d0	in range
		bgt.s		.error			exit if not

; See if this example is already loaded

		cmp.l		file.topline(a3),d0	file loaded?
		beq		.error			exit if so

; Can proceed to load the file
...
; See if last example is being displayed, if it is exit now!
		...
		cmp.l		file.lnum(a3),d0	last example?
...
; Append CLI parameter list to executable name
...
;--------------
;--------------	Start ACC editor running
;--------------

; Entry		None

DoAsm		move.l		#a68kname,d1		filename
		moveq.l		#0,d2
		moveq.l		#0,d3
		CALLDOS		Execute
		moveq.l		#0,d2
...
;--------------
;--------------	Load previous position information
;--------------

LoadPrevPos	move.l		#PositionName,d1
		...
		move.l		a0,d2			buffer
		move.l		#12,d3			size
		CALLDOS		Read			Read in last position

		move.l		d7,d1
		CALLDOS		Close

		lea		tutorial(a4),a3
		move.l		OldPos(a4),file.topline(a3)	new position
		bsr		RefreshSection			display it


		lea		control(a4),a3
		move.l		OldEg(a4),d0

; Can proceed to load the example file
...
; Move into example section and display it.
		...
		lea		example(a4),a3
		move.l		OldEgPos(a4),file.topline(a3)	new position
		bsr		RefreshSection		display example
...
SaveCurPos	lea		tutorial(a4),a0
		move.l		file.topline(a0),OldPos(a4)
		lea		control(a4),a0
		move.l		file.topline(a0),OldEg(a4)
		lea		example(a4),a0
		move.l		file.topline(a0),OldEgPos(a4)

		move.l		#PositionName,d1
...
PositionName	dc.b		'tutor.position',0
		even
...
a68kname	dc.b		'M1:c/run m1:c/ACC',0
		even
...
MyWindow:
    DC.W    0,0,640,256
    DC.B    0,1
    DC.L    GADGETUP!CLOSEWINDOW!RAWKEY!MOUSEBUTTONS!INTUITICKS
    DC.L    NOCAREREFRESH+SMART_REFRESH+ACTIVATE+BORDERLESS
    DC.L    0,0
    DC.L    0
    DC.L    0,0
