...
;	S.M. (Delete most of this when you've read it)
...
QuitReq		move.l		window.ptr(a4),a0	a0-->window
...
print_top	movea.l		start_line(a4),a1	get addr of 1st line of text
		move.l		#10,linenum(a4)	set screen position
...
print_bot	movea.l		start_line(a4),a5	get addr of 1st line of text
		move.l		scrn_size(a4),d1	get num of lines on screen
		subq.l		#2,d1		adjust for last line
.loop		move.l		node.next(a5),a5	find addr of last line
		dbra		d1,.loop
		movea.l		a5,a1		move this addr into a1
		move.l		scrn_size(a4),d1	calculate screen position
		mulu.w		font.height+2(a4),d1
		sub.l		font.height(a4),d1
		add.l		#10,d1
		move.l		d1,linenum(a4)	set screen position
...
; Entry		a5 must hold address of start of current line of text

; Exit		a5 will hold address of start of next line.

start_nextln	move.l		node.next(a5),a5
		rts

*****************************************************************************

start_prevln	move.l		node.prev(a5),a5
		rts				and return
...
;***********************************************************
	...
	INCLUDE	Workdisk:ACC_Source/cursor.s

;***********************************************************

;***********************************************************

	INCLUDE	Workdisk:ACC_Source/Acc_Window.s

;***********************************************************

;***********************************************************

	INCLUDE	Workdisk:ACC_Source/Assemble_Window.s

;***********************************************************

;***********************************************************

	INCLUDE	Workdisk:ACC_Source/About_Window.s
...
;***********************************************************
	...
	INCLUDE	Workdisk:ACC_Source/Requesters.s

;***********************************************************

;***********************************************************

	INCLUDE	Workdisk:ACC_Source/Goto_Window.s

;***********************************************************

;***********************************************************

	INCLUDE	Workdisk:ACC_Source/Strings_Variables.s
