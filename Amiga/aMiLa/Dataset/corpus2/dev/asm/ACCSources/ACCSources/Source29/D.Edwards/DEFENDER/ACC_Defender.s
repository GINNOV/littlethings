...
*    blitter do all the work?
		...
		include	ACC29_B:INCLUDE/Hardware.i

;		include	VOL45:INCLUDES/MyExec2.i
;		include	VOL45:INCLUDES/MyGraf2.i
...
;		include	VOL45:INCLUDES/DefenderCheat1.i
...
GameLoop2	move.l	CurrentPlayer(a6),a0	;configure keyboard
		...
		move.l	pd_BaiterPtr(a0),d0
		move.l	d0,BaiterPtr(a6)
...
* Activate Baiter if time's up
		...
		move.l	CurrentPlayer(a6),a0
		move.l	pd_BaiterAlarm(a0),d0
		move.l	d0,pd_BaiterClock(a0)
...
* Here check for death or end of attack wave


		move.l	CurrentPlayer(a6),a0
		tst.b	pd_Flags(a0)		;ship dead?
...
GameStop2	move.w	#$777,COLOR00(a5)

		bsr	ClearScanner	;don't want sprites on...
		...
		moveq	#_PDF_DEAD,d0	;signal that this player
		move.b	d0,pd_Flags(a0)	;is now dead
...
GameStop2e	move.w	#160,Anim_XPos(a0)	;reset centre position
		...
		lea	Player1Data(a6),a0	;ptr to players data
		add.w	d0,d0			;index on longword
		add.w	d0,d0
		move.l	0(a0,d0.w),a0		;get ptr
		move.l	a0,CurrentPlayer(a6)	;change player
		move.b	pd_Flags(a0),d0		;check if dead
		and.b	#_PDF_DEAD,d0		;is new player dead?
		bne	EndGame			;skip if so

		bra.s	GameStop2d		;else set him up


* Come here if player loses life, but has more left.


GameStop2b	subq.w	#1,d0		;lose a life
		move.w	d0,pd_Lives(a0)

GameStop2d	move.l	CurrentPlayer(a6),a0	;reset the
...
GameStop1	move.w	#$070,COLOR00(a5)
...
INP_2		addq.l	#1,pd_WaveNumber(a0)	;attack wave NNNN
		...
		moveq	#0,d1		;to start with,
		move.w	#$1200,d1	;approx 4 mins to Baiter

		move.l	d1,pd_BaiterAlarm(a0)
		...
		bne.s	INP_3		;skip if not

		moveq	#10,d0
		move.w	d0,pd_Bodies(a0)	;else re-initialise Bodies
...
BPL_8		move.l	CurrentPlayer(a6),a1	;get current player
		...
		cmp.w	#_AL_BAITER,Anim_ID(a0)	;hit a Baiter?
		beq.s	BPL_5			;skip if so
...
* else Body lives.
		...
		include	acc29_B:d.edwards/defender/Defender2.i
...
SBM_3		rts
		...
		include	acc29_B:d.edwards/defender/Defender1.i
...
_Graphics	incbin	acc29_B:d.edwards/defender/graphics.bin

A1F1_G		equ	_Graphics
