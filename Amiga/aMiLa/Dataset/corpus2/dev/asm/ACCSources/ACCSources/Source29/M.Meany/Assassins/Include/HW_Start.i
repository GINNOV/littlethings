...
.Go		lea		_grafname,a1		library name
...
; Check if quit selected, no console if it was!

		tst.w		Repeater		loop or exit?
		bne.s		.AllOver		exit!

; Call selected game

		move.l		#_consolename,d1
		move.l		#1005,d2
		move.l		_DOSBase,a6
		jsr		-30(a6)			Open() a console
		move.l		d0,conHandle		save handle
		
		move.l		PlayGame,d1		program to run
		moveq.l		#0,d2			STD IN
		move.l		d0,d3			STD OUT
		jsr		-222(a6)		Execute()
		
		move.l		conHandle,d1		handle
		jsr		-36(a6)			Close()

		jmp		start			and back to menu!
		
; If started from WorkBench, reply the message now

.AllOver	tst.l		_WBmsg
		beq.s		.done
...
.done		moveq.l		#0,d0			be kind to DOS
		rts
...
.win_error	move.l		d7,a1
		move.l		$4,a6
		jsr		-$19e(a6)	CloseLibrary()

.done		rts

		Section		blank,DATA_C
...
_intname	dc.b		'intuition.library',0
		even
_consolename	dc.b		'con:0/100/320/90/Amiganuts',0
		even
conHandle	dc.l		0
