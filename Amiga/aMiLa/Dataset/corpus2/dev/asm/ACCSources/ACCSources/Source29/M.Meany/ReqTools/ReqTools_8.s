...
; File requester for Downloading now has #?.lzh pattern gadget added to it 
;for rapid selection of archived files!
...
; Alter requester attributes

TestReqTools	move.l		LoadReq,a1		a1->requester
		lea		PatTags,a0		a0-> tags
		CALLREQ		rtChangeReqAttrA	update structure

; Use requester to obtain integer and then display it.

		move.l		LoadReq,a1		a1-> request struct
...
PatTags		dc.l		RTFI_MatchPat,LzPattern
		dc.l		0

LzPattern	dc.b		'#?.lzh',0
		even

LoadTitle	dc.b		'Download File As:',0
		even

; This is the TagLists used with the requesters.
...
CenterTags	dc.l		RT_ReqPos,REQPOS_CENTERSCR	centralise
		dc.l		TAG_DONE
