		...
		LIST
*** Input.i v1.01, by M.Meany ***
		NOLIST

; v1.01		GetKey was bugged as no handshaking was taking place with
;		CIA chip. Have added Level2 interrupt handler and corrected
;		GetKey subroutine. Bug reported by Dr. L.F.W. Rowe.

; v1.00		Initial Release

; Returns ASCII code of last key pressed, 0 if no key
...
; Clear the character that we have just read

		move.b		#$2c,kb_data	set to a dead key
...
GetKey		moveq.l		#0,d0		clear the register
		move.b		kb_data,d0	get current bit
		rts
...
InitKeyboard
		move.l		#_Keyboard,$68.w	Set Level 2 68000 autovector
		move.b		#$88,CIAAICR		keyboard interrupt-shift data int
		move.b		#1,CIAATALO		timer low byte, set to 1
		move.b		#0,CIAATAHI		timer high byte, set to 0
		rts					end kb setup routine

_Keyboard	movem.l		d0-d7/a0-a6,-(sp)
		lea		$dff000,a5		a5->hardware base
		move.w		#PORTS,INTREQ(a5)	clear interrupt request
		btst		#3,CIAAICR		is Serial Port interrupt flagged?
		beq		not_kb			no, so keyboard is not originator
		moveq.b		#0,d0			clear d0
		move.b		CIAASP,d0		read keyboard data
		not.b		d0		
		ror.b		#1,d0			shift up-down bit to bit 7
		move.b		d0,kb_data		make available to other routines
		move.b		#$57,CIAACRA		start timer A, CIA-A
		clr.b		CIAASP			
h_loop		btst		#3,CIAAICR		test SP flag
		beq		h_loop			repeat until flagged

		moveq.l		#50,d0
p_loop		dbra		d0,p_loop		wait for handshake to complete

		clr.b		CIAACRA			stop timer A
not_kb		movem.l		(sp)+,d0-d7/a0-a6
		rte					finish interrupt

kb_data		dc.b		$2c			char from keyboard
		even
