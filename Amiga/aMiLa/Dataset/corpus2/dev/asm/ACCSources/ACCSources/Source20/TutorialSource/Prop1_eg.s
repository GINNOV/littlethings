...
; Example of proportional gadgets and using HorizPot & VertPot
...
_UsageText	dc.b		$0a
		dc.b		'Proportional Gadget example written for:'
		dc.b		$0a
...
DoHProp		move.l		gg_SpecialInfo(a5),a4	SpecialInfo pointer


		move.w		pi_HorizPot(a4),d0
		mulu		#200,d0
		divu		#$ffff,d0
		move.w		d0,DStream
		move.w		d0,DStream+2
...
DoVProp		move.l		gg_SpecialInfo(a5),a4	SpecialInfo pointer

		move.w		pi_VertPot(a4),d0
		mulu		#100,d0			x value
		divu		#$ffff,d0		div by fraction
		move.w		d0,DStream
		move.w		d0,DStream+2
...
HPotTemplate	dc.b		'HorizPot = %4x, %d     ',0
		even
VPotTemplate	dc.b		'VertPot  = %4x, %d     ',0
		even
...
HPotIText	dc.b		1		FrontPen
		...
		dc.l		VPotIText	next text structure

HPotText	ds.b		30		the text itself
		even

VPotIText	dc.b		1		FrontPen
		dc.b		0		BackPen
		dc.b		RP_JAM2		DrawMode
		dc.b		0		KludgeFill00
		dc.w		20		x position
		dc.w		30		y position
		dc.l		0		font
		dc.l		VPotText	address of text to print
		dc.l		0		no more text

VPotText	ds.b		30		the text itself
		even
