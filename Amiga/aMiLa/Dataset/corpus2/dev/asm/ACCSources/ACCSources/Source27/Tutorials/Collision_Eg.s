...
; Collisions: Example 1 »»» No collision detection at all! Just move the bob
...
; Game initialisations go here before interrupt is enabled.
		...
		move.w		#100,PlayerX(a4)
		move.w		#100,PlayerY(a4)
...
; User wants to go right, if move is legal update PalyerX
		...
		addq.w		#1,NewX(a4)		no, bump position
...
; User wants to go left, if move is legal update PalyerX
		...
		subq.w		#1,NewX(a4)		no, bump position
...
.Done		move.w		NewX(a4),PlayerX(a4)	update position
