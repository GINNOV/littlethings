...
; Wait for other task to signal it's ready

		move.l		SerialRead,a1		a1->request
		move.w		#CMD_READ,IO_COMMAND(a1) request a read
		move.l		#1,IO_LENGTH(a1)	send 6 bytes
		move.l		#OutBuffer,IO_DATA(a1)	address of buffer
		CALLEXEC	DoIO			issue request
...
SendLoop	move.l		SerialWrite,a1		a1->request
		move.w		#CMD_WRITE,IO_COMMAND(a1) request a read
		move.l		d5,IO_LENGTH(a1)	send entire file
		move.l		a4,IO_DATA(a1)		address of buffer
		CALLEXEC	DoIO			issue request

; Check for errors

		move.l		SerialWrite,a1
		move.b		IO_ACTUAL(a1),d0
		bne.s		NoSErrors

B52		nop

NoSErrors	add.l		d5,a4
		sub.l		d5,d4
		beq.s		.AllDone
...
SDone		move.w		#0,OutBuffer		clear buffer
		rts


PrintByte	moveq.l		#0,d0
		move.b		(a4),d0
		move.w		d0,ReqDStream
		
		lea		PBText,a1		a1-> texts
		lea		PBReq,a2		a2-> gadget defs
		suba.l		a3,a3			no special info
		lea		ReqDStream,a4		no args
		lea		QuitTags,a0		a0-> tag list
		CALLREQ		rtEZRequestA		display request
...
DReceiveLoop	move.l		SerialRead,a1		a1->request
		move.w		#CMD_READ,IO_COMMAND(a1) request a read
		move.l		#1,IO_LENGTH(a1)	complete file
		move.l		a4,IO_DATA(a1)		address of buffer
		CALLEXEC	DoIO			issue request
...
DError1		move.l		filebuff,a1		buffer
