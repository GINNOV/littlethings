...
bob_sizeof	rs.w	0
		...
		include	source10:INCLUDE/hardstart.i
...
hang		nop
		...
		move.w	AnimLock,d0	;check if disabled
		cmp.w	#LOCK_DISABLE,d0
		bne.s	hang		;no, so do nothing

		move.l	VBLCounter,d0
		and.w	#$FFF,d0
		move.w	d0,COLOR00(a5)

		move.b	OrdKey,d0	;get key press from Int2
		bmi.s	hang		;none pressed-loop back

		move.w	#0,COLOR00(a5)

		moveq	#0,d1		;clear dir spec
		cmp.b	#$4C,d0		;cursor up?
		bne.s	notup
		st	d1		;else direction=up
notup		cmp.b	#$4D,d0		;cursor down?
		bne.s	notdown
		move.b	#1,d1		;else direction=down
notdown		swap	d1
		cmp.b	#$4E,d0		;cursor right?
		bne.s	notbwd
		move.b	#1,d1		;else direction=backward
notbwd		cmp.b	#$4F,d0		;cursor left?
		bne.s	notfwd
		st	d1		;else direction=forward
notfwd		lea	ta_1,a0		;get 1st anim ptr

doall		nop

		move.l	d1,-(sp)		;debug stuff
		moveq	#0,d0
		move.l	d1,d0
		moveq	#20,d1
		moveq	#30,d2
		...
		move.l	anim_next(a0),d0	;ptr to next anim
		ext.w	d1
		add.w	d1,anim_x(a0)	;move horizontally
		swap	d1
		ext.w	d1
		add.w	d1,anim_y(a0)	;move vertically
		swap	d1

		move.l	d0,a0		;next anim pointer
		tst.l	d0		;last anim in list?
		bne.s	doall		;back for more if not
...
* Note:for this code, source A is the MASK, source B is the
* GRAPHIC DATA, source C is the BACKGROUND. NOTE THIS WELL!

		move.l	anim_scrn(a0),d2
		move.l	d2,(a2)+		;create initial CPTH/L
		move.l	bob_mask(a1),d3
		move.l	bob_image(a1),d4
		move.l	d4,(a2)+		;and BPTH/L
		move.l	d3,(a2)+		;and APTH/L
...
;		move.l	CIACounter,d0
;		and.w	#$FFF,d0		;(debug only)
;		move.w	d0,COLOR00(a5)	;change colour
...
* This routine will also change precomputed bob_soff screen offset,
* precomputed BLTCON0 & BLTCON1 to take account of changes to the
* coordinates within the main program. These can be done without
* the knowledge of the interrupt animation system happily because
* the screen offset is used to determine plot position. The inter-
* rupt code can use this changed data to recompute the above values
* to allow entire anims to be moved about the screen.
...
* next frame and recompute the screen offset etc., to allow
* on-screen movement of the entire Anim.

* First recompute screen offsets etc.

		move.w	anim_x(a0),d0	;remove this if it
		add.w	bob_xoff(a1),d0	;doesn't work
		move.w	anim_y(a0),d1
		add.w	bob_yoff(a1),d1
		move.w	d0,d2
		asr.w	#4,d0
		add.w	d0,d0		;2*int(x/16)
		ext.l	d0
		mulu	#BP_BYTEWIDE,d1	;y*screen width
		add.l	d0,d1
		move.l	d1,bob_soff(a1)	;save new location
		and.w	#$F,d2		;frac(x/16)
		ror.w	#4,d2
		or.w	#$FCA,d2
		move.w	d2,bob_bcon0(a1)
		and.w	#$F000,d2
		move.w	d2,bob_bcon1(a1)

* Now update pointers for animation.
...
ta_1		dc.l	ta_2		;no more anims?
		...
		dc.w	48,120		;x & y coords
		dc.w	0,1		;frames
		dc.w	1		;dir

ta_2		dc.l	ta_3		;no more anims?
		dc.l	0		;scrn ptr
		dc.l	ta2_bob1		;ptr to bob
		dc.l	savebuf+1312	;ptr to background save area
		dc.w	106,133		;x & y coords
		dc.w	0,4		;frames
		dc.w	1		;dir

ta_3		dc.l	0		;no more anims
		dc.l	0		;scrn ptr
		dc.l	ta3_bob1		;ptr to bob
		dc.l	savebuf+1342	;ptr to background save area
		dc.w	68,131		;x & y coords
		dc.w	0,10		;frames
		dc.w	1		;dir
...
ta2_bob1		dc.l	ta2_img1		;image
		dc.l	ta2_msk1		;mask
		dc.w	1,15		;width, height
		dc.w	4		;bitplanes
		dc.w	0,0		;anim offsets
...
ta3_bob1		dc.l	ta3_img1		;image
		dc.l	ta3_msk1		;mask
		dc.w	2,13		;width, height
		dc.w	4		;bitplanes
		dc.w	0,0		;anim offsets
...
ta3_bob2		dc.l	ta3_img1		;image
		dc.l	ta3_msk1		;mask
		dc.w	2,13		;width, height
		dc.w	4		;bitplanes
		dc.w	0,-2		;anim offsets
...
ta3_bob3		dc.l	ta3_img1		;image
		dc.l	ta3_msk1		;mask
		dc.w	2,13		;width, height
		dc.w	4		;bitplanes
		dc.w	0,-4		;anim offsets
...
ta3_bob4		dc.l	ta3_img1		;image
		dc.l	ta3_msk1		;mask
		dc.w	2,13		;width, height
		dc.w	4		;bitplanes
		dc.w	0,-6		;anim offsets
...
ta3_bob5		dc.l	ta3_img1		;image
		dc.l	ta3_msk1		;mask
		dc.w	2,13		;width, height
		dc.w	4		;bitplanes
		dc.w	0,-8		;anim offsets
...
ta3_bob6		dc.l	ta3_img1		;image
		dc.l	ta3_msk1		;mask
		dc.w	2,13		;width, height
		dc.w	4		;bitplanes
		dc.w	0,-10		;anim offsets
...
ta3_bob10	dc.l	ta3_img1		;image
		...
		incdir	source10:bitmaps1/

ta1_img1		incbin	ship1.blit
		even


ta2_img1		incbin	flame1.blit
		even

ta2_img2		incbin	flame2.blit
		even



ta3_img1		incbin	cargo1.blit
		even



ta1_msk1		incbin	ship1.mask
		even



ta2_msk1		incbin	flame1.mask
		even

ta2_msk2		incbin	flame2.mask
		even


ta3_msk1		incbin	cargo1.mask
		even
