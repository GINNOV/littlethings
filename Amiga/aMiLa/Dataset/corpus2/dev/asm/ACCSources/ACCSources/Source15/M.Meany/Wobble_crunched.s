...
; A screen 'wobble' program. Displays and wobbles a 320x200x4 picture.

; Colour map must be infront of bpl data ( ie. cmap before ).

; This version uses a raw gfx data that has been crunched using ByteRun.
...
; M.Meany, July 1991.
		...
		lea		Crunched_gfx,a5		a5->crunched gfx
		move.l		(a5),d0			d0=uncrunched size
		move.l		d0,gfxsize		save size of block
		move.l		#$10002,d1		CHIP!CLEAR
		CALLEXEC	AllocMem		get CHIP mem
		move.l		d0,Picture		save address
		beq		quit_fast		quit if error			

		move.l		a5,a0			a0->crunched data
		move.l		d0,a1			a1->buffer
		bsr		DeCrunch		decrunch gx
...
; This code assumes the colour map is before the raw bpl data.
		...
		move.l		Picture,a0	a0-> colour data
...
;--------------	Copy scroll values from table into Copper list
...
;--------------	And finish
		...
		move.l		Picture,a1	memory address of buffer
		move.l		gfxsize,d0	size of memory
...
playcon		dc.w	$2c09,$fffe		wait 0,44	line 1
		...
		dc.w	bplcon1,$0000		scroll=0
constop		dc.w	$f309,$fffe		wait 0,46	line 200
		dc.w	bplcon1,$0000		scroll=0
