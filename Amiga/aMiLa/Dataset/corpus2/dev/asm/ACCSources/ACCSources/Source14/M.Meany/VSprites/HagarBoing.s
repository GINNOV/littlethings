...
; This demo uses a very simple ( and inadequate ) collision detection routine
;to test for bob-border collisions. If left long enough the bob gets 'trapped'
;on the bottom border.

; This program demonstrates use of a bob controlled by the Gels system. The
;bob is animated and will bounce around it's window.
...
; Set up the border collision vector for this bob.

		move.l		MyBob,a0	a0->Bob struct
		move.l		bob_BobVSprite(a0),a0 a0->VSprite struct
		move.w		#1<<BORDERHIT,vs_HitMask(a0) set hitmask
		move.l		#BORDERHIT,d0
		lea		BorderCheck,a0
		move.l		window.rp,a1
...
; Yep! it's an INTUITICK, update position and swap the bob image
		...
		move.w		xval,d3		get X increment
		move.w		yval,d4		get Y increment
		add.w		d3,vs_X(a0)	update X position
		add.w		d4,vs_Y(a0)	update Y position
...
BorderCheck	and.l		#LEFTHIT!RIGHTHIT,d0
		beq.s		.updown
		neg.w		xval
		bra.s		.done
.updown		neg.w		yval
...
ImageSwap	dc.l		SprDat1		image swapping buffer
MyGelsInfo	dc.l		0
MyBob		dc.l		0

xval		dc.w		6		x increment
yval		dc.w		4		y increment
...
WindowName	dc.b		' Bob by M.Meany. ',0
		even
...
.Text		dc.b		'This bob is controlled by the Gels system. Boundary',0
		even
...
.Text		dc.b		'collision vectors are set and movement is controlled',0
		even
...
.Text		dc.b		'by monitoring INTUITICK messages at the IDCMP port.',0
		even
