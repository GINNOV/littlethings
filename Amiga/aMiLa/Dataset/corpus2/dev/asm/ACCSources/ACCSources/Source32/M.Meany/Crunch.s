...
;*
;*
;* Purpose: Small utility to crunch files. Mainly so
...
LoadFileBuff	 rs.l		1		ponter to loaded data
LoadFileBuffLen rs.l		1		byte size of loaded data
CrunchInfo	rs.l		1		for pp lib
CrunchLen	rs.l		1		size of crunched file
...
; Obtain a CrunchInfo structure

Main		move.l		#CRUN_GOOD,d0
		move.l		#SPEEDUP_BUFFLARGE,d1
		suba.l		a0,a0
		suba.l		a1,a1
		CALLNICO	ppAllocCrunchInfo
		move.l		d0,CrunchInfo(a5)	save it
		beq		.done

; Load and decrunch the data file -- we will always recrunch using GOOD!

		move.l		(a5),a0			filename
...
; Now pack the data
		...
		move.l		CrunchInfo(a5),a0	CrunchInfo
		move.l		LoadFileBuff(a5),a1	buffer
		move.l		LoadFileBuffLen(a5),d0	length
		CALLNICO	ppCrunchBuffer		crunch it!

	; Check for compression errors and exit if they occur
	
		cmp.l		#PP_CRUNCHABORTED,d0
		beq.s		.Error2
		cmp.l		#PP_BUFFEROVERFLOW,d1
		beq.s		.Error2

		move.l		d0,CrunchLen(a5)	save length of file
...
; Write decrunch header for this file

		move.l		d7,d0			handle
		move.l		#CRUN_GOOD,d1		efficiency
		moveq.l		#0,d2			no encryption
		moveq.l		#0,d3			no checksum
		CALLNICO	ppWriteDataHeader	Write the header

; Write crunched data into it

		move.l		d7,d1			handle
		move.l		LoadFileBuff(a5),d2	buffer
		move.l		CrunchLen(a5),d3	size
		CALLDOS		Write
...
.Error2		move.l		LoadFileBuff(a5),a1	buffer
		move.l		LoadFileBuffLen(a5),d0	size
		CALLEXEC	FreeMem

; Release memory for the Crunch Info structure

.Error1		move.l		CrunchInfo(a5),a0	CrunchInfo
		CALLNICO	ppFreeCrunchInfo
