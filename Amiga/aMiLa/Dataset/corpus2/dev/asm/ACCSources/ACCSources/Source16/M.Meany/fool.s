0; Basic program to test HW_BOB.i subroutines. A simple bob is controlled
;by the joystick.

; Now uses a double bufferd screen. Still some flickering when moving off
;from stationary, don't know why!

; Solved the above problem simply by restricting the area of movement of
;the bob.

; At last, I've masterd the Blitter Cookie-Cut! 
...
.nojoy		bsr		TestJoy		see if stick moved
		tst.l		d2		yes ?
		bne.s		.BeamWait	if so jump

.not_up		btst		#6,CIAAPRA	lefty ?
		beq		.done		if so, quit!

		bra.s		.nojoy		no action !
...
.where		asr.w		d2		bit 0 => right
		bcc.s		.not_right
		addq.l		#3,XPOS
		addq.l		#3,XPOS1

.not_right	asr.w		d2		bit 1 => left
		bcc.s		.not_left
		subq.l		#3,XPOS
		subq.l		#3,XPOS1

.not_left	asr.w		d2		bit 2 => down
		bcc.s		.not_down
		addq.l		#2,YPOS
		addq.l		#2,YPOS1

.not_down	asr.w		d2		bit 3 => up
		bcc.s		.y_high
		subq.l		#2,YPOS
		subq.l		#2,YPOS1

; Make sure bob is in legal area

.y_high		move.l		YPOS,d0
		cmp.l		#Y_MAX,d0
		ble.s		.y_low
		move.l		#Y_MAX,YPOS
		bra.s		.x_high

.y_low		cmp.l		#Y_MIN,d0
		bge.s		.x_high
		move.l		#Y_MIN,YPOS

.x_high		move.l		XPOS,d0
		cmp.l		#X_MAX,d0
		ble.s		.x_low
		move.l		#X_MAX,XPOS
		bra.s		.blit

.x_low		cmp.l		#X_MIN,d0
		bge.s		.blit
		move.l		#X_MIN,XPOS
...
Init		lea		BitPlane,a0	screen data address
		...
		move.l		a0,a1		
		lea		696(a1),a1	a1->mask

		moveq.l		#3,d0		width in words
		moveq.l		#29,d1		height in lines
		moveq.l		#4,d2		depth
...
; Bob: Width=2 words, height=5 lines, depth=1 plane


MyImage		Incbin	source:bitmaps/ball.bm
