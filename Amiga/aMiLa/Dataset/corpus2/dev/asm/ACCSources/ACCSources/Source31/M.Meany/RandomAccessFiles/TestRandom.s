...
Main		lea		filename,a0		filename
		move.l		#reqSize,d0		record size
		jsr		OpenRandom		open the file
		move.l		d0,handle		save handle
		beq		ERROR

; Give the file password protection :-)

		move.l		handle,a0
		lea		PWord,a1
		jsr		RandPassword

; Add the three records
...
; Check the search routine

		move.l		handle,a0
		lea		SearchData,a1
		moveq.l		#1,d0
BREAK		jsr		SearchFRec

		tst.l		d0

; Close the file

DONE		move.l		handle,a0
		jsr		CloseRandom
...
filename	dc.b		'names.data',0
		even
newfilename	dc.b		'newnames.data',0
		even

PWord		dc.b		'©M.Meany'
		even

SearchData	dc.w		0			OR mode
		dc.w		0,5
		dc.l		FindPerson

		dc.l		0			end of data

FindPerson	dc.b		'Shaun',0
		even

handle		dc.l		1

name1		dc.b		'Mark Meany ',0
		even
addr1		dc.b		'12 Hinkler Road  ',0
		even

name2		dc.b		'Shaun Meany',0
		even
addr2		dc.b		'216 Bluebell Road',0
		even

name3		dc.b		'Sara Meany ',0
		even
addr3		dc.b		'With Bev         ',0
		even
reqSize		equ		*-name3		
