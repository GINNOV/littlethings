...
HHIT		equ	90	;range span for laser strikes
...
;		bsr	GetReadyScreen
		...
		move.l	CurrentPlayer(a6),a0	;configure for
		bsr	GetPlayerKeys		;this player!

		bsr	GetReadyScreen

		bsr	NewLSList		;create landscape
...
InitVars		moveq	#0,d0
		...
		lea	__LandL1,a0
		move.l	a0,LandScape(a6)	;landscape list ptr
		lea	__LandL2,a0
		move.l	a0,LandOff(a6)	;landscape offset ptr
...
* unless order in SpecialValues changed!!!
...
;		move.b	JoyPos(a6),d0	;get joystick value
...
Reverse		tst.b	Reversing(a6)		;last reverse done?
		...
		movem.l	FFireList(a6),d4-d5	;swap firelists
		exg	d4,d5			;around
...
BPC_1		tst.w	Anim_ID(a0)	;non-scroller?
...
BPC_2b		move.l	ScanHList(a6),a2	;ptr to horiz table
		...
		lsr.w	#6,d0		;x/64 = index into table
...
BPL_3		tst.w	Anim_ID(a0)		;non-scroller?
		beq.s	BPL_8			;skip if so
...
* Create the landscape lists. List 1 is a list of bytes,
* which are indexes into a table of landscape graphic pointers.
* List 2 is a list of offsets for the blitter code. List 1
* bytes are:
...
NewLSList	move.l	LandScape(a6),a0	;list ptr
		move.l	a0,a1		;copy it

		move.w	#512,d0		;this many entries
		moveq	#0,d1		;value to insert

NLSL_1		move.b	d1,(a1)+		;insert byte for flat land
		subq.w	#1,d0		;done them all?
		bne.s	NLSL_1		;back for more if not
		...
		bsr	Random		;generate random number
		move.l	Seed(a6),d0
		and.w	#$F,d0
		add.w	#8,d0		;this is the 'bumpiness'

NLSL_2		bsr	Random		;another random number
		move.l	Seed(a6),d2	;get it
		moveq	#0,d3		;clear this
		and.w	#$1FF,d2		;offset into table
		or.b	0(a0,d2.w),d3	;check if flat land
		bne.s	NLSL_2		;back if not
		swap	d2
		and.w	#$1FF,d2		;offset into table
		or.b	0(a0,d2.w),d3	;check if flat land
		bne.s	NLSL_2		;back if not
		move.w	d2,d3
		swap	d2
		cmp.w	d2,d3		;offsets same?
		beq.s	NLSL_2		;back if so-want different ones

		move.b	d1,0(a0,d2.w)	;random peak '/' slope
		swap	d1		;change slope direction
		move.b	d1,0(a0,d3.w)	;random peak '\' slope
		swap	d1		;change slope direction
		subq.w	#1,d0		;done them all?
		bne.s	NLSL_2		;back for more if not

		move.w	#190,d0		;initial LandYHgt
		move.l	Seed(a6),d1
		and.w	#$1F,d1		;=190+int(rand(31)-16)
		sub.w	#$10,d1		;where rand(x) = a random
		add.w	d1,d0		;no. between 0 and x
...
NLSL_3		move.b	(a0)+,d2		;get landscape byte
		cmp.b	#2,d2		;uphill land?
		bne.s	NLSL_4		;skip if not
		sub.w	#2112,d1		;else move offset up

NLSL_4		move.w	d1,(a1)+		;pop in offset byte
		cmp.b	#1,d2		;was it downhill?
		bne.s	NLSL_5		;skip if not
		add.w	#2112,d1		;else move offset down for next

NLSL_5		subq.w	#1,d0		;done them all?
		bne.s	NLSL_3		;back for more if not
...
PlotLandScape	move.l	LandScape(a6),a0	;ptr to landscape table
...
;		move.w	CurrXPos(a6),d0	;get current x pos
...
;		move.w	d0,d3		;save here for now
...
* Set DeathTime to nonzero value if F10 hit
* thus allowing DeathList routines to be debugged.

* d0 corrupt


DebugDeath	move.b	OrdKey(a6),d0	;get key press
		cmp.b	#$59,d0		;death debug key?
		beq.s	DDTH_1		;skip if so
		rts			;else leave now
...
* as the determinant.
		...
		move.l	FFireColl(a6),a3	;forward fire list
		move.w	fl_YPos(a3),d5	;y pos of laser
		beq.s	DMC_7		;no laser shot - exit
		sub.w	d4,d5		;laser y - alien y = y'
		bpl.s	DMC_6
		neg.w	d5		;abs(y')
DMC_6		cmp.w	AO_YLDisp(a1),d5	;within limits?
		bcc.s	DMC_7		;skip if not

		cmp.w	#HORG_P-HHIT,d2	;x behind LtoR facing ship?
		bcs.s	DMC_7		;skip if so
		cmp.w	#HORG_P+HHIT,d2	;x ahead of RtoL facing ship?
		bhi.s	DMC_7		;skip if so

		or.b	#_AOF_HIT,AO_Flags(a1)	;signal hit
		bra.s	DMC_1

DMC_7		move.l	RFireColl(a6),a3	;reverse fire list
		move.w	fl_YPos(a3),d5	;y pos of laser
		beq.s	DMC_9		;no laser shot - exit
		sub.w	d4,d5		;laser y - alien y = y'
		bpl.s	DMC_8
		neg.w	d5		;abs(y')
DMC_8		cmp.w	AO_YLDisp(a1),d5	;within limits?
		bcc.s	DMC_9		;skip if not

		cmp.w	#HORG_P-HHIT,d2	;x behind LtoR facing ship?
		bcs.s	DMC_9		;skip if so
		cmp.w	#HORG_P+HHIT,d2	;x ahead of RtoL facing ship?
		bhi.s	DMC_9		;skip if so
...
;		move.b	ShiftKey(a6),d0	;get shift key
...
LAND1		incbin	zland1.blit	;16x1

LAND2		incbin	zland2.blit	;16x16

LAND3		incbin	zland3.blit	;16x16
...
__SPTH		dc.b	$52,$52,$42,$32,$22,$22,$12,$02
		dc.b	$F3,$F3,$E3,$D3,$C3,$C3,$B3,$A3
		dc.b	$93,$93,$83,$73,$63,$63,$53,$43
		dc.b	$33,$33,$23,$13,$03,$03,$F4,$E4
		dc.b	$D4,$C4,$C4,$B4,$A4,$94,$94,$84
		dc.b	$74,$64,$64,$54,$44,$34,$34,$24
		dc.b	$14,$04,$04,$F5,$E5,$D5,$D5,$C5
		dc.b	$B5,$A5,$A5,$95,$85,$75,$75,$65

		dc.b	$55,$45,$45,$35,$25,$15,$15,$05
		dc.b	$F0,$F0,$E0,$D0,$C0,$C0,$B0,$A0
		dc.b	$90,$90,$80,$70,$60,$60,$50,$40
		dc.b	$30,$30,$20,$10,$00,$00,$F1,$E1
		dc.b	$D1,$D1,$C1,$B1,$A1,$A1,$91,$81
		dc.b	$71,$71,$61,$51,$41,$41,$31,$21
		dc.b	$11,$11,$01,$F2,$E2,$E2,$D2,$C2
		dc.b	$B2,$B2,$A2,$92,$82,$82,$72,$62
