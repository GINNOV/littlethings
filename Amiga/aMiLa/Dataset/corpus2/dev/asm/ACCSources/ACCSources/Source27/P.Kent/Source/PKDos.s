...
******************************************************************************
*
* WORKING TITLE: PKDOS 0.33
*        AUTHOR: PAUL KENT
*      COMPILER: DEVPAC3
*        MEMORY: As defined!
*   DIARY/NOTES-
* 0.1 13/06/92 : Updated trace routines...
* 0.2 16/06/92 : Memory allocation + disk routines put in
* 0.3 20/06/92 : Support for 'Lh' encoded files :-) Who needs powerpacker?
* - Need to integrate texthandler/disk/smpi code into some coherent jump
*   table => currently access is rather messy!!!
* 0.31 22/6/92 : ALL text control now by texthandler.s
* - Need to add command line options to remove all of my '_debug' calls/checks
*   and debugging routines - showd0 etc.
* 0.32 23/6/92 : Revised bob structure, MakeMask now works OK.
*                >>>> NB Old Bob struct NOT compatible!!!! <<<<
*      24/6/92 : BUG FIX: Was still defining screen1/2!!!
* 0.33 30/6/92 : Memory alloc bug fixed.
*                MAJOR BUG FIX IN DISK.S : Drives on system detected ok.
*                Revised bob struct : new 'ignore src modulo' mask mode :-)
*                DEFBOB macro - should be more upwardly compatible
*
* Test routines do the following:
*-Flash screen.Wait for LMB, then print 96 bobs, flash screen.
*-On LMB step through several lines of code with the tracer.
*-Test keyboard: Current code printed, keymap array also printed. ESCAPE quits
*-Test jotsticks! Print joy0/1 port values. ESCAPE again quits
*
...
;Text handler code for *ALL* text output :-)
	INCLUDE	INCLUDES/TEXTHANDLER.S
;Memory alloc routines...
	INCLUDE	INCLUDES/MEMORYMNGR.S
;Disk IO/control + DOS LOADER + LH decrunch code
	IFND	NODISK
	INCLUDE INCLUDES/DISKMACROS2.I
	INCLUDE	INCLUDES/DISK.S
	INCLUDE	INCLUDES/LHDECODE.S
	INCLUDE	INCLUDES/IFF.S
	ENDC

;AMOUNT OF K TO SIMULATE...
;NB Put 0K : MEMINFO will be adjusted.
SLOWK = 0	; Only small buffers: just need disk buff+pic buffs!
FASTK = 0
CHIPK = 192
...
B_Options	rs.b	1				; Various codes for bob inits:

; BIT 0 : SET = Calculate mask
; BIT 1 : SET = Mask for blit window only : ignore source modulos

;BITS 0/1 are mutually exclusive (at the moment!)
;BIT 2 : SET = *NO* Mask : straight OR blit (NOT YET supported)
...
B_MaskMod	rs.w	1				; Reset modulo for mask
B_len		rs.w	1

;DEFBOB MACRO:
;\1 = BOB LABEL NAME
;\2 = OPTIONS
;\3 = WORDS WIDTH OF BLIT
;\4 = HEIGHT
;\5 = EXTRA WIDTH OF ENTIRE SOURCE (WORDS)
;\6 = BITMAP LABEL OR !NOT DEF!
;\7 = MASK LABEL OR !NOT DEF!
...
\1	DC.L	'BOB0'
	...
	DS.W	4
	ENDM
;eg. 32 pixels height:full>blank over 9 frames total, two words wide
;	DEFBOB	Test_Bobs,B_OPT_CALC,2,32,16,TB_BMAP,TB_Mask
...
;Batch of test routine calls follow...
;
;************************************
;Check bob & init code...
	move.l #lw.dispfxcols,plw.cols(a5)
	BSR LoadCopCols
	lea Test_bobs,a0				; Must be iniutialised (!)
	Bsr InitBob
	FLASHBGCOL
TTB_w	mouse TTB_w

	moveq #12-1,d4					; Do 12 times
	move.l p.showpl(a5),a1
	sub.l a2,a2
TTB_lp2	

	moveq #8-1,d3					; 8 frames -1 for dbra (9th is blank!)

	move.w d4,d0					; Put at (16*d4,104)
	lsl.w	#4,d0					; *16
	move.w #104,d1
	moveq #0,d2						; Bob offset for frame #0
TTB_lp	Bsr DoBob
	add.l #16,d0					; X-add
	addq  #6,d1						; Y-add
	addq.l #4,d2					; Next frame...
	dbra d3,TTB_lp
	dbra d4,TTB_lp2
...
;Test trace code...
	...
	move.l #lw.WhiteCols,plw.cols(a5)	; Make sure text is visible!!!
	BSR LoadCopCols
	SetTrace
	MOVE.L #$ABADCAFE,D0
	MOVE.L #$DEADF00D,D1
...
;************************************
;Check keyboard code is OK...
;'Print' keymap at top...

klp	lea keymap(pc),a0
	move.l p.showpl(a5),a1
	move.w #$7f,d0
kmlp2	move.w #plwidb-1,d1
kmlp1
	move.b (a0)+,(a1)+
	subq.b #1,d0
	beq.s kmlpfin
	dbra d1,kmlp1
	lea plwidb*(npl-1)(a1),a1
	bra.s kmlp2
kmlpfin
	moveq	#0,d0
	move.b  b.ordkey,d0
	moveq	#0,d1					; Print at (0,7) on screen
	moveq	#7,d2
	bsr ShowD0
	cmp.b	#$45,d0					; ESCAPE?
	bne.s klp
...
;************************************
;Check revised timer routine in disk
;drivers...

	ifnd	nodisk
	MOVEQ #100,D1
Wlp
	move.w 6(a6),color00(a6)
	MOVEQ #10,D0
	BSR WAIT
	dbra d1,wlp
	endc
...
InitBob
	...
	move.b	b_options(a0),d0		; Check only supported bits set
	and.b	#%11111100,d0
	bne	InitBob_Nsuperror
...
InitBob_NoMask
	...
	btst	#0,B_Options(a0)		; Need to calc mask ?
	beq.s	InitBob_maskok
	bsr		MakeBobMask
	bclr	#0,B_Options(a0)		; Now have a mask
InitBob_maskok
	...
	move.w	B_CRMod(a0),B_MaskMod(a0)	; Set mask modulo as per sig...
	btst	#1,B_options(a0)
	beq.s InitBob_FullMask
	move.w  #-2,B_MaskMod(a0)			; Ignore source modulo!
InitBob_FullMask
...
Initbob_nsuperror
	lea Ib.ns.txt(pc),a0
	bra _Error
IB.ns.txt	dc.b	'INITBOB ERROR : Unsupported options in ''BOB0'' struct',0
	even
...
;	bne	DoBob_NoMask
	...
	btst	#1,B_options(a0)
	bne.s	DoBob_Win
	lea	(a2,d2.w),a2				; Dont need to move mask window...
DoBob_Win
									; D ptr is a1
	SBLITWAIT	a6
	...
	move.w	B_MaskMod(a0),bltamod(a6)
...
; 32 pixels height:full>blank over 9 frames total, two words wide
	DEFBOB	Test_Bobs,B_OPT_CALC,2,32,16,TB_BMAP,TB_Mask

TB_BMAP	incbin	gfx/Spinblocks.irawc
lw.Dispfxcols = *-32*2
TB_MASK	ds.b	9*4*5*32	; 9 Frames, 4 bytes width, 5 planes , 32 height
