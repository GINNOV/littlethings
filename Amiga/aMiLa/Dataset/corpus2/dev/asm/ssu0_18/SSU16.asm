...
.vx_cut
	...
	; ha y2 < y1 akkor felcserélem, így csak egy esetet kell programozni ----------------------------------
	cmp.w	d1,d2
	beq	.vx_skip_y
	cmp.w	d1,d2
	bge	.no_chg_y1y2
...
.d1d2_good1
	...
	swap	d2	; d2 yx2
	;bclr	#0,d6
	; ha x2 < x1 akkor felcserélem, így csak egy esetet kell programozni ----------------------------------
	cmp.w	d1,d2
	bge	.no_chg_x1x2
	bset	#0,d6	; d6:0 megjegyzem, hogy felcseréltem majd ha kész vissza cserélem
	exg	d1,d2
.no_chg_x1x2
	cmp.w	#RENDER_W-1,d1
	bge	.vx_skip_x
...
.no_neg_xl	
	...
	move.l	d2,d3	; d3 y : RENDER_W-1
	swap	d3	; d3 RENDER_W-1 : y
...
.cut_done
	cmp.l	a5,a6
	beq	.no_open_in
	move.l	#(RENDER_W-1)*$10000+RENDER_H-1,d3
