...
.vx_loop	
	...
	ext.l	d2 
	; pos in level_index
	add.l	d7,d2		; d2 Z/DETAIL + DEEP-0
...
; select patern & shape -----------------------------------------------
	...
	move.l	a1,d0
; VIEW ---------------------------------------------------------
	adda.w	0(a3,d2.w*2),a0	; SRC_START
	add.w	#1,d2
	adda.w	0(a3,d2.w*2),a1	; SRC_END
...
.x_cut
	...
	cmp.w	d1,d2
	bge	.x_lh_good
	bset	#0,d6	; d6 megjegyzem, hogy felcseréltem majd ha kész vissza cserélem
	exg	d1,d2
.x_lh_good
	; d1 y? : xl
	; d2 y? : xh
	...
	cmp.w	#0,d2
	blt	.x_cut_skip	; 0 > xh ? x_cut_skip : continue
...
.xl_plus
	cmp.w	#RENDER_W-1,d2	;  RENDER_W > d2 ? jump : cut
	blt	.xh_cut_done
	
	move.w	#RENDER_W-2,d3
	sub.w	d1,d3	; 319-xl
	sub.w	d1,d2	; xh-xl
	beq	.x_cut_skip
	...
	swap	d2	; d2 y : x
	move.w	#RENDER_W-2,d2	;d2 y : RENDER_W-1
	
	move.l	d2,d3	; d3 y : RENDER_W-2
	swap	d3	; d3 RENDER_W-2 : y
	btst	#0,d6	; ha #0 bit 1 fordított az egyenesen azaz belép a képernyõre ( Z = 0 ) 
	bne	.stp_in	; Zbit == 0 ? (in) jump : (out) ;
	; step out ------------------------------------- d3 RENDER_W-1 : out_y
.stp_out
	bset	#1,d6		; készül out
	move.l	d3,(a2)+		; point RENDER_W-1 : out_y
	;move.w	#RENDER_H-1, d3	; d3 RENDER_W-1 : RENDER_H
	move.w	#0, d3
	cmp.l	a5,a6
	beq	.no_pre_in
	move.w	(a6),d4		; d4 in_y
	cmp.w	-2(a2),d4		
	bhi	.no_pre_in	; out_y < in_y ? jump : in_y
	move.w	(a6)+, d3
.no_pre_in
	move.l	d3,(a2)+		; point RENDER_W-1 : RENDER_H
	bra	.xh_cut_done
	
.stp_in
	; step in --------------------------------------- d3 RENDER_W-1 : in_y
	bclr	#1,d6
	beq	.no_out
	; van out akkor a plusz vertexét szeretném átírni
	move.l	d3,-12(a2)	; point RENDER_W-1 : in_y
	bra	.xh_cut_done
.no_out	
	move.w	d3,-(a6)		; point RENDER_W-1 : in_y
	
.xh_cut_done
	bclr	#0,d6	; d6:0 vissza cseréljam?
	beq	.x_lh_good2
	exg	d1,d2	; csere
.x_lh_good2
	swap	d1	; d1 x1 : y1
...
.x_cut_skip:
	...
	cmp.l	a5,a6
	beq	.x_cut_done
	move.l	#(RENDER_W-1)*$10000+RENDER_H-2,d3
	;move.l	#(RENDER_W-2)*$10000+0,d3
	move.l	d3,(a2)+
	move.w	(a6)+, d3
	move.l	d3,(a2)+
...
.ylh_good
	...
	cmp.w	#0,d2
	blt	.y_cut_skip	; 0 > xh ? x_cut_skip : continue
...
.yl_plus
	...
	move.w	#RENDER_H-2,d3
	sub.w	d1,d3	; (RENDER_H-2)-yl
	sub.w	d1,d2	; yh-yl
...
.view_loop:
	move.l	(a1)+,d0	; d0 px:py 
	sub.w	d6,d0	; d0 px:py-cy
	swap	d6
	swap	d0
	sub.w	d6,d0	; d0 py-cy:px-cx
	
	swap	d0	; d0 px-cx:py-cy
	
	move.w	d4,d2	; d2 :uy
	ext.l	d2
	move.w	d5,d3	; d3 :vy
	ext.l	d3
	muls.w	d0,d2	; (py-cy)*uy
	muls.w	d0,d3	; (py-cy)*vy
	add.l	d2,d3	; (py-cy)*uy + (py-cy)*vy
	asr.l	#8,d3	; ((py-cy)*uy + (py-cy)*vy) / 256
	move.w	d3,d1
	...
	move.w	d5,d3	; d3 :vx
	ext.l	d3
	muls.w	d0,d2	; (px-cx)*ux
	muls.w	d0,d3	; (px-cx)*vx
	add.l	d2,d3	; (px-cx)*ux + (px-cx)*vx
	asr.l	#8,d3	; ((px-cx)*ux + (px-cx)*vx) / 256
	move.w	d3,d1
...
.proj_loop:
	...
	divs.w	d5,d3	; d2 y /= div ;( /Z )
	divs.w	d5,d2	; d2 x /= div ;( /Z )
	swap	d5
...
.xl_plus
	cmp.w	#RENDER_W-1,d1	;  RENDER_W > d1 ? jump : cut
	blt	.xh_cut_done
	
	move.w	#RENDER_W-2,d2
	sub.w	d0,d2	; 319-xl
	sub.w	d0,d1	; xh-xl
	beq	.x_cut_skip
	...
	swap	d1	; d1 xh-xl
	divs.w	d1,d2	; d2 (yh-yl)*(319-xl) / (yh-yl) 
	add.w	d0,d2	; d2 ((yh-yl)*(319-xl) / (yh-yl)) + yl
	swap	d0	; d0 xl
	swap	d1	; d1 yh
	move.w	d2,d1	; d1 y = ((yh-yl)*(319-xl) / (yh-yl)) + yl
	swap	d1	; d1 y : x
	move.w	#RENDER_W-2,d1	;d1 y : RENDER_W-1
...
.stp_out
	...
	;move.w	#RENDER_H-1, d2	; d2 RENDER_W-1 : RENDER_H
	move.w	#0, d2
	cmp.l	a4,a5
	beq	.no_pre_in
...
.x_cut_skip:
	...
	cmp.l	a4,a5
	beq	.x_cut_done
	move.l	#(RENDER_W-1)*$10000+RENDER_H-2,d2
...
.wait_loop:
	...
	move.l	a4,$48(a6)	; BLT.C.PTH a4 = adr + yl / BYTEWIDTH + xl / 8
	move.l	a4,$54(a6)	; BLT.D.PTH
...
gp_s_fulke:	dc.b	"PROGDIR:fulke5.iff",0
		even
gp_p_fulke:	dc.l	0
