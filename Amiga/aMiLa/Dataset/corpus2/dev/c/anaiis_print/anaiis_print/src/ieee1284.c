...
/* 18-Jul-2011 Hang up when opening/calling in Forbid mode      */
/* 29-Mar-2011 THIS CODE HAS PROBLEMS !!!!                      */
/* 15-Mar-2011 Change task to process... pff                    */
...
#define BUFFERSIZE   1 /*100*/
#define TRACE          1
#define PRINTERPROCESS 0
#define DOOUTPUT       1

#define VERSION      1
#define REVISION     8
#define VSVERSION  "1"
#define VSREVISION "08"
...
)
{
  ...
  for (i = 0 ; i < NUM_ENTRIES(db->db_Unit) ; i++)
  {
    ...
    sprintf(db->db_Unit[i].UnitName, "ieee1284.device[%ld]", i) ;

    db->db_Unit[i].du_Unit.unit_MsgPort.mp_Node.ln_Name = db->db_Unit[i].UnitName ;
...
)
{
  ...
  if ((unitNumber >= 0) && (unitNumber < NUM_ENTRIES(db->db_Unit)))
  {
    ...
    if (du->du_Unit.unit_OpenCnt == 0)
    {
      ...
      if (DOSBase != NULL)
      {
        process = CreateProc(db->db_Library.lib_Node.ln_Name,
                             0,
                             MKBADDR(segptr),
                             8000) ;
      }
                  ...
                  else
                  {
                    ...
                    if (error == 0)
                    {
                      ...
                      if ((du->ep0 != NULL) && (du->ep != NULL))
                      {
                        ...
                        USBSetInterfaceAttrs(du->ifc, USBA_FD_Title, du->UnitName, TAG_DONE) ;
