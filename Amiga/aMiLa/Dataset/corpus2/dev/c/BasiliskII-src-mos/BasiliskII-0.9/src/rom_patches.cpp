...
static void list_rom_resources(void)
{
	...
	for (;;) {
		...
		printf("%08x %c%c%c%c\t%d\t%ld\t%s\n", (unsigned int)data, (int)ReadMacInt8(lp + 16), (int)ReadMacInt8(lp + 17), ReadMacInt8(lp + 18), ReadMacInt8(lp + 19), ReadMacInt16(lp + 20), ReadMacInt32(ROMBaseMac + data - 8), name);
...
static void print_universal_info(uint32 info)
{
	...
	printf("%08x %02x\t%04x\t%04x\t%s\n", (unsigned int)(info - ROMBaseMac), id, hwcfg, rom85, name);
}
...
{
	printf("\nROM Info:\n");
	printf("Checksum    : %08x\n", (unsigned int)ReadMacInt32(ROMBaseMac));
	printf("Version     : %04x\n", ROMVersion);
	printf("Sub Version : %04x\n", (unsigned int)ReadMacInt16(ROMBaseMac + 18));
	printf("Resource Map: %08x\n", (unsigned int)ReadMacInt32(ROMBaseMac + 26));
	printf("Trap Tables : %08x\n\n", (unsigned int)ReadMacInt32(ROMBaseMac + 34));
