...
void EtherInit(void)
{
	...
	net_proc = CreateNewProcTags(
		...
		NP_TaskMsgPort, (ULONG)&proc_port,
		TAG_END	
	);
	if (net_proc == NULL)
		return;
...
static void remove_protocol(NetProtocol *p)
{
	...
	FreeTaskPooled(p, sizeof(NetProtocol));
}
...
static LONG copy_to_buff_ppc(void)
{
	uint32	packet_len;
	uint8	*to, *from;
...
static struct EmulLibEntry copy_to_buff	= { TRAP_LIB, 0, (void (*)())&copy_to_buff_ppc };

/*
 *  Copy data from Mac WDS to outgoing network packet
 */

static LONG copy_from_buff_ppc(void)
{
	uint32	packet_len;
	uint8	*to;
	char	*wds;
...
static struct EmulLibEntry copy_from_buff	= { TRAP_LIB, 0, (void (*)())&copy_from_buff_ppc };
...
static void net_func(void)
{
	...
	struct IOSana2Req write_io, control_io;
	bool opened = false;
	...
	control_io.ios2_Req.io_Message.mn_Node.ln_Type = NT_REPLYMSG;
	control_io.ios2_Req.io_Message.mn_ReplyPort = control_port;
	control_io.ios2_Req.io_Message.mn_Length = sizeof(control_io);
...
//			printf("dev=<%s> unit=%d\n", dev_name, dev_unit);
	...
	write_io.ios2_Req.io_Message.mn_Node.ln_Type = NT_REPLYMSG;
			...
			while ((msg = (NetMessage *)GetMsg(proc_port)))
			{
				...
				switch (msg->what) {
					...
					case MSG_ATTACH_PH: {
						...
						p = (NetProtocol *)AllocTaskPooled(sizeof(NetProtocol));
						if (p == NULL)
						{
							msg->result = lapProtErr;
							goto reply;
