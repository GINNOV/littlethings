...
unsigned short int updcrc(unsigned short int,int);
unsigned short int updcrcr(unsigned short int,int);
void perr(char *);
...
unsigned long num;
FILE *fp;
unsigned short int crc16tab[256] =
{
...
/* update crc */
unsigned short int updcrc(crc,c)
unsigned short int crc; int c;
{
	int tmp;
	tmp=(crc>>8)^c;
	crc=(crc<<8)^crctttab[tmp & 0xff];
	return crc;
}

/* update crc reverse */
unsigned short int updcrcr(crc,c)
unsigned short int crc; int c;
{
	int tmp;
	tmp=crc^c;
