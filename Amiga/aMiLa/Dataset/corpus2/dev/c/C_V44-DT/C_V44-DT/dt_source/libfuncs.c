...
// #include <datatypes/PictureClassExt.h> /* pic-dt V43 - NDK <= 3.1 */
...
#define NUM_METHODS 1

void __saveds __regargs CheckMethods(struct ClassBase *cb, Class *cl, Object *o)
{
 ULONG * methods;
 struct opGet opg;

 if(cb->cb_Methods) return; // already set
 ...
 methods = NULL;
 DoSuperMethodA(cl, o, (Msg)&opg);

 if(methods)
  {
   const ULONG terminator = ~0UL;
   ULONG * m = methods;
   ULONG total = NUM_METHODS+1;

   while((*m++) != terminator) total++;

   cb->cb_Methods = AllocVec(sizeof(ULONG) * total,MEMF_ANY|MEMF_PUBLIC);
   if(cb->cb_Methods)
    {
     ULONG *n = cb->cb_Methods;

     m = methods;
     while((*m) != terminator) (*n++) = (*m++);

     (*n++) = OM_GET; // only for testing: this should be DTM_WRITE instead(!)
     (*n++) = terminator;
    }
  }
}

ULONG __saveds __regargs getAttrMethod(struct ClassBase * cb,Class * cl,Object * o,struct opGet * msg)
{
 switch(msg->opg_AttrID)
  {
   case DTA_Methods:         (*msg->opg_Storage) = (ULONG) cb->cb_Methods; break;
   case PDTA_GetNumPictures: (*msg->opg_Storage) = (ULONG) 1;              break;
   ...
   case OM_NEW:
    {
     if (retval = DoSuperMethodA (cl, o, msg))
      {
       ObtainSemaphore(&cb->cb_DTSemaphore);
       CheckMethods(cb, cl, (Object *) retval);
       ReleaseSemaphore(&cb->cb_DTSemaphore);
       ...
       if(buffer)
        {
         ...
         setdtattrs (cb, o,
                     ...
                     PDTA_BitMap,       bm,
                     PDTA_ModeID,       BestModeID(BIDTAG_NominalWidth,  bmhd->bmh_Width,
...
ULONG __saveds __stdargs DTS_ReadIntoBitMap43(struct ClassBase *cb, Object * o, Class *cl, struct TagItem * attrs)
{
 ...
 if(LibVer(SuperClassBase) > 43)
  {
   ULONG pic_num = (ULONG) GetTagData (PDTA_WhichPicture, 0, attrs); // if we had more than one picture...
   if(pic_num > 0)
    {
     SetIoErr(ERROR_OBJECT_WRONG_TYPE);
     return(FALSE);
 ...
 setdtattrs(cb, o,
            ...
            PDTA_ModeID,      BestModeID(BIDTAG_NominalWidth,  bmhd->bmh_Width,
                                         ...
                                         BIDTAG_Depth,         24,
                                         TAG_END),
            PDTA_AlphaChannel, (bmhd->bmh_Masking == mskHasAlpha) ? TRUE : FALSE,
            TAG_DONE);


 buffer = (APTR) AllocVec(width*4 + 1024, MEMF_CLEAR|MEMF_PUBLIC); /* width*3 for RGB, width*4 for ARGB */
 if(buffer)
  {
   ULONG i;
#ifdef ALPHA_SUPPORT
   ...
   for(i=0; i<height; i++)
    {
     ...
     for(j=0; j<width; j++)
      {
       *bptr++ = 0xFF; // fully opaque

       FRead(fh, bptr, 3, 1); bptr += 3;
      }

     DoSuperMethod(cl, o, PDTM_WRITEPIXELARRAY, buffer, PBPAFMT_ARGB, width*4, 0, i, width, 1);
    }
#else
   for(i=0; i<height; i++)
    {
     FRead(fh, buffer, width*3, 1);
     DoSuperMethod(cl, o, PDTM_WRITEPIXELARRAY, buffer, PBPAFMT_RGB, width*3, 0, i, width, 1);
    }
#endif
