 ...
 * except '#ifdef DEBUG' and 'exit(EXIT_FAILURE)'
 *
 * 1993
...
extern int msg_shown;  /* Added, VANSOFT. */
...
findtok(x) /* Changed - Cannot exit while running under hce!. (VANSOFT). */
{
	while (cur->e_token != EOFTOK && cur->e_token != x)
		fadvnode();
	if (cur->e_token == EOFTOK)
                fatal("Unexpected end of file token!!");
...
program()
{
       ...
       skipon = 0;
more:
    if(chk_ESC()) {     /* User pressed escape gadget?. VANSOFT*/
       Get_FstERR("Esc!!");
       STOP_MSG
       STOP_COMPILE
       }
           	 last = glb_decls();
              if(cur->e_token == EOFTOK)     /* End of file.? */
                 return;
              if(last)                       /* Saw something valid.? */
                 skipon = 0;
...
more:
	...
	if (sawsome) skipon = 0;
	if (cur->e_token == '}') {
		/* if (blktab->b_regs > maxregs) */	/* V2 */
		maxregs |= blktab->b_regs; /* maxregs = blktab->b_regs; */ /* V2 */
