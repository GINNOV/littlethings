...
#define MEM_Y           3    /* note: if changed must change in pre.c. */
#define NODEINCR	100  /* Allocate 100 nodes at a time. */
...
NODE *allocnode()       /* Modified by VANSOFT to monitor memory used,   */
{                       /* and display it in the Message Box. (see mem.c)*/
	char *hcc_calloc();
	NODE *t;
	int i;
...
retry:
	...
	t = (NODE *)hcc_calloc(NODEINCR, sizeof(NODE)); /* hcc_calloc(). VS*/

        mem_mon -= (NODEINCR * sizeof(NODE)); /* Track mem free. VANSOFT. */
       if(!nmerrors)
          (void)Show_Val(((unsigned long)mem_mon / 1024), MEM_X, MEM_Y);

	if (t == 0) {
		fatal("TOTAL MEMORY FAILURE: may crash!!!");
               /*  exit(EXIT_FAILURE); VANSOFT. */
	}
	nodesmade += NODEINCR;
	nodesavail += NODEINCR;
...
NODE *t;
{
	if (t->n_flags & N_ISFREE) { /* Changed VANSOFT. */
		/* printf("%lx",t); */
                fatal("freeing free node. may crash!!");
		/* exit(EXIT_FAILURE); No No No!*/
	} else
		t->n_flags |= N_ISFREE;
	t->n_next = freelist;
	freelist = t;
