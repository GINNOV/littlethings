 ...
 * except '#ifdef DEBUG' and 'size_i' instead of 'SIZE_I'
 *
 * Previous changes by Joe Montgomery labeled JMM
 *
 * 1994
...
extern NODEP symtab[];
extern level;
extern int lineno; /* Added, VANSOFT. */
...
#ifdef	FOR_AMIGA
/* ADDED BY JMM, output an XREF for global defined externals */
/* TETISOFT: changed XREF to '.PUBLIC' */
			break;
	case K_EXTERN:
		        out_gv(xp, 0);
#endif
...
NODEP xp;
{
	...
	case K_AUTO:
	case K_REGISTER:
		if (xp->n_tptr->t_token == '[')	/* V2 */
			return;			/* V2 */
		if (cur->e_token == '=')
			a_init(xp);
		break;

/* ADDED BY TETISOFT, output '.PUBLIC' for local defined externals */
#ifdef	FOR_AMIGA
	case K_EXTERN:
		out_gv(xp, 0);
#endif
...
register NODEP tp;
{
	...
	if (cur->e_token == SCON && tp->t_token == '[' &&

/* CHANGED BY TETISOFT, allow also 'unsigned char text[] = "text"; '  */
/*	     tp->n_tptr->t_token == K_CHAR) {       hack for SCON ary */

            (tp->n_tptr->t_token == K_CHAR ||
	     tp->n_tptr->t_token == T_UCHAR))    /* hack for SCON ary */
           {
			nsee = out_scon(cur);
			fadvnode();
			a_fix(tp, nsee);
	...
	case K_STRUCT:
		...
		if(tp->n_right) {
                   nsee = inits(tp->n_right);
                          ...
                          /* Allow safer initialization of */
                          /* undefined structures. */
                       if(!(tp->n_right = safe_inits()))
                            nsee=0;
                          else nsee=1;
                       }
		break;
...
/* Initialize undefined structure. Added VANSOFT.
 * This is now called to initialize any references made
 * to structures not yet defined. This is because 'inits()' (above)
 * tends to to run off the rails if called with a NULL argument.
 */
NODEP safe_inits()
{
  NODEP np,rv,allocnode();
  int see1;

 /* If here, must be an error. */
       error_v2("undefined: structure",(lineno-1));
	...
	while (cur->e_token != '}' && cur->e_token != ';' &&
               cur->e_token != EOFTOK) 
         {
               if(!(np->n_next=allocnode()))
                  return(rv);
                  np=np->n_next;
               if(!(np->n_tptr=allocnode())) {
                  freenode(np->n_next);
                  return(rv);
                  }
	       if(cur->e_token == ',') {
...
{						/* V2 */
	...
	fldoffs = -1;				/* V2 */
						...
						/* V2 */
fld_init(np, valflg)				/* V2 */
NODEP np;					/* V2 */
{						/* V2 */
	NODEP xp;				/* V2 */
	NODEP assignx();			/* V2 */
	int val, width, woffs;			/* V2 */
	long soffs;				/* V2 */
						/* V2 */
	width = np->e_fldw;			/* V2 */
	woffs = np->e_fldo;			/* V2 */
