...
NODEP np;
{
	...
	case UNARY '-':
		...
		break;
	case TCONV:
		if (np->n_tptr->t_token != K_VOID) {		/* V2 */
			mustty(lp, R_SCALAR);			/* V2 */
			mustty(np, R_SCALAR);
		}						/* V2 */
...
/* CHANGED BY TETISOFT, function expects three parameters */
			/* newfcon(np, -(np->n_left->e_fval)); */
			newfcon(np, -(np->n_left->e_fval), 0);
