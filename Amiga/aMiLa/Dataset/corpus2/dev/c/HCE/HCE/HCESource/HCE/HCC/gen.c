...
/*				addcode(np, "\tseq\tA\n\tand.w\t#1,A\n"); */
		...
		return OKAY;
	default:
		errors("Weird u_eval %s ", np->n_name); /* Changed VANSOFT*/
		return FAIL;
	}
}
...
register NODEP np;
{
	...
	case FORVALUE:					/* V2 */
		if (anycode(np) == 0 && isimmed(np) &&	/* V2 */
		    np->g_token == ICON) {		/* V2 */
			convalue = np->g_offs;		/* V2 */
			return OKAY;			/* V2 */
		}					/* V2 */
		error("illegal CONSTANT expr");		/* V2 */
		return FAIL;				/* V2 */
	case IND0:
		outcode(np);
