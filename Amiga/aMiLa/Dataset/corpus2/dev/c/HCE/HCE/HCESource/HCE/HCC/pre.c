...
extern char curstr[];
#define MEM_X           12   /* X/Y pos in msg box where mem will show,  */
#define MEM_Y           3    /* note: if changed must change in nodes.c. */
...
extern int nmerrors;          /* VANSOFT. */
extern lineno;
extern char *inname;
extern FILE *input;
extern unsigned long mem_mon; /* Monitor memory used. VANSOFT. */
...
/* ADDED BY TETISOFT, ignore the word 'chip' */
/* Modified to also check '__chip'. VANSOFT. */
#ifdef	FOR_AMIGA
   if ((strcmp(rv->n_name,"chip")==0) || (strcmp(rv->n_name,"__chip")==0)) {
       sawchip = 1;
       freenode(rv);
       goto again;
       }

/* Ignore 'far' and '__far' keywords. Added, VANSOFT. */
   if ((strcmp(rv->n_name,"far")==0) || (strcmp(rv->n_name,"__far")==0)) {
       freenode(rv);
       goto again;
       }
...
p_inc() /* Added more error checking, VANSOFT. */
{
	int chkhere;
	FILE *newf, *srch_open();
	char *scopy(), *newnm;
...
/* Check memory and error conditions before new file. VANSOFT. */
     if(!MEM_OK || !COMP_OK) {
        STOP_COMPILE
        return;
        }
	...
	newf = srch_open(curstr, chkhere);
     ...
     if(newf == NULL) {
	fatals("Can`t open - ", curstr);
        STOP_COMPILE          /* Added. VANSOFT. */
        return;
 	}
        mem_mon -= (unsigned long)BUFSIZ; /* Dec mem free. VANSOFT. */

/* Show new Include file and Free memory. VANSOFT. */
     if(!nmerrors) {
        Any_Msg("  Include : ", curstr, 1);
        (void)Show_Val(((unsigned long)mem_mon / 1024), MEM_X, MEM_Y);
        }
...
/* CHANGED BY TETISOFT, needs less memory */
#ifndef FOR_AMIGA
...
newfile(fd,s)  /* This func is called to keep file name/line position etc*/
FILE *fd;      /* before opening a new file. */
char *s;
{
...
#else
setbuf(input, malloc_V2((int)BUFSIZ));  /* (malloc_V2-see mem.c).VANSOFT. */
#endif

#endif
}

endfile() /* This func is called on return from an include file to */
...
/* If no errors, show include file to be compiled. VANSOFT.*/
     if(!nmerrors) {
        if(inclvl)
           Any_Msg("  Include : ", inname, 1);
          else
           Any_Msg("  Include : 0", NULL, 1);
        }
	return 1;
}
...
#else							/* V2 */
char *srchlist[MAXIDIR] = {				/* V2 */
	"",						/* V2 */
	"/home/hans/sozobon/include/",	/* change this to cross incl dir */ /* V2 */
	0						/* V2 */
};							/* V2 */
static int idir_n = 2;					/* V2 */
#endif							/* V2 */
#else
char *srchlist[MAXIDIR] = {
	"",
...
char *s;
{
	...
	if (idir_n >= MAXIDIR-1) {
                Show_ErrV1("Error: To many include dirs!"); /* VANSOFT. */
                nmerrors++;
		return;
	}
