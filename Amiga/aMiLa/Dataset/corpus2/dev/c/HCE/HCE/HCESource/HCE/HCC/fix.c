 ...
 *
 * 1993
 * Modifeid by Jason Petty to work with hce.
...
NODEP np;
{
	...
	case REGVAR:
		if (np->g_rno < AREG)
			...
			return CL_AREG;
	default:
		error("Weird class "); /* Changed VANSOFT. */
		return CL_IMM;
	}
}
...
struct mtbl *tbl;
{
	...
	error("no code table match! "); /* Changed VANSOFT. */
	return FAIL;
}
...
/*		how = 2; */	/* shift too big for immediate */
	how = np->g_fldo;
	if (how > (longflag ? 32 : 16))
		warn("offset too big for int field");
...
/*	addcode(np, "\tand\t#W,A\n"); */	/* mask off top */
	while (how > 0) {
		if (how > 8)
			addcode(np, "\tlsr.S\t#8,A\n");
		else
			addcode(np, "\tlsr.S\t#Z,A\n");
		how = how - 8;
	}

	addcode(np, "\tand.S\t#W,A\n");		/* mask off top */
...
/*					"\tlsr\t#8,A\n\tlsr\t#Z,A\n"); */
	...
	while (how > 0) {
		if (how > 8)
			addcode(np, "\tlsl.S\t#8,A\n");
		else
			addcode(np, "\tlsl.S\t#Z,A\n");
		how = how - 8;
	}
	how = np->g_fldo;
	addcode(np, "\tmove.S\t<A,R1\n");		/* old value */
	addcode(np, "\tand.S\t#X,R1\n");		/* mask out old value */
	addcode(np, "\tor.S\tA,R1\n");		/* set new field */
	addcode(np, "\tmove.S\tR1,<A\n");		/* store new word */
	if (how && (flags & NOVAL_OK) == 0) {	/* return value */
		while (how > 0) {
			if (how > 8)
				addcode(np, "\tlsr.S\t#8,A\n");
...
/*	if (np->g_fldw + np->g_fldo >= 16) */
	if (np->g_fldw + np->g_fldo >= (longflag ? 32 : 16))
