...
/* CHANGED BY TETISOFT (This was a bug!) */
/*		addcode(np, "\tbra\tL2\nL1:\tclr\tA\nL2:\n"); */
		addcode(np, "\tbra\tL2\nL1:\tclr.l\tA\nL2:\n");
...
doop:
	...
	default:
		errors("Weird b_eval %s ",np->n_name); /* Changed VANSOFT*/
		return FAIL;
	}
}
...
doop:
	...
	default:
		errors("Weird as_eval %s ",np->n_name); /* Changed VANSOFT*/
		return FAIL;
	}
}
...
/* ADDED BY TETISOFT, hack ICON if -L option */
	if (longflag && np->g_token == ICON)
		np->g_sz = 4;
...
/* ADDED BY TETISOFT FOR -L option */
	 if (longflag) {
/* V2 */	if (isdreg(np))
/* V2 */	addcode(np, (np->g_ty == ET_U) ?
/* V2 */	   "\tand.l\t#$ff,A\n\tmove.l\tA,-(sp)\n" :
/* V2 */	   "\text.w\tA\n\text.l\tA\n\tmove.l\tA,-(sp)\n" );
/* V2 */	else
/* V2 */	addcode(np, (np->g_ty == ET_U) ?
...
/* at this point, have binary node */
	...
	case ASSIGN DOUBLE '>':
		...
		break;
	case FIELDAS:						/* V2 */
		np->g_type = EV_RL;	/* no PUSHR's on L */	/* V2 */
		np->g_needs = merge(rbin(l,r), 2);		/* V2 */
		flag_saves(np, l, r);				/* V2 */
		return;						/* V2 */
	default:
