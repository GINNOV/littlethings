...
*    device from User programs.
*
* AUTHOR
*    J.T. Steichen
*
* COPYRIGHT
*    ClipFuncs.c (c)1999 by J.T. Steichen, All Rights Reserved.
*
* HISTORY
...
*                        );
*
*   $VER: ClipFuncs.c 2.0 (10-Jul-2005) by J.T. Steichen
*********************************************************************
*
*/
...
//PRIVATE char V[] = "\0$VER: ClipFuncsPPC 2.0 " __DATE__ " by J.T. Steichen\0";

# define __USE_INLINE__

# include <proto/exec.h>
# include <proto/dos.h>
# include <proto/iffparse.h> // -lauto opens all the needed libraries for gcc.

IMPORT struct Library *SysBase;
IMPORT struct Library *DOSBase;
IMPORT struct Library *IFFParseBase; // Needed by gcc & SAS-C
...
PRIVATE struct Library *IFFParseBase = NULL; // Needed by gcc & SAS-C

PRIVATE char V[] = "\0$VER: ClipFuncs 2.0 " __AMIGADATE__ " by J.T. Steichen\0";
...
*
* DESCRIPTION
*    Set the given buffer back to all nils.
*****************************************************************
*
*/
...
// Function name is all the documentation required here.

PRIVATE void closeIFFParseLibrary( void )
{
#  ifndef __amigaos4__
   if (IFFParseBase)
      {
...
PRIVATE int openIFFParseLibrary( void )
{
   int rval = RETURN_OK;
...
*
* SYNOPSIS
*     struct IOClipReq *ior = (struct IOClipReq *) CBOpen( ULONG unit );
*
* DESCRIPTION
...
PUBLIC struct IOClipReq *CBOpen( ULONG unit )
{
   ...
   if (OpenDevice( "clipboard.device", unit, (struct IORequest *) ior, 0L ) != 0)
      ...
      return( (struct IOClipReq *) ior ); // clipboard is open for business!
}
...
PUBLIC int CBOpenIFF( ULONG unit, int RWFlag )
{
   ...
   if (openIFFParseLibrary() != RETURN_OK)
      return( rval );
   ...
   CB_IFF = iff; // Set Global CB_IFF here.
   
   return( rval );
}

/****h* CBWriteFTXT_IFF() [2.0] ***********************************************
*
* NAME
*    CBWriteFTXT_IFF()
*
   ...
   while (1)
      ...
      */
      ...
      if ((cn) && (cn->cn_Type == ID_FTXT) && (cn->cn_ID == ID_CHRS))
         ...
         // DBG( printf( "CHRS chunk contains:\n" ) );
...
PUBLIC char *FillCBData( struct IOClipReq *ior, ULONG size )
{
   ...
   if (!(buf = (char *) AllocVec( length, MEMF_PUBLIC | MEMF_CLEAR )))
      {
      return( NULL );
      }
...
/****h* ClipFuncs/FindCurrentWriteID() **************************
*
* NAME
*    FindCurrentWriteID()
*
...
*
* SYNOPSIS
*    int fileLength = GetFileLength( int fileNumber );
*
* DESCRIPTION
...
PUBLIC int FileToFTXT( int clipunitnum, char *filename )
{
   ...
   // Set up IFF_File for Clipboard I/O...
   if (!(iff->iff_Stream = (ULONG) OpenClipboard( clipunitnum )))
      {
      FreeIFF( iff );
      close( infile );
...
PUBLIC int FTXTToFile( int clipunitnum, char *filename )
{
   ...
   if ((error != 0) && (error != IFFERR_EOF))
      rval = error; // Override RETURN_OK with error value.
...
*
* SYNOPSIS
*    int errnum = PostFTXTClip( int unitnum, char *buffer );
*
* DESCRIPTION
*    Sends an FTXT to the Clipboard & waits for a SatisfyMsg.
*
* RETURN VALUE
*    RETURN_OK if all went well, < 0 if there was an error.
*
* NOTES
*    If this function hangs, the User can use Ctrl-C to break it.
*********************************************************************
*
*/
...
PUBLIC int PostFTXTClip( int unitnum, char *buffer )
{
   ...
   // User can use Ctrl-C to break a hang up in DoIO()...
                     ...
                     )
{
   if (h)
      {
      h->h_Entry = (ULONG (*)( struct Hook *, void *, void * )) hookEntry;
...
*
* DESCRIPTION
*    where the actual (clipHook) work gets done.
********************************************************************
*
*/
