...
char mkImg(UWORD *, UWORD *, ULONG *, LONG, LONG, LONG);
...
struct Image *MakeNewImg(struct Image *img, ULONG *pal)
{
    ...
    newimg = AllocMem(sizeof(struct Image), 0);
    if (newimg)
	{
        dep = img->Depth;
        num = 1<<dep;
        ...
        /* Alloc Planes */
        newimg->ImageData = AllocMem(img->Height*wid*dep/8, MEMF_CHIP | MEMF_CLEAR);
        if (newimg->ImageData)
		{
            /* Fill newimg fields */
            ...
            mkImg(img->ImageData, newimg->ImageData, pal, img->Height*wid/8, img->Depth, dep);
