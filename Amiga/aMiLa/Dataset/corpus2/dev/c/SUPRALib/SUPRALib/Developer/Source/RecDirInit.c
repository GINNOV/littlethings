...
*
*   CHANGES
*       littel changes for use with vbcc
*
**************************************************************************/
...
UBYTE RecDirInit(struct RecDirInfo *rdi)
{
	...
	lock = Lock(rdi->rdi_Path, ACCESS_READ);
	if (!lock) return(RDI_ERR_NONEXIST);
	...
	if (fib)
	{
		if (Examine(lock, fib))
		{
			...
			{
				ln = AllocMem(sizeof(struct LockNode), 0L);
				if (ln)
				{
					/* Everything all right till now */
					/* Now first prepare to copy a path */

					lnPath = AllocMem(strlen(rdi->rdi_Path)+2, 0L);
					if (lnPath)
					{
