...
*
*   CHANGES
*       changes vor vbcc, if ((var=fkt(...))==0) not work correctly
*
************************************************************************/
...
UBYTE RecDirNextTagList(struct RecDirInfo *rdi, struct RecDirFIB *rdf, struct TagItem *tagList)
{
	ULONG data;
	...
	if (rdf == NULL)
	{
	    rdf = AllocMem(sizeof(struct RecDirFIB), MEMF_CLEAR);
	    if (!rdf)
		{
    	    return(RDI_ERR_MEM);
		}
