...
*
*       ULONG  = ObtPens(struct ColorMap *, ULONG *, LONG *,
*                 struct TagItem *);
*
*   FUNCTION
...
*       Original: ULONG *pens
*       Changed : LONG *pens
*       08.03.1999 by Michaela Pruess because the function tests pens == -1
*       and so the type ULONG can't be right (I think). See remark in code.
*
*   SEE ALSO
...
ULONG ObtPens(struct ColorMap *cm, ULONG *paltab, LONG *pens, struct TagItem *tags)
{
	int i,j;
	int num = paltab[0]>>16;
	...
	{
        pens[i] = ObtainBestPenA(cm, paltab[3*i+1], paltab[3*i+2], paltab[3*i+3], tags);

        if (pens[i] == -1)	/* THIS TEST is reason for patch */
		{
