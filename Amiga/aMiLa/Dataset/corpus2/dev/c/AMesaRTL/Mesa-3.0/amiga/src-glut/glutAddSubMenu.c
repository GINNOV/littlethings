...
__asm __saveds void glutAddSubMenu( register __a0 const char *name, register __d0 int menu )
{
	struct GlutMenuEntry *gme;

	glutAddMenuEntry(name,menu);
	gme = stuffGetMenuEntry(glutstuff.curmenu->numentries,glutstuff.curmenu);
	gme->issubmenu = TRUE;
}
