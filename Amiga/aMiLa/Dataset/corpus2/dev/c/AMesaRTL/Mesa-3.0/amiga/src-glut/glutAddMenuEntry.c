...
__asm __saveds void glutAddMenuEntry( register __a0 const char *name, register __d0 int value )
{
	struct GlutMenuEntry *gme;

	gme = calloc(1,sizeof(struct GlutMenuEntry));
	if(gme)
	{
		gme->name = name;
		gme->value = value;
		...
		stuffLinkInMenuEntry(gme,glutstuff.curmenu);
