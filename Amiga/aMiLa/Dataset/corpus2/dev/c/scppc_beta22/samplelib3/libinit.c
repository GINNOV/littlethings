...
int CreatePPCTask(struct MyLibrary *libbase, void *entry)
{
    struct TagItem	MyTags[13];
    ...
    MyTags[4].ti_Data	=	(ULONG) 0;
    MyTags[5].ti_Tag	=	PPCTASKTAG_STACKSIZE;
    MyTags[5].ti_Data	=	0x10000;
    ...
    MyTags[9].ti_Tag	=	PPCTASKTAG_ARG3;
    MyTags[9].ti_Data	=	(ULONG) libbase;
    MyTags[10].ti_Tag	=	PPCTASKTAG_ARG4;
    MyTags[10].ti_Data	=	(ULONG) entry;
    MyTags[11].ti_Tag	=	PPCTASKTAG_ARG2;
    MyTags[11].ti_Data	=	(ULONG) _WBenchMsg;

    MyTags[12].ti_Tag	=	TAG_END;

    CloseLibrary((void *)DOSBase);

    result =  (ULONG) PPCCreateTask(libbase->ml_MyObject,
                                    &MyTags[0]);
...
/* PPC functions that run the PPC autoinitializers */
extern int __far _fpinit(void);
extern int __far _fpterm(void);

int  __saveds __asm __libfpinit(register __a6 struct MyLibrary *libbase)
{
    return CreatePPCTask(libbase, _fpinit);
}

void __saveds __asm __libfpterm(register __a6 struct MyLibrary *libbase)
{
    CreatePPCTask(libbase, _fpterm);
}
