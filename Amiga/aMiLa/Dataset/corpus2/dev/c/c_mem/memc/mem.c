/* 
   Extended Memory functions for C
   ...
   Developed under gcc v3.4.0 on 68k AmigaOS


   Suggested to compile with:-

   gcc -noixemul mem.c -o mem_test 
...
int main () {
   ...
   lng=16777215;
   sresl = upeek24 ((long*)&lng+1);
   printf("%ld\n", sresl);
   sresl = speek24 ((long*)&lng+1);
   printf("%ld\n", sresl);
   ...
   sresl = 9388607;
   memcpy (&buf, &sresl, 4);
   sresl = speek24((long*)&buf[1]);  
   printf("%ld\n", sresl);
   sresl = sfpeek24((long*)&buf[1]);
   memcpy (&buf, &sresl, 4);
   sresl = ufpeek24((long*)&buf[1]);
   printf("%ld\n", sresl);
