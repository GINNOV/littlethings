...
void parse_parser_name();
void copy_action();
void readgram();
void record_rule_line();
void packsymbols();
void output_token_defines();
void output_token_values();
void packgram();
...
static char *ifndef;      /* unique string to define in header file */
...
reader()
{
  ...
  fprintf(ftable, "// This may look like C code, but it is really -*- C++ -*-\n");
  fprintf(ftable, "\n// A Bison++ parser, made from %s \n\n", infile);
  if(fdefines) {
    fprintf(fdefines, "// This may look like C code, but it is really -*- C++ -*-\n");
    fprintf(fdefines, "\n// A Bison++ parser, made from %s \n\n", infile);
    ifndef = (char *) malloc((unsigned) (sizeof(char) * (strlen(infile) + 1)));
    strcpy(ifndef,infile);
    for(c = ifndef; *c != '\0'; c++) {
      if(*c == '.') {
	*c = '_';
...
/* copy the union declaration into fdefines,
...
readgram()
{
  ...
  if (typed == 0)/* JF put out same default YYSTYPE as YACC does */
    {
      if (fdefines)
	fprintf(fdefines, "typedef int %s_YYSTYPE;\n\n",parser_name);
    }
...
void output_class_decl(f)
     FILE *f;
{
  fprintf(fdefines, LTYPESTR, parser_name);
  fprintf(f, "\nclass %s {\nprivate:\n", parser_name);
  if (!pure_parser) {
    fprintf(f,"   int yychar;\n   %s_YYSTYPE yylval;\n   %s_YYLTYPE yylloc;\n   int yynerrs;\n", parser_name, parser_name);
  }
  if (debugflag) {
  ...
  fprintf(f,"protected:\npublic:\n  %s() {};\n  int yyparse();\n  void yyerror(char *);\n",
	  parser_name);
  if (pure_parser) {
    fprintf(f,"  int yylex(%s_YYSTYPE *, %s_YYLTYPE *);\n", parser_name, parser_name);
  } else {
    fprintf(f,"  int yylex();\n");
  }
}
	...
	for (i = ntokens; i < nsyms; i++)
	  {
	    ...
	    if (*tags[i] != '@')
	      fprintf(fdefines, "  static const int\tNT%s;\n",
		      tags[i]);
	      fprintf(ftable, "const int\t%s::NT%s\t= %d;\n",
		      parser_name,
		      tags[i],
		      i);
	  }
...
void output_token_values()
{
  ...
  for (bp = firstsymbol; bp; bp = bp->next)
      ...
      if ('\'' != *tags[bp->value] && bp != errtoken)
	{
	  ...
	  if (!c)
	    {
	      fprintf(ftable, "const int\t%s::%s\t= %d;\n",
		      parser_name,
		      tags[bp->value],
		      (translations ? bp->user_token_number : bp->value));
	      if (semantic_parser) {
		fprintf(ftable, "const int\t%s::T%s\t= %d;\n",
			parser_name,
			tags[bp->value],
			bp->value);
	      }
...
FILE *file;
{
  ...
  for (bp = firstsymbol; bp; bp = bp->next)
      ...
      if ('\'' != *tags[bp->value] && bp != errtoken)
	{
	  ...
	  if (!c)
	    {
              fprintf(file, "  static const int\t%s;\n",
		      tags[bp->value]);
	      fprintf(ftable, "const int\t%s::%s\t= %d;\n",
		      parser_name,
		      tags[bp->value],
		      (translations ? bp->user_token_number : bp->value));
	      if (semantic_parser) {
                fprintf(file, "  static const int\tT%s;\n",
			tags[bp->value]);
		fprintf(ftable, "const int\t%s::T%s\t= %d;\n",
			parser_name,
			tags[bp->value],
