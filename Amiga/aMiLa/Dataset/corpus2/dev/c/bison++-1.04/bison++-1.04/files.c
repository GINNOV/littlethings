...
#define PFILE1  "GNU_BISON:[000000]BISON.SIMPLE"
#define PFILE2  "GNU_BISON:[000000]BISON.HAIRY"
#endif
...
openfiles()
{
  ...
  if (spec_outfile)
    {
	 ...
	 For other output files, remove the ".cc" or ".tab.cc" suffix.  */
      name_base = spec_outfile;
      /* BASE_LENGTH includes ".tab" but not ".cc".  */
      base_length = strlen (name_base);
      if (!strcmp (name_base + base_length - 3, ".cc"))
	base_length -= 3;
      /* SHORT_BASE_LENGTH includes neither ".tab" nor ".cc".  */
  ...
  else
    {
	 ...
	 or use y.tab.cc, etc., if -y was specified.  */
  ...
  parser_name = (char *) malloc ((unsigned) (short_base_length + 1) * sizeof(char));
  {
    char *s, *r;
    for(s = name_base, r = parser_name; *s != '.';) *r++ = *s++ ;
    *r = '\0';
  }
  ...
  guardfile = stringappend(name_base, short_base_length, "_guard.cc");
#else
  attrsfile = stringappend(name_base, short_base_length, ".stype.h");
  guardfile = stringappend(name_base, short_base_length, ".guard.cc");
#endif
}
