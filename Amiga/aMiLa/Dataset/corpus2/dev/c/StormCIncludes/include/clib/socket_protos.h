...
#define CLIB_SOCKET_PROTOS_H
       "$Id: socket_protos.h,v 1.3 2000/12/05 22:22:44 nobody Exp $"
...
LONG send(LONG s, CONST_STRPTR msg, LONG len, LONG flags);
LONG sendto(LONG s, CONST_STRPTR msg, LONG len, LONG flags,
...
/* Arpa/inet functions */
ULONG inet_addr(CONST_STRPTR);
ULONG inet_network(CONST_STRPTR);
...
/* NetDB functions */
struct hostent  *gethostbyname(CONST_STRPTR name);
struct hostent  *gethostbyaddr(CONST_STRPTR addr, LONG len, LONG type);
struct netent   *getnetbyname(CONST_STRPTR name);
struct netent   *getnetbyaddr(LONG net, LONG type);
struct servent  *getservbyname(CONST_STRPTR name, CONST_STRPTR proto);
struct servent  *getservbyport(LONG port, CONST_STRPTR proto);
struct protoent *getprotobyname(CONST_STRPTR name);
struct protoent *getprotobynumber(LONG proto);

/* Syslog functions */
void vsyslog(int pri, const char *fmt, _BSD_VA_LIST_);
