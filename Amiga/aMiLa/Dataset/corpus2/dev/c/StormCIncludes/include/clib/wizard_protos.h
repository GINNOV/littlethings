...
/*
**  $VER: wizard_protos.h 42.100 (24.07.2000)
**  StormWizard Release 2.3
**
...
LONG WZ_EasyRequest(APTR surface,struct Window *win,ULONG id, ...);
...
ULONG WZ_WindowGadgets(APTR surface, ULONG id);
BOOL WZ_HandleIMessage(struct WizardWindowHandle *winhandle, struct IntuiMessage *);
VOID WZ_ControlBubbleHelpA(APTR surface, struct TagItem *tags);
VOID WZ_ControlBubbleHelp(APTR surface, Tag Tag1, ... );
STRPTR WZ_WindowObjectname(APTR surface, ULONG id);
STRPTR WZ_RequestObjectname(APTR surface, ULONG id);
STRPTR WZ_FontObjectname(APTR surface, ULONG id);
STRPTR WZ_LibraryObjectname(APTR surface, ULONG id);
STRPTR WZ_ImageObjectname(APTR surface, ULONG id);

#ifdef __cplusplus
}
#endif

#endif  /* CLIB_WIZARD_PROTOS_H */
