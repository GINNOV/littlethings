...
static char *rcsid = "$Id: text.cc,v 1.9 1999/01/31 20:31:10 olivier Exp $";
...
{
	return doText(gd, matrix, cxform, ShapeDraw, 0);
}

void
Text::getRegion(GraphicDevice *gd, Matrix *m, unsigned char id)
{
	doText(gd, m, 0, ShapeGetRegion, id);
}
...
int
Text::doText(GraphicDevice *gd, Matrix *matrix, Cxform *cxform, ShapeAction action, unsigned char id)
{
	TextRecord	*tr;
	...
	for(tr = textRecords; tr; tr = tr ->next)
	{
		...
		for (g = 0; g < tr->nbGlyphs; g++)
		{
			...
			if (action == ShapeDraw) {
				...
				shape->doShape(gd, &cmat, cxform, action, id);
			}
	...
	if (gd->showMore) {
		...
		x2 = boundary.xmax;
		y2 = boundary.ymax;
		gd->drawLine(tmat.getX(x1,y1),tmat.getY(x1,y1),tmat.getX(x2,y1),tmat.getY(x2,y1),20);
		gd->drawLine(tmat.getX(x2,y1),tmat.getY(x2,y1),tmat.getX(x2,y2),tmat.getY(x2,y2),20);
		gd->drawLine(tmat.getX(x2,y2),tmat.getY(x2,y2),tmat.getX(x1,y2),tmat.getY(x1,y2),20);
		gd->drawLine(tmat.getX(x1,y2),tmat.getY(x1,y2),tmat.getX(x1,y1),tmat.getY(x1,y1),20);
	}
