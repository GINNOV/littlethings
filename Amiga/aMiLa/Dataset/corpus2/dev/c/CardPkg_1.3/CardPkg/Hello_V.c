...
struct NewWindow newwindow =
  {
        20,8,   /* window XY origin relative to TopLeft of screen */
        569,190, /* window width and height */
...
void clear_a_card( int i ) /* Remove a single card. */
  {
    ShowVCard( Window->RPort,
               CARD_NONE,
               Window->BorderLeft + 2 + i % 13 * (CARD_V_WIDTH+5),
               Window->BorderTop  + 2 + i / 13 * (CARD_V_HEIGHT+2) );
  }
...
main()
  {
   ...
   if ( CardRange( &cards[ 0], 52, sizeof(cards[0]), SUIT_SPADES,  1 ) &&   /* Normal cards  */
     ...
     {
       show_cards();
       SetWindowTitles( Window,"Here are the cards!","Testing Vertical Cards");
       Delay( 250 );
