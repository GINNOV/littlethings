...
DEBUG1(3, "process entered: with file [%s]\n", filename);

#ifdef MCH_AMIGA    /* Can't find a way to pre-process files with Manx */

	if ((input = fopen(filename, "r")) == EOF )
	{
		(void) fprintf(stderr, "%s: cannot open file '%s' (%s).\n", pchProg, filename, syserrlist());
	}
	else
		level1(filename);    /* Just process file as is for now */
#else
...
char *argv[];
{
	extern int atoi();
	extern char *index();
	static char sbOpts[] = "aehitvw:x:D:f:F:U:I:"; /* valid options	*/
