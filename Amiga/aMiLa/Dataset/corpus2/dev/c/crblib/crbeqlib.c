...
#define STRSIZE          100
#define MAXDEBUGMESS     256
#define WRKSIZE          2048
#define MAXNUMPRESTORES  256
#define MAXNUMSTORES     256

#define NUMPREPARSETOKENS 50
#define NUMORDTERMS       50
...
#ifndef LOG2 /* == LN(2) */
#define LOG2 ((double)0.693147180560)
#endif
#define LN2 LOG2

#define STORECHAR '£'
  /*temp char in parsing: must not be used in any other way*/
...
#define TANH    61
#define ABS     62
#define FACT_B  64 /* special : factorial operates on register B */
#define LINT    65 /*   lower integer */
#define GINT    66 /* greater integer */
#define RINT    67 /*   round integer */
...
struct EqData * MakeCRBEQ(char *StrEQ,int inFlags);
...
void Eq2Tok       (struct EqData *D,char *S);
void Tok2Eq       (struct EqData *D,char *S);
void ReOrdTok     (struct EqData *D,char *S);
...
if ( (D->Tokens=AllocMem(NUMPREPARSETOKENS*sizeof(char *),MEMF_ANY|MEMF_CLEAR)) == NULL)
  {
  D->ParseError=1;
  strcpy(D->ErrorMess,"Couldn't malloc tokwork!");
  goto DonePreParse;
...
for (i=0;i<NUMPREPARSETOKENS;i++)
  {
  if ( (D->Tokens[i]=AllocMem(100,MEMF_ANY)) == NULL)
    {
    D->ParseError=1;
...
if ( (D->OrdTerms=AllocMem(NUMORDTERMS*sizeof(char *),MEMF_ANY|MEMF_CLEAR)) == NULL)
  {
  D->ParseError=1;
...
for (i=0;i<NUMORDTERMS;i++)
  {
  if ( (D->OrdTerms[i]=AllocMem(100,MEMF_ANY)) == NULL)
    {
    D->ParseError=1;
...
if (D->Tokens)
  ...
  for (i=0;i<NUMPREPARSETOKENS&&D->Tokens[i];i++) 
    FreeMem(D->Tokens[i],100);
  FreeMem(D->Tokens,NUMPREPARSETOKENS*sizeof(char *));
  }
...
if (D->OrdTerms)
  ...
  for (i=0;i<NUMORDTERMS&&D->OrdTerms[i];i++)
    FreeMem(D->OrdTerms[i],100);
  FreeMem(D->OrdTerms,NUMORDTERMS*sizeof(char *));
  }
...
void Eq2Tok (struct EqData *D,char *eq)
{
char numbuf[3];
char *StartP,*EndP;
int plen,eqlen;
...
void Tok2Eq (struct EqData *D,char *add)
{
char numbuf[3];
int i;
...
void ReOrdTok (struct EqData *D,char *in)
{
char *t;
...
for(i=0;i<TN;i++)
  {
  if (strchr(D->OrdTerms[i],STORECHAR)) D->OrdPri[i]=0;
...
if (d->DebugMess)
  ...
  FreeMem(d->DebugMess,MAXDEBUGMESS*sizeof(char *));
  }
...
struct EqData * MakeCRBEQ (char *inStrEq,int inFlags)
{
...
d->StrEqSize          =strlen(inStrEq)/4 +1; d->StrEqSize *= 8;
d->Result             =0;
d->X=d->Y=d->Z        =0;
d->ParseError         =0;
d->ErrorMess          =AllocMem(STRSIZE,MEMF_ANY);
d->DebugMess          =AllocMem(MAXDEBUGMESS*sizeof(char *),MEMF_ANY|MEMF_CLEAR);
d->EQ                 =AllocMem(d->EQSize,MEMF_ANY);
d->PreStored          =AllocMem(d->PreStoredSize,MEMF_ANY);
d->Store              =AllocMem(d->StoreSize,MEMF_ANY);
d->StrEq              =AllocMem(d->StrEqSize,MEMF_ANY);
...
while ((d->WrkPtr-d->WrkBase)<d->SLen && (*d->WrkPtr!=')' || *(d->WrkPtr-1)==STORECHAR))
  {
  if (*d->WrkPtr=='(' && *(d->WrkPtr-1)!=STORECHAR) fp=d->WrkPtr;
  d->WrkPtr++;
  }
...
while ((d->WrkPtr-d->WrkBase)<d->SLen&&*d->WrkPtr!=')')
  {
  if ( *d->WrkPtr == STORECHAR ) d->WrkPtr += 2;
  else
    {
    i = *d->WrkPtr;
    if ( i == '|' || i == '&' || i == '~' || i == '<' || i == '>' )
...
if (lt)
  ...
  d->WrkPtr=lt;
  while(d->WrkPtr>=d->WrkBase&&((*d->WrkPtr!='+'&&*d->WrkPtr!='-'&&*d->WrkPtr!='^'&&*d->WrkPtr!='('&&*d->WrkPtr!='*'&&*d->WrkPtr!='/')||*(d->WrkPtr-1)==STORECHAR) )
    {
    d->WrkPtr--;
    }
...
while ((d->WrkPtr-d->WrkBase)<d->SLen&&*d->WrkPtr!=')')
  {
  if (*d->WrkPtr=='^' && *(d->WrkPtr-1)!=STORECHAR) lt=d->WrkPtr;
  d->WrkPtr++;
  }
...
if (lt)
  ...
  /*back up to first term*/
  d->WrkPtr=lt;
  while(d->WrkPtr>=d->WrkBase&&((*d->WrkPtr!='+'&&*d->WrkPtr!='-'&&*d->WrkPtr!='*'&&*d->WrkPtr!='/'&&*d->WrkPtr!='(')||*(d->WrkPtr-1)==STORECHAR) )
    {
    d->WrkPtr--;
    }
  if (d->WrkPtr!=lt) d->WrkPtr++;
...
while ((d->WrkPtr-d->WrkBase)<d->SLen&&*d->WrkPtr!=')')
  {
  if ((*d->WrkPtr=='*'||*d->WrkPtr=='/') && *(d->WrkPtr-1)!=STORECHAR) lt=d->WrkPtr;
  d->WrkPtr++;
  }
...
if (lt)
  ...
  /*back up to first term*/
  d->WrkPtr=lt;
  while(d->WrkPtr>=d->WrkBase&&((*d->WrkPtr!='+'&&*d->WrkPtr!='-'&&*d->WrkPtr!='^'&&*d->WrkPtr!='(')||*(d->WrkPtr-1)==STORECHAR) )
    {
    d->WrkPtr--;
    }
...
if (i==40)
  {
  strcpy(d->ErrorMess,"Overflow in ValNumeric: number is longer than 40 characters!");
  d->ParseError=1;
  return(0);
  }
...
if ( (abs(N - intN)) < 0.00001 && intN > 0 ) /* close enough to an integer */
  {
  long Out;
...
puts("  copyright (c) 1996 by Charles Bloom");
puts(" ");
puts("---------------------------------------------");
puts("GENERAL NOTES:");
puts(" ");
...
puts("                        string are at most 4 characters");
puts("---------------------------------------------");
...
puts("               e.g. ':S,1,3: X' = 6");
puts(" ");
puts(":D:            return the derivative of the following equation,");
puts("                treated as a function of X");
puts(" ");
