...
// Add object to a window, with size and position of rect and gadetid of id
BOOL AFLed::Create(AFWindow *window, AFRect *rect, ULONG id)
{
    if(m_Global.Added)
        RemoveObject();
...
LONG AFLed::RemoveObject()
{
    ...
    if(m_Global.Image !=NULL) {
        if(m_Global.Added) {
            m_Global.Added=FALSE;
            result=RemoveGadget(m_Global.Window->m_pWindow,(struct Gadget*)m_Global.Image);
            m_Global.Window=NULL;
            return result;
        } else return FALSE;
    } else return FALSE;
...
void AFLed::SetDigits(int pair, int number)
{
    ...
    Create(m_Global.Window, &m_Global.rect, m_Global.id);
}
