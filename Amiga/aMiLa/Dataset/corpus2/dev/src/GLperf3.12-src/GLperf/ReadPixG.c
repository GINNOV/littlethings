...
char* PrintEntry(FILE *fp, int p, int sub, int mi)
{
    char* funcName = (char*)malloc(64);
    sprintf(funcName,"ReadPix%s%s%s", pntSuccinct[p], subSuccinct[sub], miSuccinct[mi]);
...
main()
{
    ...
    for (i=0; i<TOTAL_FUNCS; i++)
        names[i] = "Noop";
    header = fopen("ReadPixX.h", "w");
    fprintf(header, "/*\n * File ReadPixX.h generated from ReadPixG (source file ReadPixG.c)\n */\n\n");
    fp = fopen("ReadPixF.c", "w");
    fprintf(fp, "/*\n * File ReadPixF.c generated from ReadPixG (source file ReadPixG.c)\n */\n\n");
    fprintf(fp, "#include \"ReadPix.h\"\n");
    for (mi=0;mi<2;mi++) {
        for (sub=0;sub<2;sub++) {
            for (p=0;p<=FUNC_PTRS;p++) {
                ...
                names[function.word] = PrintEntry(fp, p, sub, mi);
    ...
    fprintf(header, "\nExecuteFunc ReadPixelsExecuteTable[] = {\n");
    for (i=0; i<TOTAL_FUNCS; i++)
        fprintf(header, "    %s,\n", names[i]);
    fprintf(header, "};\n");
    fclose(header);
