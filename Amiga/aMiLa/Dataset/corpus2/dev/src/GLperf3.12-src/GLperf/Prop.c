...
PropertyPtr new_Property(PropNameListPtr propNameListArg, AttributeListPtr attrListArg)
{
    PropertyPtr this = (PropertyPtr)malloc(sizeof(Property));
    CheckMalloc(this);
    this->propNameList = propNameListArg;
    this->attrList = (AttributeList *)attrListArg;
    return this;
}

void delete_Property(PropertyPtr this)
{
  if (this) {
    if (this->propNameList) delete_PropNameList(this->propNameList);
    if (this->attrList) delete_AttributeList(this->attrList);
    free(this);
  }
}
