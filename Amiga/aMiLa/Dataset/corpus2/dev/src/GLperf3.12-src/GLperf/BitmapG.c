...
char* PrintEntry(FILE *fp, int p, int sub, int rd, int mi, int c, int cd, int vis)
{
    char* funcName = (char*)malloc(64);
    sprintf(funcName,"Bitmap%s%s%s%sC%s%d%d", visualSuccinct[vis], pntSuccinct[p], subSuccinct[sub], miSuccinct[mi], colorSuccinct[c], rd, cd);
                    ...
                    for (c=0;c<2;c++) {
                        for (vis=0;vis<2;vis++) {
                            for (cd=3;cd<=MAX_COLOR_DIM;cd++) {
                                ...
                                names[function.word] = PrintEntry(fp, p, sub, rd, mi, c,
                                    cd, vis);
