...
/* Define calls if using function pointers or not */
  ...
  #ifdef MULTIIMAGE
    #define BITMAP_CALL   (*bitmap)(width, height, 0., 0., 0., 0., *imagePtr++);
  #else
    #define BITMAP_CALL   (*bitmap)(width, height, 0., 0., 0., 0., image);
  #endif
  #define PIXELSTORE_CALL  (*pixelStore)
  #if (RASTERPOS_DIM == 2)
    #define RASTERPOS_CALL   (*rasterPos2fv)(srcPtr); srcPtr += 2;
  #else
    #define RASTERPOS_CALL   (*rasterPos3fv)(srcPtr); srcPtr += 3;
  #endif
#else
  #if (COLOR_DIM == 3)
  ...
  #ifdef MULTIIMAGE
    #define BITMAP_CALL   glBitmap(width, height, 0., 0., 0., 0., *imagePtr++);
  #else
    #define BITMAP_CALL   glBitmap(width, height, 0., 0., 0., 0., image);
  #endif
  #define PIXELSTORE_CALL  glPixelStorei
  #if (RASTERPOS_DIM == 2)
    #define RASTERPOS_CALL   glRasterPos2fv(srcPtr); srcPtr += 2;
  #else
    #define RASTERPOS_CALL   glRasterPos3fv(srcPtr); srcPtr += 3;
  #endif
#endif
...
void FUNCTION (TestPtr thisTest)
{
  ...
  #ifdef SUBIMAGE
    GLint imageWidth = this->image_Bitmap.imageWidth;
  #endif
    int i,j;
  #ifdef MULTIIMAGE
    ...
    #endif
    void (APIENTRY *bitmap)(GLsizei, GLsizei, GLfloat, GLfloat, GLfloat, GLfloat, const GLubyte*) = glBitmap;
    ...
    #endif
    void (*bitmap)(GLsizei, GLsizei, GLfloat, GLfloat, GLfloat, GLfloat, const GLubyte*) = glBitmap;
