...
static void switch_samples(int aVoice)
{
	...
	{
		AHI_SetSound(aVoice, AHI_NOSOUND, 0, 0, gAudioCtrl, 0);
	...
	{
		AHI_SetSound(aVoice, (aVoice + (Voice->v_Playing * gNumVoices)), 0, 0, gAudioCtrl, 0);
	}
}
...
static void thread_init_voice(int aVoice, AL_CONST SAMPLE *aSample)
{
	...
	if (AHI_LoadSound(aVoice, AHIST_DYNAMICSAMPLE, &gVoices[aVoice].v_SampleInfos, gAudioCtrl) == AHIE_OK)
	{
		...
		if (AHI_LoadSound((aVoice + gNumVoices), AHIST_DYNAMICSAMPLE, &gVoices[aVoice].v_SampleInfos2, gAudioCtrl) == AHIE_OK)
		{
...
{
	AHI_UnloadSound(aVoice, gAudioCtrl);
	AHI_UnloadSound((aVoice + gNumVoices), gAudioCtrl);
}
...
static void thread_start_voice(int aVoice)
{
	...
	if (!(gPlaybackStarted))
	{
		...
		if (AHI_ControlAudio(gAudioCtrl, AHIC_Play, TRUE, TAG_DONE) == AHIE_OK)
		{
			gPlaybackStarted = 1;
		}
	}
	...
	if (gPlaybackStarted)
	{
		AHI_SetVol(aVoice, Voice->v_Volume, Voice->v_Panning, gAudioCtrl, AHISF_IMM);
		AHI_SetFreq(aVoice, Voice->v_Frequency, gAudioCtrl, AHISF_IMM);
		...
		AHI_SetSound(aVoice, aVoice, 0, 0, gAudioCtrl, AHISF_IMM);
...
static void thread_stop_voice(int aVoice)
{
	...
	if (Voice->v_Started)
	{
		AHI_SetSound(aVoice, AHI_NOSOUND, 0, 0, gAudioCtrl, AHISF_IMM);
...
static int sound_thread_init(struct AmiThread *aAmiThread)
{
	...
	gMsgPort = CreateMsgPort();
		...
		if ((gIORequest = (struct AHIRequest *) CreateIORequest(gMsgPort, sizeof(struct AHIRequest))) != NULL)
		{
			/* Set the lowest version of AHI that we are expecting to use */

			gIORequest->ahir_Version = 4;

			/* Open ahi.device for use */

			if ((gAHIDevice = OpenDevice((STRPTR)AHINAME, AHI_NO_UNIT, (struct IORequest *) gIORequest, 0)) == 0)
			{
				...
				AHIBase = (struct Library *) gIORequest->ahir_Std.io_Device;
					...
					if (AHI_GetAudioAttrs(AHI_DEFAULT_ID, NULL, AHIDB_MaxChannels, &MaxVoices))
					{
						...
						if ((gVoices = _AL_MALLOC(sizeof(struct Voice) * gNumVoices)) != NULL)
						{
							...
							gSoundFuncHook.h_Entry = (HOOKFUNC)HookEntry;
							gSoundFuncHook.h_SubEntry = (HOOKFUNC) sound_func;
							...
							if ((gAudioCtrl = AHI_AllocAudio(AHIA_Channels, gNumVoices, AHIA_SoundFunc, (ULONG) &gSoundFuncHook,
								...
								TAG_DONE)) != NULL)
							{
								...
								if ((gPlayerSigBit = AllocSignal(-1)) != -1 )
								{
									gPlayerProcess = (struct Process *)SysBase->ThisTask;
...
static void sound_thread_exit()
{
	...
	FreeSignal(gPlayerSigBit);

	/* Free the audio hardware, if it was allocated */

	if (gAudioCtrl)
	{
		AHI_FreeAudio(gAudioCtrl);
		gAudioCtrl = NULL;
	}
	...
	if (gAHIDevice == 0)
	{
		CloseDevice((struct IORequest *) gIORequest);
	}

	/* Free the AHI IO request */

	DeleteIORequest((struct IORequest *) gIORequest);
...
static void sound_thread_func(struct AmiThread *aAmiThread)
{
	...
	for ( ; ; )
	{
		Signal = Wait(ThreadSignal | SoundSignal);
...
{
	AHI_SetFreq(aVoice, aFrequency, gAudioCtrl, AHISF_IMM);
}
