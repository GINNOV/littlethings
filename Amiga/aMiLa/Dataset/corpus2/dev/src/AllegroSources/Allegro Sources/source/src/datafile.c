...
static SAMPLE *read_sample(PACKFILE *f)
{
   ...
   if (s->bits == 8) {
      ...
      if (s->data) {
	 /* 8 bit samples are in unsigned format in .dat files.  See if the platform */
	 /* needs them converted to signed format and do so if required */
	 if (_sound_signed_samples) {
	    for (i = 0; i < len; i++)
	       ((signed char *) s->data)[i] = ((unsigned char *) s->data)[i] ^ 0x80;
	 }
      }
   }
   else {
	 ...
	 for (i=0; i < (int)s->len * ((s->stereo) ? 2 : 1); i++) {
		...
		/* 16 bit samples are in unsigned format in .dat files.  See if the platform */
		/* needs them converted to signed format and do so if required */
		if (_sound_signed_samples) {
		   ((unsigned short *) s->data)[i] = ((signed short *) s->data)[i] ^ 0x8000;
	    }
	 }
