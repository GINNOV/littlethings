...
/* convert_sample_signedness:
 *  Performs a forced conversion of a sample's signedness from its current format
 *  to the opposite format.  This is a helper function for games that use samples
 *  that are in the incorrect format on disk and need to be manually converted.
 */
void convert_sample_signedness(SAMPLE *spl)
{
	int i, len;
	...
	if (spl->bits == 8)
		for (i = 0; i < len; i++)
			((signed char *) spl->data)[i] = ((unsigned char *) spl->data)[i] ^ 0x80;
	else if (spl->bits == 16)
		for (i = 0; i < len; i++)
			((signed short *) spl->data)[i] = ((unsigned short *) spl->data)[i] ^ 0x8000;
}

END_OF_FUNCTION(convert_sample_signedness);
...
static void sound_lock_mem(void)
{
   ...
   LOCK_FUNCTION(convert_sample_signedness);
}
