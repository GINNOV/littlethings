...
/*	    j1 = (j+1) * sx - EPSILON; */
	    ...
	    for (k = 0; k < components; k++) {
	       ...
	       *dst++ = (float)(s1 * (1.0 - alpha) + s2 * alpha);
	    }
	 }
      }
   }
 ...
 */
GLint /*GLAPIENTRY*/
glu_Build1DMipmaps(GLenum target, GLint components,
...
GLint /*GLAPIENTRY*/
glu_Build2DMipmaps(GLenum target, GLint components,
		  ...
		  GLenum type, const void *data)
{
   ...
   if (w != width || h != height) {
	 ...
	 return GLU_OUT_OF_MEMORY;
      ...
      error = glu_ScaleImage(format, width, height, type, data,
			    w, h, type, image);
      if (error) {
	 retval = error;
	 done = GL_TRUE;
   ...
   while (!done) {
	 ...
	 return GLU_OUT_OF_MEMORY;
      ...
      error = glu_ScaleImage(format, w, h, type, image,
