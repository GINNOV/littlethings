...
static GLList *alloc_list( GLContext *c, int list ){
	GLList        *l  = (GLList        *)calloc( 1, sizeof(GLList       ) );
	GLParamBuffer *ob = (GLParamBuffer *)calloc( 1, sizeof(GLParamBuffer) );
...
void gl_compile_op( GLContext *c, GLParam *p ){
	...
	int            index    = c->current_op_buffer_index;
  ...
  if ((index + op_size) > (OP_BUFFER_MAX_SIZE-2)) {
    ...
    ob1 = (GLParamBuffer*)calloc(1,sizeof(GLParamBuffer));
    ob1->next=NULL;
...
void glNewList( unsigned int list, GLenum mode ){
  GLList *l;
  GLContext *c=gl_get_context();
