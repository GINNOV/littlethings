...
#include "PhysicalComponents/VSplit.h"
...
//!  Constructor
VSplit::VSplit( const char *name, Area *target ):Area(name),EventListener(EVENT_MOUSE_KEY_M|EVENT_MOUSE_DRAG){
	this->target = target;
	this->drag   = false;
...
//!  Destructor
/*virtual*/ VSplit::~VSplit(){
	//  FIX
}


//!  Return event target
/*virtual*/ Area *VSplit::getTarget(){
	return this->target;
}


//!  Drawing code
void VSplit::drawSelf(){
	int width;
	int height;
...
/*virtual*/ void VSplit::mouseDrag( const Uint32 button_mask, const int x, const int y ){
