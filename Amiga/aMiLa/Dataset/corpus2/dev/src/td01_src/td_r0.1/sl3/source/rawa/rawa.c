			...
			for(j=1;j<=nofpo;j++) {
				...
				if((nofv=meshNofVerticesGet(mesh))>2) {
					...
					while(n<nofv) {
						...
						sprintf(buffer,"%0.5g %0.5g %0.5g %0.5g %0.5g %0.5g %0.5g %0.5g %0.5g\n",v1.x,v1.y,v1.z,
						v2.x,v2.y,v2.z,v3.x,v3.y,v3.z);
						if(FPuts(rawfile,buffer)!=DOSFALSE) {
							Close(rawfile);
							return(RCWRITEDATA);
						}
					}
...
ULONG __saveds ASM tdo3XLoad(register __d1 ULONG meshhandle,
	...
	while(fscanf(fp,"%f %f %f %f %f %f %f %f %f",&af[0],&af[1],&af[2],&af[3],&af[4],&af[5],&af[6],&af[7],&af[8])!=EOF) {
...
STRPTR __saveds ASM tdo3XName () {
	static STRPTR name="RAW ASCII (TD)";
