/* Peeking memory addresses one long-word at a time. */
...

{
	...
	if (strlen(argv[1]) >= 9) (argv[1]) = "16777212";
	...
	if (peekaddress >= 16777212) peekaddress = 16777212;

	/* Obtain a longword value from the memory address. */
	mylongword = *(unsigned long int *)peekaddress;

	/* Print values to the screen. */
	/* This can be used from the command line as 'print-peekl x<RETURN>' */
	/* where x is from 0 to 16777212. NOTE, for my purposes it was */
	...
	printf("\nThe longword at that address in decimal is %lu.", mylongword);
	printf("\nThe longword at that address in hexadecimal is %lX.\n\n", mylongword);

	/* Use the RETURN CODE for longword value access for Python. */
