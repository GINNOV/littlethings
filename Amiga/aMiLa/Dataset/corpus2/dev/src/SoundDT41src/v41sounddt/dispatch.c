...
ULONG __regargs Sound_GET( Class *cl, Object *o, struct opGet *opg )
{
	...
	switch( opg->opg_AttrID )
	{
		...
		case SDTA_Sample:
			data = (ULONG) ( ( id->LeftSample ) ? NULL : id->Sample );
		...
		case SDTA_SignalBitMask: // aka SDTA_SignalBit
			data = (ULONG) ( id->SignalBit == -1L ) ? 0L : ( 1L << id->SignalBit );
		...
		case SDTA_LeftSample:
			data = (ULONG) ( ( id->LeftSample ) ? id->Sample : NULL );
			dbug( kprintf("SDTA_LeftSample\n"); )
		break;
		
		case SDTA_RightSample:
			data = (ULONG) id->RightSample;
...
{
	struct ClassBase	*cb = (struct ClassBase *)cl->cl_UserData;
	STATIC ULONG		 methodID = ICM_CHECKLOOP;	
