 ...
 * This code shows how to Write and Read clip data via the clipboard.device.
...
zero_args
	...
	lea	ftxt_write(pc),a0
	move.l	a0,io_Data(a2)
	move.l	#40,io_Length(a2)		
	clr.l	io_Offset(a2)		
	clr.l	io_ClipID(a2)		
	move.l	a2,a1
	move.l	4.w,a6
	...
	bne.s	clip_error			; Yes. So exit.

	move.w	#CMD_UPDATE,io_Command(a2)	; Tell the clipboard.device
	jsr	_LVODoIO(a6)			; we have finished writing.

	move.w	#CMD_READ,io_Command(a2)	; The readbuf will contain
	lea	readbuf(pc),a0			; an FTXT data structure.
	move.l	a0,io_Data(a2)
	move.l	#40,io_Length(a2)		
	clr.l	io_Offset(a2)		
	clr.l	io_ClipID(a2)		
	...
	tst.l	io_Actual(a2)			; Have all Reads been done?
	beq.s	cont0				; Yes. So continue.

	move.w	#CMD_READ,io_Command(a2)	; No. So pass an empty read.
	clr.l	io_Data(a2)			; Clearing io_Actual tells
	clr.l	io_Length(a2)			; the clipboard.device you
	clr.l	io_Offset(a2)			; have finished reading.
	clr.l	io_Actual(a2)		
	clr.l	io_ClipID(a2)		
	move.l	a2,a1
	jsr	_LVODoIO(a6)
...
ftxt_write
	...
	dc.l	$46545854	; FTXT
	dc.l	$43485253	; CHRS
	dc.l	20		; Length of "Clipboard Write Test"
	dc.l	$436C6970	; Clip
	dc.l	$626F6172	; boar
	dc.l	$64205772	; d Wr
	dc.l	$69746520	; ite
...
readbuf		dcb.b	100,0
	...
	dc.b	'$VER: Clipboard.s V1.01 (22.4.2001)',0
