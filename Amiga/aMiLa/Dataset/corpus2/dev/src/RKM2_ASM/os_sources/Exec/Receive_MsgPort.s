 ...
 * This code shows how to set-up a Port, that can receive custom Messages
 * sent by another port (SenderPort) and reply to those custom messages.
 * Run this example first and then run Send_MsgPort.s
...
get_message
	...
	jsr	_LVOGetMsg(a6)
	move.l	d0,a1
	move.w	pm_Command(a1),160(a4)
	move.w	pm_Action(a1),162(a4)
	move.b	pm_Status(a1),164(a4)

 * Just for fun, I will ReplyMsg() changing the value of pm_Status.

	move.b	#255,pm_Status(a1)

	jsr	_LVOReplyMsg(a6)

	cmp.w	#PMCOMMAND_SETGAMEPORT,160(a4)
	beq.s	set_gameport

	cmp.w	#PMCOMMAND_USENUMERICPAD,160(a4)
	beq.s	use_numericpad

	cmp.w	#PMCOMMAND_QUIT,160(a4)
	beq.s	flash_quit

	bra.s	get_message

set_gameport
	cmp.w	#PMGAMEPORT_MOUSEINJPORT,162(a4)
	beq.s	set_joystick
	nop
	bra.s	get_message
set_joystick
...
char10		dcb.b	1,10
	...
	dc.b	'$VER: Receive_MsgPort.s V1.01 (22.4.2001)',0
