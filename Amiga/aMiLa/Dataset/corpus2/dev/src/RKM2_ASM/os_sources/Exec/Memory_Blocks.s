 ...
 * This is an example of allocating multiple memory blocks using a memory
 * list with the AllocEntry() function.
	...
	STRUCT	MB_HEAD,ML_SIZE
	STRUCT	MB_ELEMENT0,ME_SIZE
	STRUCT	MB_ELEMENT1,ME_SIZE
	STRUCT	MB_ELEMENT2,ME_SIZE
	STRUCT	MB_ELEMENT3,ME_SIZE
	LABEL	MB_SIZE
...
zero_args
	...
	move.l	d0,a0
	lea	(a0),a1				; lea	MB_HEAD(a0),a1
	move.w	#4,ML_NUMENTRIES(a1)	
	lea	MB_ELEMENT0(a0),a1
	move.l	#MEMF_PUBLIC!MEMF_CLEAR,(a1)	; ME_REQS(a1)	
	move.l	#10,ME_LENGTH(a1)	
	lea	MB_ELEMENT1(a0),a1
	move.l	#MEMF_PUBLIC!MEMF_CLEAR,(a1)	; ME_REQS(a1)	
	move.l	#20,ME_LENGTH(a1)	
	lea	MB_ELEMENT2(a0),a1
	move.l	#MEMF_PUBLIC!MEMF_CLEAR,(a1)	; ME_REQS(a1)	
	move.l	#30,ME_LENGTH(a1)	
	lea	MB_ELEMENT3(a0),a1
	move.l	#MEMF_PUBLIC!MEMF_CLEAR,(a1)	; ME_REQS(a1)	
	move.l	#40,ME_LENGTH(a1)	
	...
	move.l	178(a4),a3		; Address of the ML Structure.
	lea	ML_ME(a3),a0		; Offset Address to the 1st memory
					; (ME Structure) block.
	move.l	(a0),a1			; ME_ADDR(a0) Adr of 1st alloc. mem.
	move.l	ME_LENGTH(a0),d0	; Length of the 1st allocated memory.
	lea	ML_ME+ME_SIZE(a3),a0	; Offset Address to the 2nd memory
					; (ME Structure) block.
	move.l	(a0),a1			; ME_ADDR(a0) Adr of 2nd alloc. mem.
	move.l	ME_LENGTH(a0),d0	; Length of the 2nd allocated memory.
	lea	ML_ME+ME_SIZE*2(a3),a0	; Offset Address to the 3rd memory
					; (ME Structure) block.
	move.l	(a0),a1			; ME_ADDR(a0) Adr of 3rd alloc. mem.
	move.l	ME_LENGTH(a0),d0	; Length of the 3rd allocated memory.
	lea	ML_ME+ME_SIZE*3(a3),a0	; Offset Address to the 4th memory
					; (ME Structure) block.
	move.l	(a0),a1			; ME_ADDR(a0) Adr of 4th alloc. mem.
	move.l	ME_LENGTH(a0),d0	; Length of the 4th allocated memory.

	move.l	#$FFFFFFFF,(a1)		; Poke 4th addresses with data,
	move.l	#$FFFFFFFF,4(a1)	; to show you own it.
	move.l	#$FFFFFFFF,8(a1)
	move.l	#$FFFFFFFF,12(a1)
	move.l	#$FFFFFFFF,16(a1)
...
membuf		dcb.b	200,0
	...
	dc.b	'$VER: Memory_Blocks.s V1.01 (22.4.2001)',0
