 ...
 * This code shows how to set-up a Port and a custom Message. The custom
 * message can be sent to another port (ReceiverPort). Run this example
 * after you have run Receive_MsgPort.s
 ...
 * 148 Task address
 * 152 _GfxBase
 * 156 sender Port (this Port)
 * 160 pm_Command
 * 162 pm_Action
 * 164 pm_Status
 * 165
 * 166 Memory Buffer (12 bytes)
 * 178 sender Message (this Message)
 ...
 * Set-up JW Message structure. Message structures always contain a MN_SIZE
 * structure and then any data you wish to add to the tail of that structure.
 * I.e MN_SIZE + JWDATA_SIZE = pm_SIZEOF. The Message structure is just the
 * same as an Intuition Message but with different data/data_size. Your data
 * can be BPTR, BSTR, LONG, WORD, BYTE etc, just like in an include file.
	...
	move.l	d0,a0
	clr.l	(a0)				; clr.l	LN_SUCC(a0)
	clr.l	LN_PRED(a0)
	move.b	#NT_MESSAGE,LN_TYPE(a0)
	clr.b	LN_PRI(a0)
	clr.l	LN_NAME(a0)
	move.l	156(a4),MN_REPLYPORT(a0)
	move.w	#pm_SIZEOF,MN_LENGTH(a0)

 * The start of my own data.

	move.w	#PMCOMMAND_SETGAMEPORT,pm_Command(a0)
	move.w	#PMGAMEPORT_MOUSEINJPORT,pm_Action(a0)




 * Check if the ReceiverPort exists?
 *
 * Note: Always check that a port exists before sending it any message.

	jsr	_LVOForbid(a6)
	lea	receivername(pc),a1
	jsr	_LVOFindPort(a6)
	tst.l	d0
	beq.s	fr_msg
	move.l	d0,a0			; ReceiverPort.
	move.l	178(a4),a1		; Message.
	jsr	_LVOPutMsg(a6)		; send the Message to ReceiverPort.
	jsr	_LVOPermit(a6)

	move.l	156(a4),a0
	jsr	_LVOWaitPort(a6)	; Wait for ReceiverPort's reply.

	move.l	156(a4),a0
	jsr	_LVOGetMsg(a6)		; Get ReceiverPort Message.
	move.l	d0,a1

	move.b	pm_Status(a1),164(a4)

 * You do not ReplyMsg() as you (this port) initiated the Message.

 * Here you would use the message data just as you would any other
 * Port/Message data (I.e in the same way as an Intuition Message).

 * Clean-up by closing ReceiverPort.
 *
 * Note: Always check that a port exists before sending it any message.
	...
	move.l	d0,a0			; ReceiverPort
	move.l	178(a4),a1		; Message.
	move.w	#PMCOMMAND_QUIT,pm_Command(a1)
	jsr	_LVOPutMsg(a6)		; send the Message to ReceiverPort.
	jsr	_LVOPermit(a6)
...
sendername	dc.b	'SenderPort',0,0
receivername	dc.b	'ReceiverPort',0,0
...
char10		dcb.b	1,10
	...
	dc.b	'$VER: Send_MsgPort.s V1.01 (22.4.2001)',0
