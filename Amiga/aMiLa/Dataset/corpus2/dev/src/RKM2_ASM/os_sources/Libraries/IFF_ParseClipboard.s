 ...
 * This code shows how to Read the Clipboard, using the IFFParse Library.
 * Simply CUT or COPY some text, from a text editor for example, and then
 * run this program. You should see that text appear in the console window
 * if the text editor supports (puts text into) the clipboard.
 ...
 * Open a console window.
	...
	moveq	#PRIMARY_CLIP,d0
	jsr	_LVOOpenClipboard(a6)
	move.l	d0,clip
	beq	fr_iff

	move.l	iffhandle(pc),a0
	move.l	clip(pc),(a0)			; iff_Stream(a0)
	moveq	#IFFF_READ,d0
	jsr	_LVOOpenIFF(a6)
	tst.l	d0
	bne	cl_clip
...
cname		dc.b	'CON:0/0/640/160/ Iff_ParseClipboard.s',0
...
clipbuf		dcb.b	240,0
	...
	dc.b	'$VER: IFF_ParseClipboard.s V1.01 (22.4.2001)',0
