...
zero_args
	...
	move.w	#624,gi_rightmost(a2)
	move.w	#190,gi_bottommost(a2)
	jsr	_LVOInitGels(a6)

	lea	vs0ptr(pc),a0
	lea	bob0ptr(pc),a2
	move.w	#VSF_SAVEBACK,vs_VSFlags(a0)
	move.w	#20,vs_Y(a0)
	move.w	#20,vs_X(a0)
	move.w	#8,vs_Height(a0)
	move.w	#2,vs_Width(a0)
	move.w	#4,vs_Depth(a0)
	move.w	#$0400,vs_MeMask(a0)
	move.w	#$8000,vs_HitMask(a0)
	lea	image0,a1
	...
	move.w	#2,vs_Width(a0)
	move.w	#4,vs_Depth(a0)
	move.w	#$8000,vs_MeMask(a0)
	move.w	#$0400,vs_HitMask(a0)
	lea	image1,a1
	move.l	a1,vs_ImageData(a0)
	lea	bl1ptr(pc),a1
	move.l	a1,vs_BorderLine(a0)
	...
	move.w	#2,vs_Width(a0)
	move.w	#4,vs_Depth(a0)
	move.w	#$0010,vs_MeMask(a0)
	move.w	#$8400,vs_HitMask(a0)
	lea	image2,a1
	move.l	a1,vs_ImageData(a0)
	lea	bl2ptr(pc),a1
	move.l	a1,vs_BorderLine(a0)
 ...
 * Loop0 shows bob 1 colliding with bob 0. When bob 1 hits bob 0 the
 * collision routine is: HitMask $0400 (bob 1 - bit 10) ANDed with MeMask
 * $0400 (bob 0 - bit 10). So bit 10 is set - Collision routine 10 called.

	move.w	#100,d5

loop0	lea	vs1ptr(pc),a0
	move.w	d5,vs_X(a0)
	bsr	draw_bob
	subq.w	#1,d5
	cmp.w	#16,d5
 ...
 * Reset bob 1 to its original position.
 ...
 * Loop1 shows bob 0 colliding with bob 1. When bob 0 hits bob 1 the
 * collision routine is: HitMask $8000 (bob 0 - bit 15) ANDed with MeMask
 * $8000 (bob 1 - bit 15). So bit 10 is set - Collision routine 10 called.
 ...
 * Reset bob 0 to its original position.
 ...
 * Loop2 shows bob 2 colliding first with bob 1 and then with bob 0.
 * So collision 10 and then collision 15 is called.

	move.w	#200,d5

loop2	lea	vs2ptr(pc),a0
	move.w	d5,vs_X(a0)
	bsr	draw_bob
	subq.w	#1,d5
	cmp.w	#16,d5
 ...
 * Reset bob 2 to its original position.
...
bmstg0		dc.b	'Bobs Over Text',0,0
...
image2	dc.w	$FFFF,$FFFF
	...
	dc.b	'$VER: Bob_BobCollisions.s V1.01 (22.4.2001)',0
