 ...
 * LockPubScreen() replaces the old 1.3 (V34) GetScreenData() function.
 * GetScreenData() should Not be used in OS2+ as it gives false info
 * (on purpose).
	...
	move.l	d0,156(a4)		   ; valid only while screen is open.
	beq	unlock_screen
 ...
 * Get the DisplayInfoRecord handle for the ModeID. Note: OpenMonitor() does
 * not need to be used (Its just the way I wrote this code).
 ...
 * I have not displayed the results of the following displayinfos. This is
 * because, at this time, I feel you should use/check the results as opposed
 * to displaying them. I just used MonAm to check this code was correct.
	...
	tst.l	d0				; number of bytes transfered.
	beq.s	close_monitor
...
membuf		dcb.b	300,0
disp		dcb.b	dis_SIZEOF,0
dims		dcb.b	dim_SIZEOF,0
mon		dcb.b	mtr_SIZEOF,0
name		dcb.b	nif_SIZEOF,0


	SECTION	VERSION,DATA

	dc.b	'$VER: MonitorID_Info.s V1.01 (22.4.2001)',0
