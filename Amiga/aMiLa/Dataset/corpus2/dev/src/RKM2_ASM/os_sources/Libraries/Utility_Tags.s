 ...
 * This file demonstrates the tag functions that you would normally use.
 * Some tag functions are not demonstrated here because I feel they are
 * aimed more at translating Include bits, etc. For example. PackBoolTags()
 * just converts tag.data into bool. E.g: GACT_FOLLOWMOUSE ($0008) is
 * translated into a TRUE or FALSE bit.
 ...
 * Allocate 10 tags.
	...
	move.l	#WA_Left,8(a0)
	clr.l	12(a0)
	move.l	#WA_Width,16(a0)
	move.l	#640,20(a0)
	move.l	#WA_Height,24(a0)
	move.l	#200,28(a0)
	move.l	#TAG_DONE,32(a0)

 * Clone the window tags (wndwtags).

	lea	wndwtags(pc),a0
	jsr	_LVOCloneTagItems(a6)
	move.l	d0,clonedtags
	beq.s	fr_tags

 * Try and find the WA_Width tag.
	...
	move.l	#1280,ti_Data(a0)	; Change the tag's ti_Data.

 * Try and find the 1280 tag.data

	move.l	#1280,d0
	move.l	#640,d1			; default data to use if 1280 is
					; not found.
	lea	wndwtags(pc),a0
	jsr	_LVOGetTagData(a6)	; returns tag.data value or 640.

 * Get the next tag item.

	move.l	newtags(pc),a0
	jsr	_LVONextTagItem(a6)
	tst.l	d0
	beq.s	fr_tags
	move.l	d0,a0
	move.l	(a0),d3			; store the next tag's ti_Tag in d3.
	move.l	ti_Data(a0),d4		; store the next tag's ti_Data in d4.

	jsr	_LVONextTagItem(a6)	; a0 already entered above.
	tst.l	d0
	beq.s	fr_tags
	move.l	d0,a0
	move.l	(a0),d3			; store the next tag's ti_Tag in d3.
...
membuf		dcb.b	300,0
	...
	dc.b	'$VER: Utility_Tags.s V1.01 (22.4.2001)',0
