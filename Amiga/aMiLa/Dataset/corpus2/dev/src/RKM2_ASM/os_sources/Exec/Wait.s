 ...
 * This code shows how to Wait for a specific Signal.
...
zero_args
	...
	move.b	d0,d5                ; Signal Bit (a #bit between 0 and 31)
	cmp.l	#-1,d0
	beq.s	cl_icon

	moveq	#0,d6
	bset	d5,d6                ; Set the `signal bit' of d6. So. If
                                     ; the `signal bit' (inside d5) is 29
                                     ; d6 will have its 29th bit set. d6 is
                                     ; then known as the `signal Mask'.
...
waiting	move.l	d6,d0
	jsr	_LVOWait(a6)	      ; Wait for another task to signal us.
...
membuf		dcb.b	200,0
	...
	dc.b	'$VER: Wait.s V1.01 (22.4.2001)',0
