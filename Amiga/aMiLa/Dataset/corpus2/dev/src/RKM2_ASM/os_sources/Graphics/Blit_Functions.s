 ...
 * Minterms.
 *
 * $C0 = Normal (Vanilla) copy.
 * $30 = Invert the source before copying.
 * $50 = Ignore the source and invert the destination.
 * $60 = Put B where C is not and vice versa (Cookie Cut).
 * $80 = Only put bits in destination where there is a bit in the same
 *       position for both source and destination (Sieve Operation).
	...
	move.w	#100,d4
	move.w	#100,d5
	move.b	#$C0,d6			; Minterm = True Copy
	move.b	#$03,d7			; Mask    = Planes
	suba.l	a2,a2			; TempA   = <>0 memory is allocated
					;           for a blit function call.
	move.l	152(a4),a6
 ...
 * Can't say too much about this except that it suppose to scale your
 * source rectangle into a scaled destination rectangle. The RKM are
 * absolutely crap when it comes to information. I did not even see an
 * example of this function.

	lea	bmargs(pc),a0
	clr.w	(a0)				; bsa_SrcX(a0)
	clr.w	bsa_SrcY(a0)
	move.w	#10,bsa_SrcWidth(a0)
	move.w	#10,bsa_SrcHeight(a0)
	move.w	#2,bsa_XSrcFactor(a0)
	move.w	#2,bsa_YSrcFactor(a0)
	move.w	#20,bsa_DestX(a0)
	move.w	#160,bsa_DestY(a0)
	move.w	#100,bsa_DestWidth(a0)
	move.w	#100,bsa_DestHeight(a0)
	move.w	#10,bsa_XDestFactor(a0)
	move.w	#10,bsa_YDestFactor(a0)
	move.l	212(a4),bsa_SrcBitMap(a0)
	move.l	216(a4),bsa_DestBitMap(a0)
	clr.l	bsa_Flags(a0)
	clr.w	bsa_XDDA(a0)
	clr.w	bsa_YDDA(a0)
...
membuf		dcb.b	300,0
bmargs		dcb.b	bsa_SIZEOF,0


	SECTION	VERSION,DATA

	dc.b	'$VER: Blit_Functions.s V1.01 (22.4.2001)',0
