 ...
 * An example of ExAll(). Note: In this example I have used V39 libraries
 * and #ED_OWNER (see the dos/exall.i include). Also note: Some filesystems
 * (Disk Drives for example) do not support Exall() all that well. Hence
 * why this example uses the common hard drive partition - WORK:
 ...
 * Allocate an ExAll structure.

	move.l	#EXALL_BUFSIZE,d0
 ...
 * Print WORK:'s filename in the console window.
 ...
 * Allocate an ExAll Control structure.
 * Note: This must be done via AllocDosObject() and NOT by AllocMem().
 ...
 * Call ExAll().
	...
	move.l	160(a4),d1
	move.l	156(a4),d2
	move.l	#EXALL_BUFSIZE,d3
	moveq	#ED_OWNER,d4
	move.l	188(a4),a0
	clr.l	eac_LastKey(a0)
	clr.l	eac_MatchString(a0)
	clr.l	eac_MatchFunc(a0)
	move.l	a0,d5
	jsr	_LVOExAll(a6)
	tst.l	d0
	beq.s	exall_e
 ...
 * Check how many Entries there are.
	...
	move.l	188(a4),a0
	move.l	(a0),d0				; move.l  eac_Entries(a0),d0
	tst.l	d0
	ble.s	fr_obj				; There are no Entries.

 * Get the details from the ExAll structure.
	...
	tst.l	d6
	ble.s	fr_obj
loop	move.l	(a3),a3				; move.l  ed_Next(a3),a3
	bsr	show_ea
	dbra	d6,loop

 * Call ExAll again.

loop1	move.l	160(a4),d1
	move.l	156(a4),d2
	move.l	#EXALL_BUFSIZE,d3
	moveq	#ED_OWNER,d4
	move.l	188(a4),a0
 ...
 * Check how many Entries there are.
	...
	move.l	(a0),d0				; move.l  eac_Entries(a0),d0
	tst.l	d0
	ble.s	fr_obj				; There are no more Entries.
 ...
 * Get the details from the ExAll structure.
	...
	tst.l	d6
	ble.s	fr_obj
loop2	move.l	(a3),a3				; move.l  ed_Next(a3),a3
	bsr	show_ea
	dbra	d6,loop2
...
fr_obj	moveq	#DOS_EXALLCONTROL,d1
	move.l	188(a4),d2
	move.l	4(a4),a6
...
show_ea	move.l	ed_Name(a3),a0
	bsr	findlen
	bsr	txt
	bsr	eol
	move.l	ed_Type(a3),d1
	bsr	convert_number
	bsr	number
	bsr	space
	move.l	ed_Size(a3),d1
	bsr	convert_number
	bsr.s	number
	bsr	space
	move.l	ed_Prot(a3),d1
	bsr	convert_number
	bsr.s	number
	bsr.s	space
	move.l	ed_Days(a3),d1
	bsr	convert_number
	bsr.s	number
	bsr.s	space
	move.l	ed_Mins(a3),d1
	bsr	convert_number
	bsr.s	number
	bsr.s	space
	move.l	ed_Ticks(a3),d1
	bsr	convert_number
	bsr.s	number
	bsr.s	eol
	move.l	ed_Comment(a3),a0
	bsr.s	findlen
	bsr.s	txt
	bsr.s	eol
	move.w	ed_OwnerUID(a3),d1
	bsr.s	convert_number
	bsr.s	space
	bsr.s	eol
	move.w	ed_OwnerGID(a3),d1
	bsr.s	convert_number
	bsr.s	number
	bsr.s	eol
	bsr.s	eol
...
cname		dc.b	'CON:0/0/400/160/ ExAll.s',0,0
drive		dc.b	'WORK:',0
...
char32		dcb.b	1,32
	...
	dc.b	'$VER: ExAll.s V1.01 (22.4.2001)',0
