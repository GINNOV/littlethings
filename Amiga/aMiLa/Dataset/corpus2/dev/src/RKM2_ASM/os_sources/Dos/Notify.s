 ...
 * An example of DOS Notify. If you create a new serial.prefs file or
 * delete, use or save an existing serial.prefs file this program will
 * notify you via a Signal. If you press Cancel in the serial prefs program
 * you do not get notified.
 *
 * Notes: This program can be used on other files apart from prefs files,
 *        but the notification is no good (only if you delete the file).
 *        If you change the rwed flags of a file no notification is sent.
 *        Obviously I have not tried every kind of file, thus, experiment.
 *
 * The best use for this kind of program, in my opinion, is when you are
 * looking at a (MODE_READWRITE) file and want to know if anyone is tampering
 * with that file whilst you are tampering with it!
 ...
 * Open a console window.
	...
	move.l	156(a4),a0
	lea	ser_prefs(pc),a1
	move.l	a1,(a0)				; move.l  a1,nr_Name(a0)
	move.l	#NRF_SEND_SIGNAL,nr_Flags(a0)
	move.l	148(a4),nr_Task(a0)
	move.b	d5,nr_SignalNum(a0)
	move.l	a0,d1
	move.l	4(a4),a6
...
cname		dc.b	'CON:0/0/320/160/ Notify.s',0,0
ser_prefs	dc.b	'ENV:Sys/serial.prefs',0,0
...
char10		dcb.b	1,10
	...
	dc.b	'$VER: Notify.s V1.01 (22.4.2001)',0
