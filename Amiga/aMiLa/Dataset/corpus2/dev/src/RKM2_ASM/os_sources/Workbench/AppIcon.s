 ...
 * This program requires you to put the AppIcon icon into Ram: and then
 * execute this program. Then drop upto two FILEs (not DRAWERs) onto the
 * newly created (TestAppIcon) workbench drop-icon.
 *
 * Note: You must grab the filenames before ReplyMsg() as ReplyMsg() trashes
 *       the message. Thus, am_ArgList is no longer valid.
 *
 * Note: The window is only for displaying the results. The program actually
 *       WaitPort()s on the created port and not on the window's port. Thus,
 *       you do not need a window for the actual AppIcon.
 ...
 * path_name is the name of an icon. That icon's imagery is used only. Thus,
 * path_name could be a pointer to the name of an icon in ram: (like this
 * example) or to a wb disk icon, etc. I used ram:AppIcon as it uses custom
 * imagery (i.e. create the image you want as an icon). You could of course
 * create and save an icon from within this program (see Icon.s).
...
msg_loop
	...
	move.l	am_NumArgs(a3),d5
	lea	am_ArgList(a3),a5
	moveq	#16,d0
	moveq	#40,d1
	move.l	204(a4),a1
	move.l	152(a4),a6
	...
	jsr	_LVOMove(a6)
	move.l	(a5),a0
	move.l	12(a0),a0		; the next wa_Name
	moveq	#8,d0
	move.l	204(a4),a1
	jsr	_LVOText(a6)

no_more	move.l	a3,a1
	move.l	4.w,a6
	jsr	_LVOReplyMsg(a6)
...
fr_app	move.l	196(a4),a0
	move.l	184(a4),a6
	jsr	_LVORemoveAppIcon(a6)

fr_port	move.l	192(a4),a0
	move.l	4.w,a6
	jsr	_LVODeleteMsgPort(a6)

fr_obj	move.l	188(a4),a0
	move.l	12(a4),a6
	jsr	_LVOFreeDiskObject(a6)
...
wndw_title	dc.b	'AppIcon.s',0
font_name	dc.b	'topaz.font',0,0
path_name	dc.b	'Ram:AppIcon',0
app_name	dc.b	'TestAppIcon',0
...
membuf		dcb.b	300,0
	...
	dc.b	'$VER: AppIcon.s V1.01 (22.4.2001)',0
