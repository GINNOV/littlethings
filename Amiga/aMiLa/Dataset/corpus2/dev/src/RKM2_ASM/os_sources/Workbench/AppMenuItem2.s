 ...
 * This program requires you to execute this program, then drop upto two
 * FILEs (not DRAWERs) into the window. You will see the filename/s in the
 * window.
 *
 * Note: You must grab the filenames before ReplyMsg() as ReplyMsg() trashes
 *       the message. Thus, am_ArgList is no longer valid.
 *
 * This example shows how to Wait() for two message port signals.
...
wndw	move.l	200(a4),a0
	move.l	wd_UserPort(a0),a0
	jsr	_LVOGetMsg(a6)
	move.l	d0,a3
	move.l	im_Class(a3),d2
	move.l	a3,a1
	jsr	_LVOReplyMsg(a6)
...
membuf		dcb.b	300,0
	...
	dc.b	'$VER: AppWindow.s V1.01 (22.4.2001)',0
