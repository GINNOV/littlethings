...
/*
 * This example just takes what was created
 * in the previous examples and drawns pixels,
 * lines, and rectangles into different windows.
 *  I just had to change loopy() not to have any
 * Wait() function but rather just to poll the
 * screen's port.
 *
 * Note there seems to be an error in the library
 * as this program leads to a guru meditation if
 * I leave it for long enough, but I don't know
 * the reason.  Maybe sometime, I'll sit down and
 * check, but since I don't know what combination
 * of primitive sizes causes the problem, I'll
 * leave it as is.
 */
...
unsigned long rand();
#define rnd() (rand()&0xffff)

unsigned long width,height;
union G_Point p1,p2;

union G_Point rand_pixel( USHORT width, USHORT height )
{
  union G_Point xy;

  xy.Coor.X = width*rnd()>>16;
  xy.Coor.Y = height*rnd()>>16;
  gc.FgPen  = rand()&255;

  return(xy);
}

void rand_line( USHORT width, USHORT height )
{
  p1           = rand_pixel(width,height);
  p2           = rand_pixel(width,height);
  gc.LineWidth = 15*rnd()>>16;
  gc.FgPen     = rand()<<8;

  return;
}

void draw_pixel( G_Layer layer )
{
  union G_Point size,p;

  if(size.XY=G_GetLayerSize(layer))
    {
      p=rand_pixel(size.Coor.X,size.Coor.Y);
      G_Pixel(layer,&gc,p.Coor.X,p.Coor.Y);
    }
  return;
}

void draw_line( G_Layer layer )
{
  union G_Point size;

  if(size.XY=G_GetLayerSize(layer))
    {
      rand_line(size.Coor.X,size.Coor.Y);
      G_Line(layer,&gc,p1.Coor.X,p1.Coor.Y,
                       p2.Coor.X,p2.Coor.Y);
    }
  return;
}
...
void draw_rectangle( G_Layer layer )
{
  union  G_Point     size;
  struct G_Rectangle rect;

  if(size.XY = G_GetLayerSize(layer))
    {
      p1 = rand_pixel(size.Coor.X,size.Coor.Y);
      p2 = rand_pixel(size.Coor.X,size.Coor.Y);
      rect.Min.Coor.X = lesser(p1.Coor.X,p2.Coor.X);
      rect.Min.Coor.Y = lesser(p1.Coor.Y,p2.Coor.Y);
      rect.Max.Coor.X = greater(p1.Coor.X,p2.Coor.X);
      rect.Max.Coor.Y = greater(p1.Coor.Y,p2.Coor.Y);

      gc.LineWidth = 15*rnd()>>16;
      gc.Round     = 30*rnd()>>16;
      gc.FgPen     = rand()<<8;

      G_Rectangle(layer,&gc,rect.Min.Coor.X,rect.Min.Coor.Y,
		  G_RectWidth(rect),G_RectHeight(rect));
    }
  return;
}
  ...
  if(root=(G_Layer)G_OpenRootLayer(screen->Map,NULL))
      ...
      if(openwindow(&win1,screen->Map->Width/5,screen->Map->Height/5))
	{
	  if(openwindow(&win2,screen->Map->Width/3,screen->Map->Height/4))
	    {
	      if(openwindow(&win3,screen->Map->Width/4,screen->Map->Height/2))
	    return TRUE;
	      closewindow(&win2);
	    }
...
void loopy( void )
{
  ...
  do
      ...
      while(msg=(E_EGSMsgPtr)GetMsg(screen->Port))
	{
      ...
      draw_pixel(win1.v);
      draw_line(win2.v);
      draw_rectangle(win3.v);

    }while(!(task->tc_SigRecvd&SIGBREAKF_CTRL_C));
  task->tc_SigRecvd &= ~SIGBREAKF_CTRL_C;
  return;
}
