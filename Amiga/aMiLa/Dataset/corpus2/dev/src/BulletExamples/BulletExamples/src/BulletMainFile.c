...
static UBYTE *readargsstring = "FileName,FontName,Size/N,Leading/N,Descender/N,XDPI/N,YDPI/N,N=NoKern/S,S=SpcKern/S,T=TabFigs/S,R=Tracking/K,U=UCS/K,V=VPModeID/K,B=BitDepth/K,C=Colors/K\n";
...
#define NUM_ARGS    15 /* Arguments for ReadArgs(). */
#define VIEW_FILE   0
#define FONT_NAME   1
#define SIZE        2
#define LEADING     3
...
int main(void)
{
  ...
  char           *deffile = "Text/SampleText";  /* Default values for ReadArgs() variables. */
...
/* default X and Y DPI values.  This bug was also present in BulletMain.c.          */
        ...
        if (! args[VIEW_FILE])
        {
          ...
          {
            if (NameFromLock(lock, path, BUFSIZE))
            {
              if (*(path + strlen(path) - 1) != ':')
                strcat(path, "/");
              strcat(path, deffile);
              args[VIEW_FILE] = (LONG) path;
            }
          }
        }
        if (! args[SIZE])
