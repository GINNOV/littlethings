...
#ifdef PARSE_DEBUG
# define DBG( f, v )     fprintf( stderr, f, v )
#else
# define DBG( f, v )
#endif
...
void yyerror( char *s )
{
   UserInfo( s, "CHECK YOUR EXPRESSION:" );
...
PUBLIC int Calculate( char *expr )
{
   ...
   int   rval  = 0;
   FILE *tfile = fopen( TTTempFile, "w" );
   
   if (tfile == NULL)
      {
      sprintf( ErrMsg, "Could NOT open %s for writing!", TTTempFile );
      ...
      if ((tfile = fopen( TTTempFile, "r" )) == NULL)
         {
         sprintf( ErrMsg, "Could NOT open %s for reading!", TTTempFile );
