...
main(void)
{
  ...
  if (RTGBase = OpenLibrary("rtg.library",42)) {
    if(sc=OpenScreenTags(NULL,SA_LikeWorkbench,TRUE,SA_Depth,DEPTH,SA_Title,
      ...
      if (dbm = rtgAllocBitMapTags(NULL,32,8,
				   ABMA_RGBFormat,RGBFB_R5G6B5PC,
	...
	if(wd=OpenWindowTags(NULL,WA_Backdrop,TRUE,WA_Borderless,TRUE,WA_IDCMP,
	  ...
	  for(y=0; y<16; y++){
	    BltBitMap(dbm,  4, 0, rp->BitMap, 16+0*32+8, 24+13*y, 8, 8, (y<<4), MASK, NULL);
	  }
	  
	  for(y=0; y<16; y++){
	    BltBitMap(dbm,  0, 0, rp->BitMap, 16+1*32+8, 24+13*y, 16, 8, (y<<4), MASK, NULL);
	  }
	  
	  for(y=0; y<16; y++){
	    BltBitMap(dbm,  0, 0, rp->BitMap, 16+2*32+8, 24+13*y, 32, 8, (y<<4), MASK, NULL);
	  }
