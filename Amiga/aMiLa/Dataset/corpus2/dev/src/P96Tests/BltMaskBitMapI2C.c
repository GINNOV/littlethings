...
main(void)
{
	...
	if(sc=OpenScreenTags(NULL,SA_Depth,DEPTH,SA_LikeWorkbench,TRUE,SA_Title,"BltBitMap Test (Planar to Chunky)",SA_FullPalette,TRUE,TAG_DONE))
	{
		struct Window		*wd;
		...
		if(wd=OpenWindowTags(NULL,WA_Backdrop,TRUE,WA_Borderless,TRUE,WA_IDCMP,IDCMP_MOUSEBUTTONS,WA_CustomScreen,sc,TAG_DONE))
		{
			...
			struct Message		*msg;
			struct BitMap  *ibm = AllocBitMap(bm.BytesPerRow * 8,bm.Rows,bm.Depth,BMF_INTERLEAVED,NULL);
			if (ibm) {
			  int	y;

			  BltBitMap(&bm       ,  0, 0, ibm       , 0          ,  0  , bm.BytesPerRow * 8, bm.Rows,
				    0xc0,0xff,NULL);
			  ...
			  for(y=0; y<16; y++){
			    BltMaskBitMapRastPort(ibm, 4, 0, rp, 16+0*32+8, 24+13*y, 8, 8, (y<<4), Mask);
			  }
			  
			  for(y=0; y<16; y++){
			    BltMaskBitMapRastPort(ibm, 0, 0, rp, 16+1*32+8, 24+13*y, 16, 8, (y<<4), Mask);
			  }
			  
			  for(y=0; y<16; y++){
			    BltMaskBitMapRastPort(ibm, 0, 0, rp, 16+2*32+8, 24+13*y, 32, 8, (y<<4), Mask);
			  }
