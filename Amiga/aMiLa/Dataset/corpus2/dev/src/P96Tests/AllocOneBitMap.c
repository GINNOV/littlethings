...
main(void)
{
	...
	if(sc=OpenScreenTags(NULL,SA_Depth,DEPTH,SA_LikeWorkbench,TRUE,SA_Title,"BltBitMap Test (Planar to Planar)",SA_FullPalette,TRUE,TAG_DONE))
	{
		...
		if(wd=OpenWindowTags(NULL,WA_Backdrop,TRUE,WA_Borderless,TRUE,WA_IDCMP,IDCMP_MOUSEBUTTONS,WA_CustomScreen,sc,TAG_DONE))
		{
			...
			struct BitMap *bmn  = AllocBitMap(32,8,1,0,NULL);
			struct Message		*msg;
			int	y;
			...
			BltBitMap(&bm,0,0,bmn,0,0,32,8,0xc0,0xff,NULL);
			...
			for(y=0; y<16; y++){
				BltBitMap(bmn,  4, 0, rp->BitMap, 16+0*32+8, 24+13*y, 8, 8, (y<<4), MASK, NULL);
			}

			for(y=0; y<16; y++){
				BltBitMap(bmn,  0, 0, rp->BitMap, 16+1*32+8, 24+13*y, 16, 8, (y<<4), MASK, NULL);
			}

			for(y=0; y<16; y++){
				BltBitMap(bmn,  0, 0, rp->BitMap, 16+2*32+8, 24+13*y, 32, 8, (y<<4), MASK, NULL);
			}
