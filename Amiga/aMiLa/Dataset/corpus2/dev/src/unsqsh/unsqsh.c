...
void unpack(byte *src, byte *dst, int len);
...
int main(int ac,char *av[]) {
  ...
  if ('X' != getchar() ||
      ...
      'K' != getchar() ||
      'F' != getchar()) {
    fprintf(stderr,"no XPK file\n");
    exit(1);
  }
  srclen  = getchar(); srclen <<= 8;
  srclen |= getchar(); srclen <<= 8;
  srclen |= getchar(); srclen <<= 8;
  srclen |= getchar();
  fprintf(stderr,"source file length      : %d bytes\n",srclen+8);
  if ('S' != getchar() ||
      'Q' != getchar() ||
      'S' != getchar() ||
      'H' != getchar()) {
    fprintf(stderr,"no SQSH file\n");
    exit(1);
  }
  dstlen  = getchar(); dstlen <<= 8;
  dstlen |= getchar(); dstlen <<= 8;
  dstlen |= getchar(); dstlen <<= 8;
  dstlen |= getchar();
  fprintf(stderr,"destination file length : %d bytes\n",dstlen);
  ...
  if (1 != fread(src,srclen-8,1,stdin)) {
    fprintf(stderr,"error reading source file\n");
    exit(1);
  }

  unpack(src,dst,dstlen);
...
int bfextu(byte *p,int bo,int bc) {
  int r;
...
int bfexts(byte *p,int bo,int bc) {
  int r;
