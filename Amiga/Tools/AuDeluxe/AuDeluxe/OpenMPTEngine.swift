...
nonisolated private func isPlayable(fileURL: URL, supportedExtensions: [String]) -> Bool {
    supportedExtensions.contains(fileURL.pathExtension.lowercased())
}

nonisolated private func getMetadata(for fileURL: URL, ratingKey: String, titleKey: String, artistKey: String) -> PlaylistItem? {
...
nonisolated private func setAttribute(key: String, value: Any, forFileAt fileURL: URL, in musicFolderURL: URL) {
    guard musicFolderURL.startAccessingSecurityScopedResource() else { return }
    defer { musicFolderURL.stopAccessingSecurityScopedResource() }
