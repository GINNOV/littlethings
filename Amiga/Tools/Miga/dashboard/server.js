...
const RUN_MODE_PRESETS = {
  ...
  copper_reference: {
    label: 'Copper reference benchmark',
    source: null,
    env: {},
  },
  copper_mutation: {
    label: 'Copper mutation benchmark',
    source: null,
    env: {},
  },
};
...
function normalizeSettingsProfile(raw) {
  ...
  profile.dashboard.defaults.runMode = normalizeChoice(
    dashboardDefaults.runMode,
    Object.keys(RUN_MODE_PRESETS),
...
function historyRunModeLabel(mode) {
  switch (String(mode || '').trim()) {
    ...
    case 'copper_benchmark':
      return 'Copper reference benchmark';
    case 'copper_mutation':
    case 'copper_source_benchmark':
      return 'Copper mutation benchmark';
...
function getRunModePreset(mode) {
  return RUN_MODE_PRESETS[mode] || RUN_MODE_PRESETS.corpus_validation;
}

function isBenchmarkRunMode(mode) {
  return mode === 'copper_reference' || mode === 'copper_mutation';
}

function benchmarkEvalScriptForMode(mode) {
  if (mode === 'copper_mutation') {
    return AMIGA_SOURCE_BENCHMARK_EVAL_SCRIPT;
  }
  return AMIGA_BENCHMARK_EVAL_SCRIPT;
}
...
async function executeIteration(job, iterationIdx) {
  ...
  const evalScript = isBenchmarkRunMode(job.runMode) ? benchmarkEvalScriptForMode(job.runMode) : AMIGA_EVAL_SCRIPT;
  const evalScriptRel = path.relative(ROOT, evalScript);
  state.current.phase = isBenchmarkRunMode(job.runMode) ? 'benchmark-evaluate' : 'evaluate';
  pushLiveLog(`[eval] python3 ${evalScriptRel}`);
  const evalEnv = {};
  ...
  const evalResult = await runProcess('python3', [evalScriptRel], {
    cwd: ROOT,
    prefix: '[eval]',
    env: evalEnv,
    onLine: (line) => pushLiveLog(line),
...
async function enrichHistoryRow(row) {
  ...
  const candidates = ['benchmark_capture.png', 'emulator_capture.png'];
  let screenshotUrl = '';
  ...
  const isBenchmark = reportMode === 'benchmark';
...
async function main() {
  ...
  app.post('/api/run', requireRunControlAuth, async (req, res) => {
    ...
    const job = {
      ...
      iterations,
      runMode:
        typeof body.runMode === 'string' && Object.prototype.hasOwnProperty.call(RUN_MODE_PRESETS, body.runMode)
          ? body.runMode
          : 'corpus_validation',
