...
class ADFService {
    ...
    // Standard Kickstart 1.3 Bootblock (OFS)
    ...
    // Standard Kickstart 2.0+ Bootblock (FFS)
    ...
    internal func populateDiskInfo() {
        ...
        if adfReadBootBlock(vol, &bootBlock) == ADF_RC_OK {
            ...
            if isBootable {
                // The fourth byte of the DOSType indicates compatibility.
                // 0 is standard for Kickstart 1.3 (OFS), 1 is standard for 2.0+ (FFS).
                // Other values indicate more advanced filesystems or custom bootblocks.
                switch dosTypeBytes[3] {
                case 0:
                    bootBlockType = "Kickstart 1.3 Compatible"
                case 1:
                    bootBlockType = "Kickstart 2.0+ Compatible"
                default:
