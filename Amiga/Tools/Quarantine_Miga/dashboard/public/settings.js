...
function setRunModeOptions(select, modes) {
  if (!select) return;
  const current = String(select.value || '').trim();
  select.innerHTML = '';
  for (const mode of Array.isArray(modes) ? modes : []) {
    ...
    if (current === mode.id) option.selected = true;
    select.appendChild(option);
  }
}
  ...
  return {
    dashboard: {
      defaults: {
        ...
        runMode: stringValue(el.dashboardRunMode) || 'copper_bars_suite',
...
function applyProfileToForm(profile) {
  ...
  el.dashboardRunMode.value = String(dashboard.runMode || 'copper_bars_suite');
...
function applySettingsPayload(payload) {
  ...
  availableRunModes = Array.isArray(payload.runModes) ? payload.runModes : [];
  ...
  setRunModeOptions(el.dashboardRunMode, availableRunModes);
