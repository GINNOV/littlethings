...
```bash
python amiga_eval_benchmark_source.py --benchmark-config amiga_workspace/benchmarks/copper_bars/source_benchmark_midband.json
```

Run the two-task suite for the inline-assembly workspace:

```bash
python amiga_eval_benchmark_suite.py --benchmark-config amiga_workspace/benchmarks/copper_bars/asm_mutation_benchmark_suite.json
```

Run the first setup-sensitive benchmark, which adds a visible bitplane marker so
display-window and fetch changes show up on screen:

```bash
python amiga_eval_benchmark_source.py --benchmark-config amiga_workspace/benchmarks/copper_bars/asm_playfield_marker_benchmark.json
```

Validate the intentionally wrong setup control against that same crop:

```bash
python amiga_eval_benchmark_source.py --benchmark-config amiga_workspace/benchmarks/copper_bars/asm_playfield_structure_benchmark.json
```

Run the first temporal scroller runway suite:

```bash
python amiga_eval_benchmark_suite.py --benchmark-config amiga_workspace/benchmarks/sincos_scroller/reference_suite.json
```

Check the broken scroller control against that same three-frame suite:

```bash
python amiga_eval_benchmark_suite.py --benchmark-config amiga_workspace/benchmarks/sincos_scroller/control_suite.json
```

Run the degradable sin/cos mutation suite:

```bash
python amiga_eval_benchmark_suite.py --benchmark-config amiga_workspace/benchmarks/sincos_scroller/mutation_suite.json
```

Run the dedicated keep/revert loop for the sin/cos scroller:

```bash
python amiga_sincos_mutation_loop.py --iterations 10
```

Probe the first experimental audio benchmark groundwork:

```bash
python amiga_eval_benchmark_memory.py --benchmark-config amiga_workspace/benchmarks/audio_tone/reference_benchmark.json
```

Run a native vAmiga RetroShell script through the app UI with:
...
This currently uses the app's `Machine -> Import script...` path plus macOS UI automation, and is the first reusable native-vAmiga control surface for future audio/video capture work.

Current status of that audio family:

- `amiga_workspace/run_vamigaweb.js` can now export wasm audio-buffer captures and raw memory captures
- `amiga_workspace/benchmarks/audio_tone/reference/current.s` is a simple Paula tone source
- `amiga_workspace/benchmarks/audio_tone/control/current.s` is the wrong-pitch control
- the benchmark is not validated yet because headless vAmigaWeb is still returning zeroed PCM for this source, and direct HEAP memory capture has not yet been proven to map Amiga RAM addresses correctly
- use it as groundwork, not as a trusted pass/fail signal yet

Scaffold a new benchmark family template with:

```bash
python tools/new_amiga_benchmark.py "My New Benchmark"
```

This generates the repetitive folder/config/docs layout under
`amiga_workspace/benchmarks/<slug>/` so you only need to fill in the real source,
golden captures, crop bounds, and thresholds.

Reset the mutable scroller back to its degraded seed state before another recovery run:
...
That assembly-source workspace currently passes the two-task suite:

- full crop score: `0.999007`
- midband crop score: `1.000000`
- suite average: `0.999504`

The older pair-file workspace still passes the original full-crop benchmark, but
it does not pass the two-task suite:

- full crop score: `0.999007`
- midband crop score: `0.997220`
- suite average: `0.998113`

There is now a separate setup-sensitive benchmark family as well:

- playfield-marker pass source score: `1.000000`
- intentionally wrong setup control score: `0.818565`

That benchmark uses a visible bitplane marker in the top-left playfield area, so
wrong display-window and fetch constants become objectively visible instead of
slipping through the Copper-only bars task.

There is now also a temporal sin/cos text benchmark family:

- `amiga_workspace/benchmarks/sincos_scroller/reference/current.s` renders a
  glyph-based text band and passes a three-frame suite at `2s`, `4s`, and `6s`
- `amiga_workspace/benchmarks/sincos_scroller/control/current.s` keeps the text
  fixed in place and fails that same suite
- `amiga_workspace/benchmarks/sincos_scroller/mutation/current.s` is a degraded
  motion workspace used by `amiga_sincos_mutation_loop.py`
- `amiga_workspace/benchmarks/sincos_scroller/mutation/reference.s` is the
  passing block-by-block reference for that loop
- `amiga_workspace/benchmarks/sincos_scroller/mutation/seed.s` lets you reset
  the mutable source to the degraded baseline before another recovery run
- the current milestone is a stable byte-aligned text scroller benchmark, not
  full smooth-pixel scrolling yet

That gives the project its first benchmarked text-effect path toward a real
scrolling-text-with-sine/cosine generator.
