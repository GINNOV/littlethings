...
BEGIN
  ...
  IF text.autoout THEN
    ...
    text.leftout:=SHORT(SHORT(tfont.xSize/2+0.5));
  END;
END Init;
...
BEGIN
  ...
  IF gad.autoout THEN
    ...
    gad.leftout:=SHORT(SHORT(tfont.xSize/2+0.5));
  END;
END Init;
...
BEGIN
  ...
  END;
  i:=gad.minvisible;
  gad.minwi:=gad.minwi+i*tfont.xSize+12;
  gad.minhe:=tfont.ySize+6;
  gad.width:=gad.minwi;
  gad.height:=gad.minhe;
  IF gad.autoout THEN
...
BEGIN
  ...
  WHILE gad.textarray[i]#NIL DO
    ...
    INC(i);
  END;
  gad.minwi:=wi+SHORT(SHORT(gt.mxWidth/8*tfont.ySize))+tfont.xSize;
(*  IF gad.text#NIL THEN
...
(*  IF gad.text#NIL THEN
  ...
  ELSE
    gad.minwi:=0;
  END;*)
  gad.minwi:=SHORT(2*gad.levellen*tfont.xSize)+tfont.xSize;
  gad.minhe:=tfont.ySize+2;
  gad.width:=gad.minwi;
  gad.height:=gad.minhe;
  IF gad.autoout THEN
...
BEGIN
  ...
  ELSIF i<4 THEN
    i:=4;
  END;
  gad.minwi:=tfont.xSize*2+tfont.xSize*i;
...
BEGIN
  IF box.bordertype=noBorder THEN
    ...
    leftout:=SHORT(SHORT(tfont.xSize/2+0.5));
  ELSE
    topout:=SHORT(SHORT(tfont.ySize/3+0.5));
    leftout:=SHORT(SHORT(tfont.xSize/1.5+0.5));
  END;
        ...
        IF node.prev IS Box THEN
          IF node.prev(Box).bordertype#noBorder THEN
            ...
            ELSIF box.boxorient=horizBox THEN
              node.leftout:=SHORT(SHORT(tfont.xSize/1.5+0.5));
            END;
          END;
        END;
      END;
...
BEGIN
  IF gad.text#NIL THEN
    gad.textwi:=TextLength(gad,gad.text)+tfont.xSize;
  ELSE gad.textwi:=0;
  END;
END SetTextWidth;
...
(* Same text width *)
  ...
  IF cancelGad IN root.gadgets THEN
    width:=width+g.TextLength(fontrast,std.canceltext^,st.Length(std.canceltext^))+8+SHORT(SHORT(tfont.xSize/2+0.5));
  ...
  IF helpGad IN root.gadgets THEN
    width:=width+g.TextLength(fontrast,std.helptext^,st.Length(std.helptext^))+8+SHORT(SHORT(tfont.xSize/2+0.5));
  ...
  IF width>root.minwi THEN
    root.minwi:=width;
  END;
  minwi:=root.minwi+tfont.xSize*2;
...
(*  IF gad.text#NIL THEN
  ...
  ELSE
    i:=0;
  END;*)
  i:=SHORT(gad.levellen*tfont.xSize)+tfont.xSize;
...
BEGIN
  ...
  x:=wind.borderLeft+tfont.xSize;
  y:=wind.borderTop+(tfont.ySize DIV 2);
  width:=wind.width-wind.borderLeft-wind.borderRight-tfont.xSize*2;
...
(* Check for standard gadgets *)
  ...
  IF root.gadgets#LONGSET{} THEN
    ...
    IF cancelGad IN root.gadgets THEN
      ...
      END;
    END;
    dx:=SHORT(SHORT(tfont.xSize/2+0.5));
    ...
    END;
    IF cancelGad IN root.gadgets THEN
      x:=wind.width-wind.borderRight-tfont.xSize-mincawi;
...
PROCEDURE SetRootBox*(boxorient:INTEGER;bordertype:LONGINT;recessed:BOOLEAN;title:e.STRPTR;gadgets:LONGSET;screen:I.ScreenPtr;ffont:g.TextAttrPtr):RootPtr;
...
BEGIN
  ...
  END;
  tfont:=gr.SetFont(fontrast,font);
  vi:=rg.StartGadgets(screen,font);
  vinfo:=vi;
  IF vinfo#NIL THEN
    NEW(root(Root));
    WITH root: Root DO
...
PROCEDURE (root:RootPtr) EndRoot*():I.GadgetPtr;

VAR node : l.Node;

BEGIN
  IF laststring#NIL THEN
    laststring.nextstring:=firststring;
  END;
  IF actbox#NIL THEN
    node:=actbox.prev;
    actbox.Remove;
...
PROCEDURE NewBox*(boxorient:INTEGER;bordertype:LONGINT;recessed:BOOLEAN;title:e.STRPTR):BoxPtr;
...
PROCEDURE (root:RootPtr) NewSameTextWidth*;

VAR node : l.Node;

BEGIN
  NEW(node(SameTextWi));
  node(SameTextWi).list:=l.Create();
  root(Root).sametextwi.AddTail(node);
END NewSameTextWidth;

PROCEDURE (root:RootPtr) AddSameTextWidth*(gad:GadgetPtr);
...
BEGIN
  IF root.sametextwi.tail#NIL THEN
    ...
    root.sametextwi.tail(SameTextWi).list.AddTail(node);
  END;
END AddSameTextWidth;

PROCEDURE SetText*(text:e.STRPTR;color:INTEGER):TextPtr;
...
BEGIN
  ...
  IF node#NIL THEN
    ...
    WITH node: Text DO
      ...
      node.sizex:=-1;
      node.sizey:=-1;
      node.minwi:=g.TextLength(fontrast,text^,st.Length(text^));
      node.minhe:=tfont.ySize;
      node.autoout:=TRUE;
      node.root:=actroot;
    END;
  END;
...
PROCEDURE SetBooleanGadget*(text:e.STRPTR):BooleanGadgetPtr;

VAR gad  : I.GadgetPtr;
    node : GadgetPtr;

BEGIN
  gad:=rg.SetBooleanGadget(-tfont.xSize*10-10,-tfont.ySize*2-10,tfont.xSize*10,tfont.ySize*2,text,prev);
  node:=NIL;
  ...
  IF node#NIL THEN
    ...
    WITH node: BooleanGadget DO
      ...
      node.sizex:=0;
      node.sizey:=1;
      node.minwi:=g.TextLength(fontrast,text^,st.Length(text^))+8;
      node.minhe:=tfont.ySize+6;
      node.autoout:=TRUE;
      node.root:=actroot;
    END;
  END;
...
PROCEDURE SetStringGadget*(text:e.STRPTR;maxchars:INTEGER):StringGadgetPtr;

VAR gad  : I.GadgetPtr;
    node : GadgetPtr;

BEGIN
  gad:=rg.SetStringGadget(-tfont.xSize*10-10,-tfont.ySize*2-10,tfont.xSize*10,tfont.ySize*2,text,maxchars,prev);
  node:=NIL;
  NEW(node(StringGadget));
  IF node#NIL THEN
    IF laststring#NIL THEN
      laststring.nextstring:=node(StringGadget);
    ELSE
      firststring:=node(StringGadget)
    END;
    laststring:=node(StringGadget);
    AddObject(node);
    WITH node: StringGadget DO
      ...
      node.sizey:=-1;
      IF text#NIL THEN
        node.minwi:=g.TextLength(fontrast,text^,st.Length(text^))+12;
      ELSE
        node.minwi:=0;
      END;
      node.minhe:=tfont.ySize+6;
...
PROCEDURE SetCycleGadget*(text:e.STRPTR;textarray:s.ADDRESS;numactive:INTEGER):CycleGadgetPtr;

VAR gad  : I.GadgetPtr;
    node : GadgetPtr;

BEGIN
  gad:=rg.SetCycleGadget(-tfont.xSize*10-10,-tfont.ySize*2-10,tfont.xSize*10,tfont.ySize*2,text,textarray,numactive,prev);
  node:=NIL;
...
PROCEDURE SetCheckboxGadget*(text:e.STRPTR;checked:BOOLEAN):CheckboxGadgetPtr;

VAR gad  : I.GadgetPtr;
    node : GadgetPtr;

BEGIN
  gad:=rg.SetBooleanGadget(-tfont.xSize*10-10,-tfont.ySize*2-10,tfont.xSize*2,tfont.ySize,text,prev);
  node:=NIL;
...
PROCEDURE SetPaletteGadget*(depth,numcols,actcol:INTEGER):PaletteGadgetPtr;

VAR gad  : I.GadgetPtr;
    node : GadgetPtr;

BEGIN
  gad:=rg.SetPaletteGadget(-tfont.xSize*10-10,-tfont.ySize*2-10,tfont.xSize*10,tfont.ySize*2,NIL,depth,numcols,actcol,prev);
  node:=NIL;
...
PROCEDURE SetRadioButtons*(textarray:s.ADDRESS;numactive:INTEGER):RadioButtonsPtr;

VAR gad  : I.GadgetPtr;
    node : GadgetPtr;

BEGIN
  gad:=rg.SetRadioGadget(-tfont.xSize*10-10,-tfont.ySize*2-10,tfont.xSize*10,tfont.ySize*2,NIL,textarray,numactive,1,prev);
  node:=NIL;
...
PROCEDURE SetSliderGadget*(min,max,level,levellen:LONGINT;formatstring:e.STRPTR):SliderGadgetPtr;

VAR gad  : I.GadgetPtr;
    node : GadgetPtr;

BEGIN
  gad:=rg.SetSliderGadget(-tfont.xSize*10-10,-tfont.ySize*2-10,tfont.xSize*10,tfont.ySize*2,NIL,min,max,level,levellen,I.lorientHoriz,formatstring,prev);
  node:=NIL;
  ...
  IF node#NIL THEN
    ...
    WITH node: SliderGadget DO
      ...
      node.sizex:=5;
      node.sizey:=0;
      node.minwi:=SHORT(2*levellen*tfont.xSize);
      node.minhe:=tfont.ySize+2;
      node.autoout:=TRUE;
      node.root:=actroot;
    END;
...
PROCEDURE SetScrollerGadget*(total,visible,top:LONGINT;arrows:BOOLEAN;orientation:LONGINT;thickness:INTEGER):ScrollerGadgetPtr;

VAR gad  : I.GadgetPtr;
    node : GadgetPtr;

BEGIN
  gad:=rg.SetScrollerGadget(-tfont.xSize*10-10,-tfont.ySize*2-10,tfont.xSize*10,tfont.ySize*2,NIL,total,visible,top,arrows,orientation,prev);
  node:=NIL;
...
PROCEDURE SetListView*(title:e.STRPTR;datalist:l.List;printproc:PrintProc;actentry:INTEGER;stringgadact:BOOLEAN;maxchars:INTEGER):ListViewPtr;
  ...
  IF node#NIL THEN
    ...
    WITH node: ListView DO
      ...
      IF datalist#NIL THEN
        node.total:=SHORT(datalist.nbElements());
      END;
      node.standard:=TRUE;
      node.visible:=2;
      node.top:=0;
      ...
      node.itemheight:=tfont.ySize+2;
      node.scrollgad:=rg.SetScrollerGadget(-tfont.xSize*10-10,-tfont.ySize*2-10,tfont.xSize*10,tfont.ySize*2,NIL,node.total,node.visible,node.top,TRUE,I.lorientVert,prev);
      IF stringgadact THEN
        node.stringgad:=rg.SetStringGadget(-tfont.xSize*10-10,-tfont.ySize*2-10,tfont.xSize*10,tfont.ySize*2,NIL,maxchars,prev);
      END;
      node.sizex:=100;
      node.sizey:=200;
      node.minwi:=tfont.xSize*2;
...
PROCEDURE SetGradientSlider*(numpens:INTEGER;thickness:INTEGER):GradientSliderPtr;

VAR gad  : I.GadgetPtr;
    node : GadgetPtr;

BEGIN
  gad:=rg.SetGradientSlider(-50,-50,10,40,s.ADR(gradpens),I.freeVert,prev);
  node:=NIL;
...
PROCEDURE SetColorWheel*(slider:GadgetPtr):ColorWheelPtr;
...
BEGIN
  IF slider#NIL THEN
    gad:=rg.SetColorWheel(-50,-50,40,40,slider.gadget,prev);
  ELSE
    gad:=rg.SetColorWheel(-50,-50,40,40,NIL,prev);
  END;
  node:=NIL;
  NEW(node(ColorWheel));
  IF node#NIL THEN
...
PROCEDURE GetIMes*(root:ObjectPtr;VAR class:LONGSET;VAR code:INTEGER;VAR address:s.ADDRESS);
...
PROCEDURE CheckObject(node:l.Node);
...
BEGIN
  IF node IS Box THEN
    node2:=node(Box).objectlist.head;
    WHILE node2#NIL DO
      CheckObject(node2);
      node2:=node2.next;
    END;
  ELSIF node IS ListView THEN
    WITH node: ListView DO
      IF I.gadgetDown IN class THEN
        IF address=node.scrollgad THEN
          node.top:=code;
          node.Refresh;
          old:=code;
          REPEAT
            ...
            code:=0;
            address:=NIL;
            message:=gt.GetIMsg(wind.userPort);
            IF message#NIL THEN
              class:=message.class;
              code:=message.code;
            ...
            END;
            node.top:=rg.GetScroller(node.scrollgad,wind);
            IF old#node.top THEN
              node.Refresh;
              old:=node.top;
            END;
      ...
      ELSIF I.mouseButtons IN class THEN
        IF code=I.selectDown THEN
          ...
          x:=message.mouseX;
          y:=message.mouseY-node.titlehe;
          IF (x>=node.xpos) AND (x<node.xpos+node.width-node.scrollerwi) AND (y>=node.ypos) AND (y<node.ypos+node.height-node.stringhe) THEN
            oldact:=node.actentry;
            REPEAT
              ...
              IF message#NIL THEN
                ...
                ELSE
                  x:=wind.mouseX;
                  y:=wind.mouseY-node.titlehe;
                END;
              END;
              old:=node.actentry;
              oldtop:=node.top;
              node.actentry:=node.top+((y-node.ypos-2) DIV node.itemheight);
              IF y>=node.ypos+2+node.itemheight*node.visible THEN
                node.actentry:=node.top+node.visible;
              ELSIF y<node.ypos+2 THEN
                node.actentry:=node.top-1;
              END;
              IF node.actentry>=node.total THEN
                node.actentry:=node.total-1;
              END;
              IF node.actentry<0 THEN
                node.actentry:=0;
              END;
              IF old#node.actentry THEN
                node.BackFill(old,FALSE);
                node.BackFill(node.actentry,TRUE);
              END;
              IF node.actentry<node.top THEN
(*                node.top:=node.actentry;*)
                node.Scroll(-1);
              ELSIF node.actentry>=node.top+node.visible THEN
(*                node.top:=node.actentry-node.visible+1;*)
                node.Scroll(1);
              END;
              IF oldtop#node.top THEN
(*                node.Refresh;*)
                IF oldtop#node.top THEN
                  gt.SetGadgetAttrs(node.scrollgad^,wind,NIL,gt.scTop,node.top,
                                                             u.done);
                  d.Delay(3);
                END;
              END;
            UNTIL I.mouseButtons IN class;
            class:=LONGSET{};
            IF oldact#node.actentry THEN
              address:=node.scrollgad;
              class:=LONGSET{I.gadgetUp};
              code:=newActEntry;
              node.sec:=sec;
              node.mic:=mic;
            ELSE
              IF I.DoubleClick(node.sec,node.mic,sec,mic) THEN
                address:=node.scrollgad;
                class:=LONGSET{I.gadgetUp};
                code:=doubleClicked;
                node.sec:=0;
      ...
      END;
    END;
  ELSIF node IS Gadget THEN
    IF I.gadgetUp IN class THEN
      IF address=node(Gadget).gadget THEN
        IF node IS CheckboxGadget THEN
          node(CheckboxGadget).checked:=NOT(node(CheckboxGadget).checked);
        ELSIF node IS StringGadget THEN
          IF node(StringGadget).nextstring#NIL THEN
            bool:=I.ActivateGadget(node(StringGadget).nextstring.gadget^,wind,NIL);
          END;
        ELSE
          node(Gadget).SetValue(code);
        END;
      END;
    ...
    ELSIF I.vanillaKey IN class THEN
      IF (node(Gadget).shortcut#0X) AND (st.CapIntl(CHR(code))=st.CapIntl(node(Gadget).shortcut)) THEN
        node(Gadget).KeyboardAction;
        address:=node(Gadget).gadget;
        class:=LONGSET{I.gadgetUp};
        code:=0;
      END;
    END;
...
BEGIN
  ...
  IF message#NIL THEN
    ...
    IF I.newSize IN class THEN
      root(Root).ResizeRoot(wind);
    ELSE
      check:=TRUE;
      IF I.vanillaKey IN class THEN
        IF (root(Root).oksc#0X) AND (st.CapIntl(CHR(code))=st.CapIntl(root(Root).oksc)) THEN
          PressBoolean(root(Root).ok,wind);
          class:=LONGSET{I.gadgetUp};
          code:=0;
          address:=root(Root).ok;
          check:=FALSE;
        ELSIF (root(Root).casc#0X) AND (st.CapIntl(CHR(code))=st.CapIntl(root(Root).casc)) THEN
          PressBoolean(root(Root).ca,wind);
          class:=LONGSET{I.gadgetUp};
          code:=0;
          address:=root(Root).ca;
          check:=FALSE;
        ELSIF (root(Root).helpsc#0X) AND (st.CapIntl(CHR(code))=st.CapIntl(root(Root).helpsc)) THEN
          PressBoolean(root(Root).help,wind);
          class:=LONGSET{I.gadgetUp};
          code:=0;
          address:=root(Root).help;
          check:=FALSE;
        END;
      ELSIF (I.rawKey IN class) OR (I.gadgetUp IN class) THEN
      ...
      END;
      IF check THEN
        node:=root(Root).objectlist.head;
        WHILE node#NIL DO
          CheckObject(node);
          node:=node.next;
        END;
