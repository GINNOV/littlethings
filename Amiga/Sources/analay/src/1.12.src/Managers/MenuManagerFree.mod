...
BEGIN
  ...
  diff:=add*(width+4);
  diff:=screen.width-(diff+menuxpos+xpos)-6;
  IF diff<0 THEN
    item:=first;
    WHILE item#NIL DO
...
BEGIN
  ...
  item:=first;
  WHILE item#NIL DO
    DoSubStrip(item.subItem,maxheight-item.topEdge-2,item.leftEdge,itemwidth+10);
    INC(actsubnum);
    item:=item.nextItem;
  END;
END DoStrip;
...
BEGIN
  point:=e.AllocMem(st.Length(name)+1,LONGSET{e.memClear});
  COPY(name,point^);
  RETURN s.ADR(point^);
END AllocName;

PROCEDURE NewMenu*(name:ARRAY OF CHAR);
...
BEGIN
  ...
  lastmenu:=actmenu;
  actmenu:=NIL;
  actmenu:=e.AllocMem(s.SIZE(I.Menu),LONGSET{e.memClear});
...
BEGIN
  ...
  intui.nextText:=NIL;
  IF right[0]#0X THEN
    intui2:=e.AllocMem(s.SIZE(I.IntuiText),LONGSET{e.memClear});
    intui2.frontPen:=screen.detailPen;
    intui2.backPen:=screen.blockPen;
    intui2.drawMode:=g.jam2;
    intui2.leftEdge:=g.TextLength(rast,right,st.Length(right));
    intui2.topEdge:=1;
...
BEGIN
  lastitem:=actitem;
  actitem:=NIL;
  actitem:=e.AllocMem(s.SIZE(I.MenuItemPtr),LONGSET{e.memClear});
...
BEGIN
  lastsub:=actsub;
  actsub:=NIL;
  actsub:=e.AllocMem(s.SIZE(I.MenuItem),LONGSET{e.memClear});
  IF lastsub#NIL THEN
...
BEGIN
  lastitem:=actitem;
  actitem:=NIL;
  actitem:=e.AllocMem(s.SIZE(I.MenuItem),LONGSET{e.memClear});
  IF lastitem#NIL THEN
    lastitem.nextItem:=actitem;
  ELSE
    firstitem:=actitem;
...
BEGIN
  ...
  actsub.leftEdge:=0;
  actsub.topEdge:=subheight[actsubnum];
  actsub.width:=0;
  actsub.height:=5;
  INC(stripheight,5);
...
PROCEDURE NewItem*(text:ARRAY OF CHAR;cut:CHAR);

BEGIN
  Item(text,"",cut,FALSE,FALSE);
END NewItem;

PROCEDURE NewItem2*(text,cut:ARRAY OF CHAR);

BEGIN
  Item(text,cut,0X,FALSE,FALSE);
END NewItem2;

PROCEDURE NewCheckItem*(text:ARRAY OF CHAR;cut:CHAR;checked:BOOLEAN);

BEGIN
  Item(text,"",cut,TRUE,checked);
END NewCheckItem;

PROCEDURE NewCheckItem2*(text,cut:ARRAY OF CHAR;checked:BOOLEAN);

BEGIN
  Item(text,cut,0X,TRUE,checked);
END NewCheckItem2;

PROCEDURE NewSubItem*(text:ARRAY OF CHAR;cut:CHAR);

BEGIN
  SubItem(text,"",cut,FALSE,FALSE);
END NewSubItem;

PROCEDURE NewSubItem2*(text,cut:ARRAY OF CHAR);

BEGIN
  SubItem(text,cut,0X,FALSE,FALSE);
END NewSubItem2;

PROCEDURE NewSubCheckItem*(text:ARRAY OF CHAR;cut:CHAR;checked:BOOLEAN);

BEGIN
  SubItem(text,"",cut,TRUE,checked);
END NewSubCheckItem;

PROCEDURE NewSubCheckItem2*(text,cut:ARRAY OF CHAR;checked:BOOLEAN);
...
BEGIN
  ...
  ELSE
    e.FreeMem(item.itemFill,s.SIZE(I.Image));
  END;
  e.FreeMem(item,s.SIZE(I.MenuItem));
END FreeItem;
...
BEGIN
  WHILE menu#NIL DO
    ...
    freemenu:=menu;
    menu:=menu.nextMenu;
    e.FreeMem(freemenu,s.SIZE(I.Menu));
  END;
END FreeMenu;
