...
#ifdef PARANOID
if (count & 1)
	debug_printf("odd count in block_stuff\n");
#endif PARANOID
count >>= 1;
while (--count >= 0)
	*s++ = value;
...
#ifdef PARANOID
if (count & 1)
	debug_printf("odd count in block_zero\n");
#endif PARANOID
count >>= 1;
while (--count >= 0)
	*d++ = 0;
...
{
#ifdef DEBUG
debug_printf("copy_structure(%lx %lx %lx)\n",s,d,count);
#endif DEBUG
#ifdef PARANOID
if (count & 1)
	debug_printf("odd count in copy_structure\n");
#endif PARANOID
count >>= 1;
while(--count>=0)
	*(d++) = *(s++);
...
{
#ifdef DEBUG
debug_printf("or_structure(%lx %lx %lx)\n",s,d,count);
#endif DEBUG

#ifdef PARANOID
if (count & 1)
	debug_printf("odd count in or_structure\n");
#endif PARANOID
...
#ifdef DEBUG
debug_printf("clone_structure(->%lx count0x%lx)", s,count);
#endif DEBUG
...
#ifdef PARANOID
	if (count & 1)
		debug_printf("odd count in clone_structure\n");
#endif PARANOID
	count++;
	count >>= 1;
